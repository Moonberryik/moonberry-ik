/**
 * MOONBERRY İK - LEGACY.JS
 * Henüz modüllere taşınmamış fonksiyonlar
 * @version 20
 */
...
// Firebase değişkenleri - DOMContentLoaded'da initialize edilecek
let db = null;

// ==================== GLOBAL YARDIMCI FONKSİYONLAR ====================
// Türkçe karakter destekli isim normalize - puantajNotes ID için
...
// Tam isimden kısa görüntü formatı: "BÜŞRA SÖĞÜT" → "Büşra S."
...
// ==================== ANA GÖRÜNTÜ FONKSİYONU ====================
// codeName varsa onu, yoksa formatDisplayName(name) döndür
// BU FONKSİYON HER YERDE KULLANILMALI!
function getPersonelDisplayName(personelData) {
    if (!personelData) return '';
    
    // personelData obje ise
    if (typeof personelData === 'object') {
        // codeName varsa ve boş değilse onu kullan
...
async function getPersonelDisplayNameFromList(searchKey, branch) {
        ...
        try {
            ...
            snapshot.forEach(doc => {
                ...
                const keys = [
                    ...
                    data.codeName?.split(' ')[0],
                    data.codeName?.split(' ')[0]?.toLowerCase(),
                    data.codeName?.split(' ')[0]?.toUpperCase(),
                    data.name?.split(' ')[0],
                    ...
                    normalizeTurkishChars(data.name?.split(' ')[0]),
                    normalizeTurkishChars(data.codeName?.split(' ')[0]),
                ].filter(k => k);
                
                keys.forEach(key => {
                    if (!_personelDisplayCache.has(key)) {
            ...
            console.error('Personel cache hatası:', e);
            return searchKey;
        }
    }
    ...
    const result = _personelDisplayCache.get(searchKey) || 
                   ...
                   _personelDisplayCache.get(normalizeTurkishChars(searchKey)) ||
    ...
    return searchKey; // Bulunamadı, gelen değeri döndür
}

// Cache'i temizle (yeni personel eklendiğinde)
...
// Helper: Türkçe karakterleri normalize et
// ÖNEMLİ: Önce büyük Türkçe harfleri replace et, sonra toLowerCase!
function normalizeTurkishChars(str) {
    if (!str) return '';
    return str
        // Önce büyük Türkçe harfleri küçült (toLowerCase'dan önce!)
        .replace(/İ/g, 'i').replace(/I/g, 'i')
        .replace(/Ğ/g, 'g').replace(/Ü/g, 'u').replace(/Ş/g, 's')
        .replace(/Ö/g, 'o').replace(/Ç/g, 'c')
        // Sonra toLowerCase
        .toLowerCase()
        // Sonra küçük Türkçe harfleri normalize et
        .replace(/ğ/g, 'g').replace(/ü/g, 'u').replace(/ş/g, 's')
        .replace(/ı/g, 'i').replace(/ö/g, 'o').replace(/ç/g, 'c')
        .replace(/i̇/g, 'i'); // combining dot varsa temizle
}

// Personel map oluştur: displayName/codeName → {uniqueId, fullName}
// Belirsiz key'ler (aynı ilk isim için birden fazla personel) işaretlenir
async function buildPersonelMap() {
    ...
    // Önce tüm personeli topla ve ilk isim sayılarını hesapla
    const allPersonel = [];
    const firstNameCount = {}; // "BÜŞRA" → 2 gibi
    ...
    snapshot.forEach(doc => {
        ...
        // İlk isim sayısını artır (branch bazlı)
    ...
    allPersonel.forEach(p => {
        ...
        // Bu ilk isim belirsiz mi? (aynı branch'te 2+ kişi)
        const countKey = `${branch}_${firstNameNorm}`;
        const isAmbiguous = firstNameCount[countKey] > 1;
        
        // Kesin eşleşme key'leri (her zaman ekle)
        const certainKeys = [
            // Tam isim varyasyonları
            fullName,
            fullName?.toLowerCase(),
            fullName?.toUpperCase(),
            normalizeTurkishChars(fullName),
            
            // Display name varyasyonları (codeName veya "İsim S.")
            displayName,
            displayName?.toLowerCase(),
            displayName?.toUpperCase(),
            normalizeTurkishChars(displayName),
            
            // Kod adı varyasyonları (TAM)
            data.codeName,
            data.codeName?.toLowerCase(),
            data.codeName?.toUpperCase(),
            normalizeTurkishChars(data.codeName),
        ...
        const ambiguousKeys = isAmbiguous ? [] : [
            // İlk isim varyasyonları (BÜŞRA, Büşra, busra)
            firstName,
        ...
        if (isAmbiguous) {
            console.log(`[Migration] ⚠️ Belirsiz ilk isim: "${firstName}" (${branch}) - ${firstNameCount[countKey]} kişi`);
        }
    });
    
    console.log(`[Migration] Personel map oluşturuldu: ${_migrationPersonelMap.size} key`);
...
async function migratePersonelUniqueIds() {
    ...
    console.log('[Migration] 1/3 Personel uniqueId migrasyon başlıyor...');
    ...
    try {
        ...
        for (const doc of snapshot.docs) {
            ...
            console.log(`  ✓ ${data.name} → ${uniqueId}`);
            updated++;
        }
        
        console.log(`[Migration] Personel: ${updated} güncellendi, ${skipped} atlandı`);
        return { updated, skipped };
    } catch (e) {
        console.error('[Migration] Personel hatası:', e);
        return { updated: 0, skipped: 0, error: e.message };
    }
}

// 2. Shift verilerini güncelle - KEY'LERİ DEĞİŞTİRME, SADECE MAPPING EKLE
async function migrateShiftData() {
    ...
    console.log('[Migration] 2/4 Shift mapping güncelleniyor (key değişikliği YOK)...');
    ...
    try {
        ...
        for (const doc of shiftsSnapshot.docs) {
            ...
            // Key'leri DEĞİŞTİRME - sadece mapping ekle
            for (const oldKey of Object.keys(personelShifts)) {
                ...
                if (personelInfo) {
                    ...
                    hasChanges = true;
                    console.log(`  ✓ ${oldKey} → display: "${personelInfo.displayName}"`);
                } else {
                    // Bulunamadı - key'i displayName olarak kullan
                    if (!newPersonelNames[oldKey]) {
                        newPersonelNames[oldKey] = oldKey;
                        newPersonelFullNames[oldKey] = oldKey;
                        hasChanges = true;
                        console.warn(`  ⚠️ "${oldKey}" için personel bulunamadı, olduğu gibi bırakıldı`);
                    }
                }
            }
        ...
        console.log(`[Migration] Shift: ${updated} güncellendi, ${skipped} atlandı`);
        return { updated, skipped };
    } catch (e) {
        console.error('[Migration] Shift hatası:', e);
        return { updated: 0, skipped: 0, error: e.message };
    }
}

// 3. Eski puantajNotes kayıtlarını temizle
async function cleanOldPuantajNotes() {
    ...
    try {
        ...
        for (const doc of snapshot.docs) {
            ...
            // Eski format kontrolü: personelId kısa isim formatında mı?
            // Yeni format: busra_sogut (tam isim normalize)
            // Eski format: busra, b__ra, balra (kısa isim)
            ...
            // Eski format: tek kelime veya çok kısa
        ...
        // Batch halinde sil (500'lük gruplar)
        const batchSize = 450;
        ...
        console.error('[Migration] PuantajNotes hatası:', e);
        return { deleted: 0, error: e.message };
    }
}

// 4. Shift sıralama düzeltme - BÖLGE MÜDÜRÜ TÜM ŞUBELERDE + ROLE SIRALAMA
async function fixShiftOrdering() {
    console.log('[Migration] 4/4 Shift sıralaması düzeltiliyor...');
    
    try {
        // Önce tüm personeli çek
        const personelSnap = await db.collection('personel').get();
        const allPersonel = [];
        const bolgeMudurleri = []; // Bölge müdürleri TÜM şubelerde görünecek
        ...
        personelSnap.forEach(doc => {
            ...
            // Bölge müdürlerini ayır - TÜM şubelerde görünecekler
            if (p.role === 'bolge_muduru') {
                bolgeMudurleri.push(info);
                console.log(`[Bölge Müdürü] ${info.displayName} tüm şubelerde görünecek`);
            }
        });
        ...
        branches.forEach(branch => {
            // Bu şubenin personeli + TÜM bölge müdürleri
            const branchPersonel = allPersonel.filter(p => p.branch === branch);
            
            // Bölge müdürlerini ekle (zaten bu şubede değilse)
        ...
        // Her şube için RÜTBEYE GÖRE sırala
        ...
        Object.keys(personelByBranch).forEach(branch => {
            ...
            console.log(`[Sıralama] ${branch}:`, personelByBranch[branch].map(p => `${p.displayName}(${p.role})`).join(', '));
        });
        
        // Tüm shift'leri güncelle
        ...
        for (const doc of shiftsSnap.docs) {
            ...
            // Bölge müdürlerini shift'e ekle (yoksa)
            bolgeMudurleri.forEach(bm => {
                const key = bm.displayName;
                if (!personelShifts[key]) {
                    personelShifts[key] = {}; // Boş vardiya
                    personelNames[key] = bm.displayName;
                    personelFullNames[key] = bm.name;
                    console.log(`  ✓ ${branch}: Bölge müdürü "${key}" eklendi`);
                }
            });
            ...
            // Sıralı listedeki her personel için
            ...
            // Sıralamada olmayan personeli EN SONA ekle
        ...
        console.log(`[Migration] Shift sıralama: ${updated} güncellendi`);
        return { updated };
    } catch (e) {
        console.error('[Migration] Shift sıralama hatası:', e);
        return { updated: 0, error: e.message };
    }
}

// ANA MİGRASYON FONKSİYONU
async function fullSystemMigration() {
    console.log('═══════════════════════════════════════════');
    console.log('   🚀 TAM SİSTEM MİGRASYONU BAŞLIYOR');
    console.log('═══════════════════════════════════════════');
    
    const startTime = Date.now();
    
    // 0. Esin'i bölge müdürü yap (codeName "Esin İ." olan)
    console.log('[Migration] 0/4 Bölge müdürü rol kontrolü...');
    try {
        const personelSnap = await db.collection('personel').get();
        for (const doc of personelSnap.docs) {
            const p = doc.data();
            // Esin İ. veya Merve İlgar → bolge_muduru
            if ((p.codeName && p.codeName.toLowerCase().includes('esin')) || 
                ...
                if (p.role !== 'bolge_muduru') {
                    ...
                    console.log(`  ✓ ${p.name || p.codeName} → bolge_muduru`);
                }
            }
        }
    } catch (e) {
        console.warn('[Migration] Bölge müdürü rol hatası:', e);
    }
    ...
    // 4. Shift sıralamasını düzelt + bölge müdürünü tüm şubelere ekle
    ...
    console.log('═══════════════════════════════════════════');
    console.log(`   Personel: ${personelResult.updated} güncellendi`);
    console.log(`   Shift: ${shiftResult.updated} güncellendi`);
    console.log(`   PuantajNotes: ${notesResult.deleted} silindi`);
    console.log(`   Sıralama: ${orderResult.updated} shift düzeltildi`);
    console.log(`   Süre: ${duration} saniye`);
    console.log('═══════════════════════════════════════════');
    
    toast(`Migrasyon tamamlandı! (${duration}s)`, 'success');
...
window.syncAllShiftPersonelNames = syncAllShiftPersonelNames; // Manuel senkronizasyon için
...
// Sayfa yüklendiğinde doldurulur, her yerde kullanılır
let PERSONEL_DISPLAY_CACHE = new Map();

// Cache'i doldur (sayfa yüklendiğinde çağrılır)
async function loadPersonelDisplayCache() {
    try {
        ...
        console.log(`[Cache] Personel cache yüklendi: ${PERSONEL_DISPLAY_CACHE.size} key`);
    } catch (e) {
        console.error('[Cache] Personel cache yüklenemedi:', e);
    }
}
...
function lookupPersonelDisplay(searchKey, branch = null) {
    ...
    if (info && branch && info.branch !== branch) {
        // Branch uyuşmuyor, başka aday ara
        const candidates = [];
...
// ==================== OTOMATİK SHIFT-PERSONEL SENKRONİZASYONU ====================
// Bu fonksiyon TÜM shift'lerdeki personel isimlerini güncel personel verileriyle eşleştirir
async function syncAllShiftPersonelNames() {
    if (!db) return;
    
    try {
        // 1. Tüm personeli çek ve map oluştur
        const personelSnap = await db.collection('personel').get();
        const personelList = [];
        ...
        for (const shiftDoc of shiftsSnap.docs) {
            ...
            // 3. Her key için doğru personeli bul
            for (const key of Object.keys(personelShifts)) {
                const keyLower = key.toLowerCase();
                const keyNorm = normalizeTurkishChars(key);
                const keyFirst = keyNorm.split(/[_\s\.]/)[0];
                
                // Önce aynı şubeden ara, sonra bölge müdürlerinden
                let matchedPersonel = null;
                
                // Öncelik 1: Aynı şubeden + tam eşleşme
                ...
                // Öncelik 2: fullName eşleşmesi (mevcut kayıtlı isimle)
                if (!matchedPersonel && currentFullNames[key]) {
                    const currentFullNorm = normalizeTurkishChars(currentFullNames[key]);
                    matchedPersonel = personelList.find(p => {
                        if (p.branch !== shiftBranch && p.role !== 'bolge_muduru') return false;
                        const pNameNorm = normalizeTurkishChars(p.name);
                        // İlk isim de kontrol et
                        const currentFirst = currentFullNorm.split(/[_\s]/)[0];
                        const pNameFirst = pNameNorm.split(/[_\s]/)[0];
                        return pNameNorm === currentFullNorm || (currentFirst.length > 2 && currentFirst === pNameFirst);
                    });
                }
                
                // Öncelik 3: İlk isim eşleşmesi (aynı şube) - EN GENİŞ
                if (!matchedPersonel && keyFirst && keyFirst.length > 2) {
                    matchedPersonel = personelList.find(p => {
                        if (p.branch !== shiftBranch && p.role !== 'bolge_muduru') return false;
                        
                        const pNameFirst = normalizeTurkishChars(p.name).split(/[_\s]/)[0];
                        const pCodeFirst = normalizeTurkishChars(p.codeName || '').split(/[_\s\.]/)[0];
                        const pDisplayFirst = normalizeTurkishChars(p.displayName).split(/[_\s\.]/)[0];
                        
                        return keyFirst === pNameFirst || keyFirst === pCodeFirst || keyFirst === pDisplayFirst;
                    });
                }
                ...
                if (matchedPersonel) {
                    ...
                    if (newNames[key] !== matchedPersonel.displayName || newFullNames[key] !== matchedPersonel.name) {
                        console.log(`[Senkron] ${shiftBranch}: "${key}" → "${matchedPersonel.displayName}" (${matchedPersonel.name})`);
                        newNames[key] = matchedPersonel.displayName;
                        newFullNames[key] = matchedPersonel.name;
                        needsUpdate = true;
                    }
                } else {
                    // Eşleşme bulunamadı - debug için log
                    if (!newNames[key] || newNames[key] === key) {
                        console.warn(`[Senkron] ⚠️ Eşleşme bulunamadı: "${key}" (${shiftBranch}) - keyNorm: "${keyNorm}", keyFirst: "${keyFirst}"`);
                    }
                }
            }
            
            // 4. Güncelleme gerekiyorsa kaydet
            if (needsUpdate) {
        ...
        if (totalUpdated > 0) {
            console.log(`[Senkron] ✓ Toplam ${totalUpdated} shift güncellendi`);
        } else {
            console.log(`[Senkron] ✓ Tüm shift'ler güncel`);
        }
    } catch (e) {
...
const DEFAULT_STATE = {
    ...
    positions: [
        ...
        {id: 'mudur_yardimcisi', name: 'Müdür Yardımcısı', type: 'Gri Yaka'},
        {id: 'magaza_muduru', name: 'Mağaza Müdürü', type: 'Beyaz Yaka'},
        {id: 'bolge_muduru', name: 'Bölge Müdürü', type: 'Stratejik'}
    ],
    violations: [
        {id:'V01',title:'Telefon Kullanımı',risk:'low',legal:'İş K. 25/II-h',steps:['Sözlü uyarı','Yazılı uyarı','İhtar'],desc:'Mesai saatinde telefon kullanımı'},
        {id:'V02',title:'Geç Kalma (<30dk)',risk:'low',legal:'İş K. 25/II-h',steps:['Sözlü uyarı','Tekrarda yazılı','İhtar'],desc:'30 dakika altı geç kalma'},
        {id:'V03',title:'Geç Kalma (>30dk)',risk:'medium',legal:'İş K. 25/II-g',steps:['Tutanak','Savunma','İhtar','Fesih'],desc:'30 dakika üstü geç kalma'},
        {id:'V04',title:'1 Gün Devamsızlık',risk:'medium',legal:'İş K. 25/II-g',steps:['Tutanak','Savunma','İhtar'],desc:'Mazeretsiz 1 gün devamsızlık'},
        {id:'V05',title:'2+ Gün Devamsızlık',risk:'high',legal:'İş K. 25/II-g',steps:['Her gün tutanak','Savunma','6 iş günü içinde fesih'],desc:'Ardı ardına 2+ gün mazeretsiz devamsızlık - Haklı fesih sebebi'},
        {id:'V06',title:'Kasa Açığı',risk:'medium',legal:'TBK 407',steps:['Sayım tutanağı','Savunma','Borç İkrar'],desc:'Kasada nakit eksikliği tespit edilmesi'},
        {id:'V07',title:'Ürün Götürme',risk:'high',legal:'İş K. 25/II-e',steps:['Tutanak','Kamera kayıt','Savunma','Fesih'],desc:'İzinsiz ürün götürme - Güveni kötüye kullanma'},
        {id:'V08',title:'Müşteriye Hakaret',risk:'high',legal:'İş K. 25/II-d',steps:['Tutanak','Şikayet kaydı','Savunma','Fesih'],desc:'Müşteriye kaba davranış veya hakaret'},
        {id:'V09',title:'İşyerinde Kavga',risk:'high',legal:'İş K. 25/II-d',steps:['İki taraf tutanak','Şahit ifadeleri','Savunma','Fesih'],desc:'İşyerinde fiziksel veya sözlü kavga'},
        {id:'V10',title:'Hijyen İhlali',risk:'high',legal:'İş K. 25/II-ı',steps:['Tutanak','Savunma','Derhal fesih'],desc:'El yıkamama, SKT ihlali, çapraz bulaşma'},
        {id:'V11',title:'Alkol/Uyuşturucu',risk:'high',legal:'İş K. 25/II-d',steps:['Tutanak (2 şahit)','Derhal fesih'],desc:'İşe alkollü veya uyuşturucu etkisinde gelme'},
        {id:'V12',title:'İş Reddi',risk:'medium',legal:'İş K. 25/II-h',steps:['Tutanak','Savunma','İhtar','Fesih'],desc:'Verilen görevi reddetme - "Benim işim değil" tutumu'},
        {id:'V13',title:'Ticari Sır İfşası',risk:'high',legal:'TCK 239',steps:['Delil toplama','Tutanak','Fesih + Suç duyurusu'],desc:'Reçete, maliyet, tedarikçi bilgisi paylaşımı'},
        {id:'V14',title:'Sosyal Medya İhlali',risk:'medium',legal:'İş K. 25/II-b',steps:['Ekran görüntüsü','Tutanak','Savunma'],desc:'İşveren aleyhine sosyal medya paylaşımı'},
        {id:'V15',title:'İmzadan İmtina',risk:'low',legal:'Yargıtay içtihatı',steps:['İmtina tutanağı','2 şahit imzası'],desc:'Belge imzalamaktan kaçınma'},
        {id:'V16',title:'Kılık Kıyafet',risk:'low',legal:'İş K. 25/II-h',steps:['Sözlü uyarı','Yazılı uyarı','İhtar'],desc:'Üniforma giymeme veya uygunsuz kıyafet'},
        {id:'V17',title:'Demirbaş Hasarı',risk:'medium',legal:'TBK 407',steps:['Hasar tutanağı','Savunma','Borç İkrar'],desc:'POS, tablet, ekipman hasarı'},
        {id:'V18',title:'Yetkisiz İndirim',risk:'high',legal:'İş K. 25/II-e',steps:['POS kaydı kontrolü','Tutanak','Fesih'],desc:'İzinsiz ücretsiz ürün verme veya indirim yapma'},
        {id:'V19',title:'Hırsızlık',risk:'high',legal:'İş K. 25/II-e + TCK 141',steps:['Tutanak','Kamera kayıt','Kolluk bildirimi','Derhal fesih + Suç duyurusu'],desc:'İşyerinden para, ürün veya malzeme çalma'},
        {id:'V20',title:'Zimmet',risk:'high',legal:'TCK 247',steps:['Mali denetim','Tutanak','Fesih + Suç duyurusu'],desc:'Kendisine tevdi edilen şirket malını sahiplenme'},
        {id:'V21',title:'Sahte Belge',risk:'high',legal:'TCK 204 + İş K. 25/II-a',steps:['Belge incelemesi','Tutanak','Fesih + Suç duyurusu'],desc:'Sahte rapor, diploma veya evrak sunma'},
        {id:'V22',title:'İzinsiz İşten Ayrılma',risk:'high',legal:'İş K. 25/II-g',steps:['Devamsızlık tutanağı','Noter ihtarı','Fesih'],desc:'Haber vermeden işi terk etme'},
        {id:'V23',title:'Cinsel Taciz',risk:'high',legal:'İş K. 25/II-c + TCK 105',steps:['Şikayet kaydı','İfade alma','Fesih + Suç duyurusu'],desc:'İşyerinde cinsel taciz'},
        {id:'V24',title:'Sözlü Taciz',risk:'medium',legal:'İş K. 25/II-d',steps:['Tutanak','Şahit ifadeleri','Savunma','Fesih'],desc:'Sözlü sarkıntılık, uygunsuz espri veya laf atma'},
        {id:'V25',title:'Tehdit',risk:'high',legal:'İş K. 25/II-d + TCK 106',steps:['Tutanak','Şahit ifadeleri','Fesih + Suç duyurusu'],desc:'İş arkadaşı veya yöneticiye tehdit'},
        {id:'V26',title:'Mobbing Uygulama',risk:'high',legal:'İş K. 25/II-d + TBK 417',steps:['Şikayet kaydı','Soruşturma','Savunma','Fesih'],desc:'Sistematik psikolojik baskı uygulama'},
        {id:'V27',title:'Güvenlik Kuralı İhlali',risk:'high',legal:'İş K. 25/II-ı',steps:['Tutanak','Savunma','İhtar veya Fesih'],desc:'İş güvenliği kurallarını ihlal etme'},
        {id:'V28',title:'Yangın Güvenliği İhlali',risk:'high',legal:'İş K. 25/II-ı',steps:['Tutanak','Savunma','Derhal fesih'],desc:'Yangın çıkışını engelleme, yangın söndürücü ile oynama'},
        {id:'V29',title:'İş Kazası Gizleme',risk:'high',legal:'5510 SK + İş K. 25/II',steps:['Tutanak','SGK bildirimi','Savunma','Fesih'],desc:'İş kazasını bildirmeme veya gizleme'},
        {id:'V30',title:'Müşteri Verisi Paylaşma',risk:'high',legal:'KVKK + İş K. 25/II-e',steps:['Delil toplama','Tutanak','Fesih'],desc:'Müşteri bilgilerini izinsiz paylaşma'},
        {id:'V31',title:'Rüşvet/Komisyon',risk:'high',legal:'TCK 252 + İş K. 25/II-e',steps:['Delil toplama','Tutanak','Fesih + Suç duyurusu'],desc:'Tedarikçi veya müşteriden rüşvet/komisyon alma'},
        {id:'V32',title:'Rakiple İşbirliği',risk:'high',legal:'TBK 396 + İş K. 25/II-e',steps:['Delil toplama','Tutanak','Fesih'],desc:'Rakip firma ile gizli işbirliği'},
        {id:'V33',title:'Dedikodu/Fitne',risk:'low',legal:'İş K. 25/II-d',steps:['Sözlü uyarı','Tutanak','Savunma','İhtar'],desc:'İşyeri huzurunu bozucu dedikodu yayma'},
        {id:'V34',title:'Mal Kabul Usulsüzlüğü',risk:'high',legal:'İş K. 25/II-e',steps:['Stok kontrolü','Tutanak','Fesih'],desc:'Teslim alınmayan malı kabul etmiş gibi gösterme'},
        {id:'V35',title:'Eğitime Katılmama',risk:'low',legal:'İş K. 25/II-h',steps:['Sözlü uyarı','Yazılı uyarı','İhtar'],desc:'Zorunlu eğitimlere mazeretsiz katılmama'},
        {id:'V36',title:'İzinsiz Fotoğraf/Video',risk:'medium',legal:'KVKK + İş K. 25/II-e',steps:['Tutanak','Savunma','İhtar veya Fesih'],desc:'İşyerinde izinsiz fotoğraf veya video çekme'},
        {id:'V37',title:'Mesai Saati Manipülasyonu',risk:'high',legal:'İş K. 25/II-e',steps:['PDKS kaydı','Tutanak','Fesih'],desc:'Giriş-çıkış saatlerini manipüle etme'},
        {id:'V38',title:'Sahte Mazeret',risk:'medium',legal:'İş K. 25/II-a',steps:['Belge doğrulama','Tutanak','Savunma','Fesih'],desc:'Gerçek dışı mazeret beyan etme'}
    ],
    templates: {
        barista: `<div class="doc-article"><div class="doc-article-title">MADDE 2: GÖREV TANIMI, YETKİ VE SORUMLULUKLAR</div><div class="doc-article-content">
<p><strong>2.1. Pozisyonun Niteliği:</strong> Personel, işletmede "BARİSTA VE MAĞAZA OPERASYON SORUMLUSU" unvanı ile istihdam edilmiştir. Taraflar, bu pozisyonun doğası gereği sadece içecek hazırlamayı değil; mağazanın genel düzenini, hijyenini, lojistiğini ve operasyonel akışını sağlamayı kapsayan bütünleşik bir görev olduğu konusunda mutabıktır.</p>
<p><strong>2.2. Asli Görevler (Çok Yönlü Çalışma Taahhüdü):</strong> Personel, aşağıda belirtilen görevlerin iş tanımının asli unsurları olduğunu, bu görevlerin kendisinden talep edilmesinin 4857 sayılı İş Kanunu'nun 22. maddesi kapsamında "çalışma koşullarında esaslı değişiklik" sayılmayacağını peşinen kabul ve taahhüt eder:</p>
<p>a) <strong>Üretim ve Servis:</strong> İşletmenin standart reçetelerine (gramaj, ısı, sunum) birebir uyarak kahve, soğuk/sıcak içecek ve yiyecek hazırlamak; müşteriyi karşılamak, sipariş almak, masaya servis yapmak ve boşları toplamak.</p>
<p>b) <strong>Sanitasyon ve Temizlik:</strong> Gıda güvenliği (HACCP) ve işletme standartları gereği; bar istasyonu, espresso makinesi, müşteri masaları, zeminler, depo alanları, camlar ve mağaza tuvaletlerinin (WC) hijyen planına uygun olarak periyodik temizliğini, dezenfeksiyonunu ve kontrolünü bizzat yapmak.</p>
<p>c) <strong>Lojistik ve Stok:</strong> Mal kabulü yapmak, kolileri taşımak, rafları düzenlemek, FIFO (İlk Giren İlk Çıkar) kuralına göre stok takibi ve Son Kullanma Tarihi (SKT) kontrollerini gerçekleştirmek.</p>
<p>d) <strong>Destek Hizmetleri (Kurye/Valet):</strong> İşverenin talep etmesi ve Personelin gerekli yasal ehliyet ve yetkinlik belgelerine (Sürücü Belgesi, SRC, Psikoteknik vb.) sahip olması kaydıyla; paket servis lojistiği ve otopark düzeni konularında destek vermek. Personel, bu görevleri ifa ederken Karayolları Trafik Kanunu'na ve İSG kurallarına tam uyum sağlamakla yükümlüdür.</p>
<p><strong>2.3. Nakil Yetkisi:</strong> İşveren, operasyonel gereklilikler halinde Personel'i, aynı il sınırları içerisindeki mevcut veya ileride açılacak diğer "Moonberry Coffee" şubelerinde, unvanı ve ücreti korunmak kaydıyla geçici veya sürekli olarak görevlendirme hakkına sahiptir. Personel, bu değişikliği peşinen kabul eder.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 3: ÇALIŞMA SÜRESİ, FAZLA MESAİ VE ÜCRET</div><div class="doc-article-content">
<p><strong>3.1. Çalışma Süresi:</strong> Haftalık çalışma süresi yasal sınır olan 45 saattir. İşveren, bu süreyi işyerinin açılış-kapanış saatlerine ve yoğunluğuna göre, günde 11 saati aşmamak kaydıyla haftanın çalışılan günlerine farklı şekillerde bölebilir. İşveren, İş Kanunu m. 63 uyarınca 2 (iki) aylık süreler içinde "Denkleştirme" uygulayabilir.</p>
<p><strong>3.2. Ücret ve Fazla Mesai (Yıllık 270 Saat Dahil):</strong></p>
<p>a) Personelin aylık maktu brüt ücreti {{UCRET}} TL'dir.</p>
<p>b) Belirlenen bu ücret, yasal asgari ücretin üzerinde (emsal ücret) tespit edilmiştir. Taraflar; İş Kanunu'nun 41. maddesi uyarınca yapılabilecek yıllık 270 saate kadar olan fazla çalışmaların ile ulusal bayram ve genel tatil günlerindeki çalışmaların karşılığının, kararlaştırılan bu aylık ücrete dahil olduğu hususunda mutabık kalmışlardır.</p>
<p>c) Personel, yıllık 270 saati aşmayan bu çalışmalar için aylık maaşı dışında ayrıca bir fazla mesai ücreti talep etmeyeceğini beyan eder.</p>
<p>d) Yıllık 270 saati aşan fazla çalışmaların ücreti, İş Kanunu hükümleri uyarınca ayrıca ödenir veya mevzuata uygun olarak serbest zaman şeklinde kullandırılır.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 4: KASA TAZMİNATI VE AÇIKLAR (TBK m. 407/2 Uyumlu)</div><div class="doc-article-content">
<p><strong>4.1. Kasa Tazminatı:</strong> Kasa sorumluluğu verilen Personele, üstlendiği finansal riskin ve özen borcunun karşılığı olarak, aylık ücret bordrosunda "Kasa Tazminatı" adı altında ayrı bir ek ödeme yapılır.</p>
<p><strong>4.2. Açıkların Mahsubu:</strong> Kasada oluşacak nakit veya hesaben açıklar, öncelikle ve münhasıran o ay Personele tahakkuk ettirilen Kasa Tazminatı tutarından mahsup edilir.</p>
<p><strong>4.3. Tazminatı Aşan Açıklar:</strong> Kasa açığının, ödenen kasa tazminatı tutarını aşması halinde; Personel, söz konusu açığın kendi kusurundan veya ihmalinden (hatalı işlem, eksik tahsilat, kayıp vb.) kaynaklandığını kabul ettiği takdirde, ilgili tutarın ücretinin yasal olarak haczedilebilir kısmından (1/4 oranında) kesilmesine her somut olayda ayrıca yazılı onay verecektir. Personelin rızasının olmadığı veya kasıt/ağır ihmal şüphesinin bulunduğu durumlarda İşverenin yasal yollara başvurma hakkı saklıdır.</p>
<p><strong>4.4. Bahşiş (Tip) Politikası:</strong> İşletmede müşteriler tarafından bırakılan bahşişler (tip), İşveren tarafından verilen bir ücret veya ücret eki değildir. İşveren, kredi kartı veya nakit olarak toplanan bahşişlerin personele dağıtılmasında sadece "aracı" sıfatına sahiptir. Bahşiş gelirleri, kıdem ve ihbar tazminatı hesabına dahil edilen giydirilmiş ücret kavramına dahil edilmez.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 5: EĞİTİM GİDERLERİ VE CEZAİ ŞART</div><div class="doc-article-content">
<p><strong>5.1.</strong> İşveren tarafından Personele, mesleki gelişimini sağlamak amacıyla verilecek olan sertifikalı barista eğitimleri, özel kahve teknikleri kursları ve benzeri yüksek maliyetli eğitimler karşılığında; Personel, eğitimin tamamlandığı tarihten itibaren en az 12 (on iki) ay süreyle İşveren nezdinde çalışmayı taahhüt eder.</p>
<p><strong>5.2.</strong> Personelin, bu süre dolmadan haklı bir neden olmaksızın istifa etmesi veya İşveren tarafından haklı nedenle (İş K. m. 25/II - Ahlak ve iyi niyet kurallarına aykırılık) işten çıkarılması halinde; Personel, kendisine yapılan belgelendirilmiş eğitim masraflarını (eğitmen ücreti, kurs bedeli, materyal vb.), kalan çalışma süresiyle orantılı olarak İşverene nakden ve defaten ödemeyi kabul ve taahhüt eder.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 6: YASAKLAR VE FESİH NEDENLERİ</div><div class="doc-article-content">
<p><strong>6.1. Artık Ürün (Leftover/Waste):</strong> İşyerinde satılamayan, zayi olan veya müşteriden dönen ürünlerin (waste) Personel tarafından izinsiz tüketilmesi, eve götürülmesi veya üçüncü şahıslara ikram edilmesi kesinlikle yasaktır. Bu eylem, değeri ne olursa olsun "İşverenin Güvenini Kötüye Kullanma" (İş K. m. 25/II-e) ve TCK m. 155 kapsamında suç teşkil eder; tazminatsız derhal fesih sebebidir.</p>
<p><strong>6.2. Tıbbi Gizleme:</strong> Personel, gıda güvenliğini tehdit eden bulaşıcı hastalıkları (sarılık, tüberküloz, açık yara, ishal vb.) derhal işverene bildirmek zorundadır. Bu durumun gizlenerek çalışmaya devam edilmesi, halk sağlığını tehlikeye attığı gerekçesiyle haklı fesih nedenidir.</p>
<p><strong>6.3. Kişisel Veriler ve Sır Saklama:</strong> Personel, işyeri sınırları içerisinde (mutfak, bar, depo vb.) izinsiz fotoğraf veya video çekemez. İşletmenin reçetelerini, operasyonel sırlarını veya müşterileri içeren görsellerin sosyal medyada paylaşılması "Sadakat Borcuna Aykırılık" sayılır ve haklı fesih nedenidir.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 7: İŞYERİ GÜVENLİĞİ VE KAMERA KAYDI (KVKK AYDINLATMA)</div><div class="doc-article-content">
<p><strong>7.1.</strong> İşyeri güvenliği, gıda güvenliği süreçlerinin takibi, iş sağlığı ve güvenliği ile hırsızlık gibi suçların önlenmesi amacıyla; işyerinin ortak alanlarında, üretim bölümlerinde, kasa noktalarında ve depo alanlarında 7/24 görüntü (CCTV/Video) kaydı yapılmaktadır.</p>
<p><strong>7.2.</strong> İşyerinde, yasal zorunluluklar ve istisnalar saklı kalmak kaydıyla, ses (audio) kaydı alınmamaktadır. Tuvalet, soyunma odası gibi mahremiyet içeren alanlarda kesinlikle izleme yapılmaz.</p>
<p><strong>7.3.</strong> PERSONEL, işyerinde güvenlik kameralarının bulunduğunu bildiğini, bu verilerin işlenmesine ilişkin KVKK Aydınlatma Metni'nin kendisine tebliğ edildiğini ve İşverenin meşru menfaati kapsamında görüntü kaydı alınmasına rıza gösterdiğini kabul eder.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 8: DENEME SÜRESİ VE YÜRÜRLÜK</div><div class="doc-article-content">
<p><strong>8.1.</strong> İşbu sözleşmede deneme süresi 2 (İki) aydır. Taraflar bu süre içinde iş sözleşmesini bildirim süresine gerek olmaksızın ve tazminatsız feshedebilir.</p>
<p><strong>8.2.</strong> 8 (Sekiz) maddeden ve 3 (üç) sayfadan ibaret işbu sözleşme, {{BASLAMA}} tarihinde taraflarca okunarak, içeriği tam olarak anlaşılmak suretiyle tek nüsha olarak imzalanmış ve aslı İşverende kalmak üzere, bir sureti (fotokopisi) PERSONEL'e elden teslim edilmiştir.</p>
</div></div>`,
        mudur_yardimcisi: `<div class="doc-article"><div class="doc-article-title">MADDE 2: SÖZLEŞMENİN KONUSU VE HUKUKİ STATÜ</div><div class="doc-article-content">
<p>İşbu sözleşmenin konusu, PERSONEL'in İŞVEREN'e ait "Moonberry Coffee" markalı işyerlerinde "Mağaza Müdür Yardımcısı" unvanı ile, aşağıda belirtilen görev tanımı, 4857 sayılı İş Kanunu, 6098 sayılı Türk Borçlar Kanunu ve işletme prosedürleri çerçevesinde iş görme edimini; İŞVEREN'in de buna karşılık ücret ödeme edimini yerine getirmesidir. PERSONEL, kendisine bağlı alt ekipler (barista, servis elemanı, temizlik personeli vb.) üzerinde yönetim, denetim, vardiya düzenleme ve talimat verme yetkisine sahip olup; işletmenin işleyişinden sorumlu yönetici sıfatını haizdir.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 3: GÖREV TANIMI VE "HİBRİT ÇALIŞMA" ESASI</div><div class="doc-article-content">
<p><strong>3.1. Entegre ve Çok Yönlü Görev Tanımı:</strong> PERSONEL, bu pozisyonun sadece "idari/masa başı" bir yöneticilik olmadığını; perakende ve hizmet sektörünün doğası gereği hem yönetimsel hem de operasyonel (fiili) görevleri kapsadığını kabul eder. Aşağıdaki görevlerin tamamı PERSONEL'in "Asli İşi" sayılır ve bu görevlerin talep edilmesi "çalışma koşullarında esaslı değişiklik" (İş K. m. 22) teşkil etmez:</p>
<p>a) <strong>Yönetim ve Temsil:</strong> Mağaza Müdürü'nün bulunmadığı zamanlarda işletmeyi tam yetkili olarak sevk ve idare etmek, vardiya çizelgelerini (shift plan) yasal mevzuata ve operasyonel verimliliğe uygun hazırlamak, personelin kılık-kıyafet, mesai ve hijyen denetimini yapmak.</p>
<p>b) <strong>Fiili Operasyonel Destek:</strong> İşletmenin yoğun olduğu saatlerde, personel eksikliğinde, mola değişimlerinde veya işin gerektirdiği hallerde; bizzat bar arkasına geçerek ürün hazırlamak, servis yapmak, masaları toplamak, bulaşık alanını koordine etmek ve kasaya bakmak.</p>
<p>c) <strong>Sanitasyon ve Denetim:</strong> Gıda güvenliği (HACCP) standartlarının uygulanmasını denetlemek ve bizzat sağlamak. Gerektiğinde; müşteri tuvaletleri, zemin, depo ve üretim alanlarının temizlik kontrollerini yapmak ve temizlik operasyonuna fiilen katılmak.</p>
<p>d) <strong>Stok ve Maliyet Kontrolü:</strong> Günlük, haftalık ve aylık stok sayımlarını bizzat yönetmek; zayi (waste) oranlarını raporlamak, Son Kullanma Tarihi (SKT) kontrollerini yapmak, mal kabul süreçlerini yürütmek.</p>
<p><strong>3.2. Görev Yeri ve Nakil Yetkisi:</strong> İŞVEREN, işletmesel gereklilik halinde PERSONEL'i, aynı il sınırları (Büyükşehir Belediyesi sınırları dahil) içerisindeki mevcut veya açılacak diğer şubelerinde, personelin ikametgahına makul ulaşım imkanları gözetilerek aynı veya eşdeğer bir pozisyonda görevlendirme hakkına sahiptir. PERSONEL, bu nakil yetkisini peşinen kabul ettiğini beyan eder.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 4: ÇALIŞMA SÜRELERİ, FAZLA MESAİ VE ÜCRET SİSTEMİ</div><div class="doc-article-content">
<p><strong>4.1. Çalışma Düzeni:</strong> Haftalık çalışma süresi yasal sınır olan 45 saattir. PERSONEL, yönetici sıfatıyla mağazanın açılış-kapanış, sayım ve raporlama süreçlerinden sorumlu olduğundan, mesai saatleri bu gerekliliklere göre İŞVEREN tarafından düzenlenir. İŞVEREN, İş Kanunu m. 63 uyarınca 2 (iki) aylık denkleştirme uygulayabilir.</p>
<p><strong>4.2. Yıllık 270 Saat Fazla Mesainin Ücrete Dahil Olması:</strong> PERSONEL'in aylık brüt ücreti, üstlendiği yönetim sorumluluğu ve operasyonel yük dikkate alınarak yasal asgari ücretin üzerinde (emsal ücret) tespit edilmiştir. Taraflar, yıllık 270 saate kadar (aylık ortalama 22,5 saat) yapılacak fazla çalışma ücretlerinin, kararlaştırılan aylık ücrete dahil olduğu hususunda mutabık kalmışlardır. PERSONEL, yıllık 270 saati aşmayan fazla çalışmalar için ayrıca bir ücret talep etmeyeceğini kabul eder. (Not: Ulusal Bayram ve Genel Tatil günlerinde yapılan çalışmalar ile hafta tatili çalışmaları bu 270 saatlik kapsama dahil olmayıp, çalışılması halinde ayrıca ödenir.)</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 5: ÜCRET, YAN HAKLAR VE MALİ SORUMLULUK</div><div class="doc-article-content">
<p><strong>5.1. Ücret:</strong> PERSONEL'e aylık Brüt {{UCRET}} TL ücret ödenecektir. Ücret ödemeleri banka kanalıyla yapılır. Varsa yol ve yemek yardımları şirket politikalarına göre ayni veya nakdi olarak ayrıca belirlenir.</p>
<p><strong>5.2. Kasa Tazminatı ve Mali Sorumluluk:</strong> Nakit, stok ve zimmet güvenliğini yönetmesi sebebiyle PERSONEL'e, aylık ücretine ek olarak (bordroda belirtilmek suretiyle) "Mali Sorumluluk / Kasa Tazminatı" ödenir. PERSONEL, kendi görev alanı ve denetim sorumluluğundaki kasa açıklarından ve stok kayıplarından (hırsızlık, kayıp, usulsüz ikram vb.), kusurunun tespiti halinde şahsen sorumludur. Tespit edilen ve PERSONEL'in kusuruna (kasıt, ihmal veya dikkatsizlik) dayanan zararlar; öncelikle olay bazında alınacak yazılı onayı ile kasa tazminatından ve hakedişinden mahsup edilir. Personelin onay vermemesi halinde İŞVEREN'in Borçlar Kanunu ve İş Kanunu hükümleri uyarınca yasal takip ve tazminat talep etme hakkı saklıdır.</p>
<p><strong>5.3. Bahşiş (Tip) Politikası:</strong> İşletmede müşteriler tarafından bırakılan bahşişler, ücretin bir parçası olmayıp, İŞVEREN sadece dağıtım için aracı konumundadır. PERSONEL, bahşişlerin kıdem/ihbar tazminatı hesabına dahil edilmeyeceğini kabul eder.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 6: GİZLİLİK VE REKABET YASAĞI</div><div class="doc-article-content">
<p><strong>6.1. Ticari Sırlar:</strong> PERSONEL, görevi gereği eriştiği; mağaza ciro bilgileri, karlılık raporları, tedarikçi fiyatları, özel ürün reçeteleri, personel maaş bilgileri ve müşteri datalarını "Ticari Sır" olarak saklamakla yükümlüdür. Bu bilgilerin rakip firmalarla veya yetkisiz üçüncü kişilerle paylaşılması, TCK m. 239 kapsamında suç duyurusu ve haklı nedenle derhal fesih sebebidir.</p>
<p><strong>6.2. Rekabet Yasağı (Sınırlı):</strong> PERSONEL, iş sözleşmesi herhangi bir nedenle sona erdikten sonra 1 (bir) yıl süreyle, son görev yaptığı şubenin bulunduğu ilçe sınırları içerisinde, Moonberry Coffee ile doğrudan rakip olan (benzer konseptteki kahve zincirleri) bir işletmede yönetici, ortak veya danışman olarak çalışmamayı taahhüt eder. Bu yasağın ihlali halinde, son brüt maaşının 3 (üç) katı tutarında cezai şartı İŞVEREN'e ödemeyi kabul ve taahhüt eder.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 7: YASAKLAR VE FESİH NEDENLERİ</div><div class="doc-article-content">
<p>Aşağıdaki haller, İŞVEREN için 4857 sayılı İş Kanunu m. 25/II kapsamında haklı nedenle derhal fesih sebebidir:</p>
<p>a) Kasa işlemlerinde usulsüzlük (fişsiz satış, hileli iptal/iade, zimmet).</p>
<p>b) İşletme ürünlerinin (yemek, kahve, pasta vb.) izinsiz tüketilmesi, dışarı çıkarılması veya üçüncü şahıslara bedelsiz ikram edilmesi (Güveni Kötüye Kullanma).</p>
<p>c) Astı konumundaki personele mobbing uygulamak, hakaret etmek, ayrımcılık yapmak veya cinsel tacizde bulunmak.</p>
<p>d) Gıda güvenliğini (HACCP) tehlikeye atacak hijyen ihlallerine göz yumulması veya bizzat yapılması.</p>
<p>e) İş Sağlığı ve Güvenliği (İSG) talimatlarına uymamak, kişisel koruyucu donanım kullanmamak, işyerini ve müşterileri tehlikeye düşürecek davranışlarda bulunmak.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 8: KİŞİSEL VERİLER, KAMERA KAYDI VE TEBLİGAT</div><div class="doc-article-content">
<p><strong>8.1. Kamera Kaydı:</strong> İşyeri güvenliği, hırsızlık olaylarının önlenmesi ve operasyonel standartların takibi amacıyla; işyerinin ortak alanlarında, ofis, depo ve kasa noktalarında 7/24 görüntü kaydı (CCTV) yapılmaktadır. Ses kaydı yapılmamaktadır. PERSONEL, işyerinde güvenlik kameralarının bulunduğunu bildiğini, bu kayıtların KVKK ve İŞVEREN'in meşru menfaati kapsamında işlendiğini kabul eder.</p>
<p><strong>8.2. Tebligat:</strong> Tarafların Madde 1'de belirtilen adresleri yasal tebligat adresleridir. Adres değişikliği yazılı olarak bildirilmediği sürece, bu adreslere yapılacak tebligatlar geçerli sayılacaktır.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 9: DENEME SÜRESİ VE YÜRÜRLÜK</div><div class="doc-article-content">
<p><strong>9.1.</strong> İşbu sözleşmede deneme süresi 2 (İki) aydır. Taraflar bu süre içinde iş sözleşmesini bildirim süresine gerek olmaksızın ve tazminatsız feshedebilir.</p>
<p><strong>9.2.</strong> İşbu sözleşme, 9 maddeden ibaret olup, {{BASLAMA}} tarihinde taraflarca okunarak, içeriği tam olarak anlaşılmak suretiyle tek nüsha olarak imzalanmış ve aslı İŞVEREN'de kalmak üzere bir kopyası PERSONEL'e teslim edilmiştir.</p>
</div></div>`,
        magaza_muduru: `<div class="doc-article"><div class="doc-article-title">MADDE 2: GÖREV TANIMI, YETKİ VE İŞVEREN VEKİLLİĞİ STATÜSÜ</div><div class="doc-article-content">
<p><strong>2.1. Görev Tanımı:</strong> YÖNETİCİ, ŞİRKET'e ait "Moonberry Coffee" mağazasında MAĞAZA MÜDÜRÜ unvanı ile görev yapacaktır. YÖNETİCİ; mağazanın ticari başarısından, operasyonel standartlarından, yasal mevzuata uyumundan (Gıda güvenliği, İSG vb.) ve ekibin sevk ve idaresinden tam yetkili ve sorumludur.</p>
<p><strong>2.2. İşveren Vekilliği (Hukuki Statü):</strong> Taraflar; YÖNETİCİ'nin işyerindeki konumu itibarıyla, 4857 sayılı İş Kanunu'nun 2. maddesi ve 18. maddesinin son fıkrası anlamında "İşyerinin bütününü sevk ve idare eden ve işçiyi işe alma ve işten çıkarma yetkisi bulunan İŞVEREN VEKİLİ" statüsünde olduğunu kabul ve beyan ederler. Bu statünün gereği olarak YÖNETİCİ;</p>
<p>• Mağaza personel kadrosunu belirleme,</p>
<p>• Adaylarla mülakat yapıp işe alma kararını tek başına verme,</p>
<p>• Performans düşüklüğü, disiplin suçları veya işletme gerekleri nedeniyle alt kademe personelin iş sözleşmelerini tek başına ve nihai karar verici olarak feshetme yetkisine haizdir.</p>
<p>YÖNETİCİ, bu nitelikli yetkileri haiz olması sebebiyle, 4857 sayılı Kanun'un iş güvencesi (işe iade davası) hükümlerinden yararlanamayacağını bildiğini gayrikabili rücu kabul eder.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 3: ÇALIŞMA SÜRESİ VE FAZLA MESAİ DÜZENLEMESİ</div><div class="doc-article-content">
<p><strong>3.1. Mesaiyi Belirleme Serbestisi:</strong> YÖNETİCİ, işyerinin en üst düzey amiri sıfatıyla; görev ve sorumluluklarının gerektirdiği şekilde mesai saatlerini, işe başlama-bitiş zamanlarını, ara dinlenmelerini ve izin düzenini şahsen belirleme ve tanzim etme yetkisine sahiptir. YÖNETİCİ'ye kart basma veya saatlik puantaj zorunluluğu uygulanmaz; esas olan sonuç odaklı yönetimdir.</p>
<p><strong>3.2. Fazla Çalışma Ücrete Dahildir:</strong> YÖNETİCİ'ye ödenen aylık ücret, üstlendiği idari sorumluluk, bağımsız karar alma yetkisi ve mesaisini kendisinin belirlemesi hususları dikkate alınarak, yasal asgari ücretin ve emsal ücretlerin üzerinde (nitelikli ücret) tespit edilmiştir. Bu sebeple;</p>
<p>• Yasal haftalık 45 saati aşan çalışmaların (Fazla Mesai),</p>
<p>• Hafta tatili çalışmalarının,</p>
<p>• Ulusal Bayram ve Genel Tatil günlerinde yapılan çalışmaların,</p>
<p>karşılığı olan ücretler, YÖNETİCİ'nin kararlaştırılan aylık ücretine dahildir. YÖNETİCİ, yıllık 270 saate kadar olan bu tür çalışmaları için ayrıca bir ücret, zamlı ödeme veya serbest zaman talep etmeyeceğini kabul ve taahhüt eder.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 4: ÜCRET VE YAN HAKLAR</div><div class="doc-article-content">
<p><strong>4.1. Kök Ücret:</strong> YÖNETİCİ'ye, yukarıdaki çalışma şartları karşılığında aylık Brüt {{UCRET}} TL ücret ödenecektir.</p>
<p><strong>4.2. Mali Sorumluluk (Kasa) Tazminatı:</strong> YÖNETİCİ'ye, mağazadaki nakit akışı, stok, emtia ve demirbaşların korunması riskini üstlenmesi karşılığında, aylık kök ücretine ek olarak ve bordroda ayrı bir kalem olarak gösterilmek suretiyle her ay "Mali Sorumluluk (Kasa) Tazminatı" ödenir.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 5: MALİ SORUMLULUK, ZAYİAT VE KESİNTİLER</div><div class="doc-article-content">
<p><strong>5.1. Zararın Tazmini ve Mahsup:</strong> Mağaza kasasında, stok sayımlarında veya zimmetli demirbaşlarda; hırsızlık, dikkatsizlik, prosedür hatası veya YÖNETİCİ'nin özen borcuna aykırılığı nedeniyle oluşan noksanlıklar (açıklar/zararlar), öncelikle ve münhasıran YÖNETİCİ'ye o ay ödenen "Mali Sorumluluk Tazminatı"ndan mahsup edilir.</p>
<p><strong>5.2. Tazminatı Aşan Zararlar:</strong> Oluşan zarar miktarının, ödenen Mali Sorumluluk Tazminatını aşması durumunda; aşan kısım için YÖNETİCİ'nin kusuru oranında rücu edilir. YÖNETİCİ, tazminatı aşan zararların tahsili için olayın vuku bulduğu tarihte düzenlenecek mutabakat tutanağı ile ücretinden kesinti yapılmasını veya borcu def'aten ödemeyi kabul eder.</p>
<p><strong>5.3. Kasıtlı Haller:</strong> Zimmet, emniyeti suistimal, hırsızlık veya ŞİRKET'e kasten zarar verme halleri, tazminat limitine bakılmaksızın 4857 sayılı İş Kanunu m. 25/II uyarınca haklı nedenle derhal fesih sebebidir ve oluşan tüm zarar yasal yollarla YÖNETİCİ'den tahsil edilir.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 6: PERFORMANS, REKABET YASAĞI VE GİZLİLİK</div><div class="doc-article-content">
<p><strong>6.1. Performans Hedefleri:</strong> YÖNETİCİ, kendisine tebliğ edilen ciro, kârlılık (P&L), fire/zayi oranı ve denetim puanı hedeflerini (KPI) tutturmakla yükümlüdür. Makul mazeret (mücbir sebep) olmaksızın hedeflerin süreklilik arz edecek şekilde tutturulamaması, "Performans Düşüklüğü/Yetersizlik" sayılır. ŞİRKET, bu durumda YÖNETİCİ'den yazılı savunma almak kaydıyla iş sözleşmesini geçerli nedenle feshedebilir.</p>
<p><strong>6.2. Rekabet Yasağı:</strong> YÖNETİCİ, iş sözleşmesi herhangi bir nedenle sona erdikten sonra 1 (Bir) yıl süreyle; son görev yaptığı mağazanın bulunduğu ilçe sınırları içerisinde, Moonberry Coffee ile doğrudan rakip olan (Kahve Zinciri/Kafe/Restoran) işletmelerde çalışmamayı, ortak olmamayı veya danışmanlık yapmamayı taahhüt eder.</p>
<p><strong>6.3. Cezai Şart (Karşılıklılık Esası):</strong> YÖNETİCİ'nin Rekabet Yasağını (Madde 6.2) ihlal etmesi halinde, son brüt ücretinin 6 (Altı) katı tutarında cezai şartı ŞİRKET'e ödemeyi kabul eder. ŞİRKET'in iş sözleşmesini haksız yere feshetmesi durumunda da, ŞİRKET aynı tutarda (6 brüt maaş) cezai şartı YÖNETİCİ'ye ödemeyi taahhüt eder. (TBK m. 420 ve Yargıtay içtihatları gereği karşılıklılık ilkesi).</p>
<p><strong>6.4. Gizlilik:</strong> ŞİRKET'e ait kahve reçeteleri, tedarikçi sözleşmeleri, personel ücret bilgileri, mali veriler ve müşteri veri tabanı "Ticari Sır" niteliğindedir. Bu bilgilerin üçüncü kişilerle veya rakip firmalarla paylaşılması, tazminatsız derhal fesih nedenidir ve ayrıca cezai sorumluluk (TCK m. 239) doğurur.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 7: İŞYERİ GÜVENLİĞİ VE KİŞİSEL VERİLER (KVKK)</div><div class="doc-article-content">
<p><strong>7.1. Görüntü Kaydı (CCTV):</strong> İşyeri güvenliği, iş sağlığı ve güvenliği ile operasyonel denetim amacıyla mağaza genelinde, kasa noktalarında ve çalışma alanlarında (tuvalet/giyinme kabini hariç) 7/24 Görüntü Kaydı (Video) yapılmaktadır. YÖNETİCİ, bu uygulamanın KVKK m. 5/2-f (Meşru Menfaat) kapsamında olduğunu bilir, kabul eder ve mağaza genelinde bu sistemin sürekli çalışır durumda olmasını sağlamakla yükümlüdür.</p>
<p><strong>7.2. Ses Kaydı Yasağı:</strong> Türk Ceza Kanunu'nun 133. maddesi ve KVKK ilkeleri gereği, işyerinde çalışanların ve müşterilerin özel hayatının gizliliğini ihlal edecek şekilde genel ve sürekli ses kaydı yapılmamaktadır. YÖNETİCİ, mağaza içinde izinsiz ses kaydı alan cihazların kullanılmasını engellemekle sorumludur.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 8: SON HÜKÜMLER</div><div class="doc-article-content">
<p><strong>8.1. Delil Sözleşmesi:</strong> İşbu sözleşmeden doğacak uyuşmazlıklarda ŞİRKET'in ticari defterleri, bordroları, banka kayıtları, e-posta yazışmaları ve kamera görüntüleri HMK m. 193 uyarınca kesin delil niteliğindedir.</p>
<p><strong>8.2. Yürürlük:</strong> İşbu sözleşme, 2 (iki) sayfa ve 8 (sekiz) maddeden ibaret olup, {{BASLAMA}} tarihinde taraflarca okunup, müzakere edilerek tam bir mutabakatla imzalanmış ve bir nüshası YÖNETİCİ'ye teslim edilmiştir.</p>
</div></div>`,
        bolge_muduru: `<div class="doc-article"><div class="doc-article-title">MADDE 2: GÖREV TANIMI VE "İŞVEREN VEKİLİ" STATÜSÜ</div><div class="doc-article-content">
<p><strong>2.1. Görev:</strong> PERSONEL, ŞİRKET bünyesinde "BÖLGE MÜDÜRÜ" unvanı ile istihdam edilmiştir. Sorumluluk alanı, Şirket tarafından kendisine yazılı olarak veya şirket veri sistemleri üzerinden bildirilen coğrafi bölge ve bu bölgedeki tüm şubelerdir.</p>
<p><strong>2.2. Hukuki Statü (İşveren Vekilliği):</strong> PERSONEL, kendisine bağlı bölgedeki işyerlerini (şubeleri) sevk ve idare eden, işletmenin stratejik hedeflerine yön veren ve sorumluluk bölgesindeki alt kademe çalışanları (Şube Müdürü, Barista, Servis Elemanı vb.) işe alma, işten çıkarma, terfi ettirme ve disiplin cezası verme yetkisine müstakil olarak (tek başına) sahip olan yöneticidir. Taraflar; PERSONEL'in bu yetki ve sorumlulukları gereği, 4857 sayılı İş Kanunu'nun 18. maddesinin son fıkrası ve 2. maddesi anlamında "İşletmenin Bütününü Sevk ve İdare Eden İşveren Vekili Yardımcısı" veya "İşyerinin Bütününü Sevk ve İdare Eden ve İşçi Alma/Çıkarma Yetkisi Bulunan İşveren Vekili" statüsünde olduğunu ve bu statünün yasal sonuçlarını (iş güvencesi hükümlerinin uygulanmayacağını) bildiğini kabul ve beyan ederler.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 3: GÖREV, YETKİ VE SORUMLULUKLAR</div><div class="doc-article-content">
<p>PERSONEL, sorumluluk alanındaki bölgenin kârlılığı, büyümesi ve marka standartlarının korunmasından tam sorumludur. Asli görevleri şunlardır:</p>
<p>a) <strong>Stratejik Yönetim:</strong> Bölgedeki şubelerin ciro, kârlılık (P&L) ve büyüme hedeflerini belirlemek, bütçeyi yönetmek ve Genel Müdürlük stratejilerini sahada bizzat uygulamak.</p>
<p>b) <strong>İK ve Organizasyon Yönetimi:</strong> Bölge kadrosunu oluşturmak, norm kadro çalışmaları yapmak, Şube Müdürlerini atamak, performanslarını değerlendirmek, işe alım ve fesih kararlarını nihai merci olarak vermek.</p>
<p>c) <strong>Denetim ve Standartlar:</strong> Şubelerin operasyonel standartlara (hijyen, servis kalitesi, ürün reçeteleri, Moonberry marka kimliği) uygunluğunu denetlemek, "Gizli Müşteri" skorlarını yükseltmek.</p>
<p>d) <strong>Yasal Temsil:</strong> Bölgesindeki resmi kurumlar, belediyeler ve tedarikçiler nezdinde ŞİRKET'i temsil etmek, gerekli ruhsat ve izin süreçlerini yönetmek.</p>
<p>e) <strong>Kriz Yönetimi:</strong> Bölgesinde oluşabilecek operasyonel krizleri (gıda güvenliği, iş kazası, sosyal medya krizi vb.) yönetmek.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 4: ÇALIŞMA SÜRELERİ VE FAZLA ÇALIŞMA DÜZENİ</div><div class="doc-article-content">
<p><strong>4.1. Mesai Düzeni (Time Sovereignty):</strong> PERSONEL, "Üst Düzey Yönetici" konumunda olup; görev ve sorumluluklarının gerektirdiği şekilde mesai saatlerini, şube ziyaret planlarını ve çalışma düzenini işin gerekliliklerine göre bizzat kendisi belirleme yetkisine sahiptir. İŞVEREN, PERSONEL'e günlük kart basma/imza atma zorunluluğu uygulamaz; esas olan hedeflerin gerçekleşmesidir.</p>
<p><strong>4.2. Ücretin Kapsamı:</strong> PERSONEL'in aylık brüt ücreti, üstlendiği yönetim sorumluluğu, yetkileri ve mesaisini kendisinin belirlemesi hususları dikkate alınarak, asgari ücretten ve emsal çalışanlardan belirgin şekilde yüksek tespit edilmiştir. Bu çerçevede; yasal sınır olan yıllık 270 saate kadar yapılacak fazla çalışmalar, hafta tatili çalışmaları ve ulusal bayram/genel tatil günü çalışmaları kararlaştırılan aylık ücrete dahildir. PERSONEL, bu çalışmalar için ayrıca ücret talep etmeyeceğini kabul eder.</p>
<p><strong>4.3. Denkleştirme ve Serbest Zaman:</strong> Yıllık 270 saati aşan bir fazla çalışma yapılması ve bunun yazılı delillerle ispatlanması durumunda; aşan kısmın karşılığı nakdi ücret olarak değil, 4857 sayılı İş Kanunu'nun 41. maddesi uyarınca zamlı oran üzerinden hesaplanacak "Serbest Zaman" (İzin) olarak kullandırılacaktır.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 5: ÜCRET VE YAN HAKLAR</div><div class="doc-article-content">
<p><strong>5.1. Ücret:</strong> PERSONEL'e aylık Brüt {{UCRET}} TL ücret ödenecektir. Ücret, her ayın sonunda PERSONEL'in banka hesabına yatırılır.</p>
<p><strong>5.2. Performans Primi:</strong> İŞVEREN, tek taraflı inisiyatifi ile belirleyeceği yıllık hedeflerin (KPI) tutturulması halinde PERSONEL'e prim ödemesi yapabilir. Prim uygulaması, "İşyeri Şartı" niteliğinde olmayıp, İŞVEREN tarafından her dönem değiştirilebilir veya tamamen kaldırılabilir.</p>
<p><strong>5.3. Araç ve Ekipman:</strong> Görev gereği tahsis edilen şirket aracı, bilgisayar ve telefon, sadece iş amaçlı kullanım içindir. İş sözleşmesinin sona ermesi veya İŞVEREN'in talebi halinde bu ekipmanlar derhal ve eksiksiz iade edilecektir.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 6: ŞİRKET ARACI KULLANIMI VE TRAFİK CEZALARI</div><div class="doc-article-content">
<p><strong>6.1. Sorumluluk:</strong> PERSONEL, kendisine tahsis edilen şirket aracını Karayolları Trafik Kanunu'na ve iş güvenliği kurallarına uygun kullanmakla yükümlüdür.</p>
<p><strong>6.2. Cezaların Rücu Edilmesi:</strong> PERSONEL'in kullanımında iken (ister mesai içinde ister dışında) araca kesilen trafik cezalarından (hız, park, kırmızı ışık vb.) PERSONEL şahsen sorumludur. ŞİRKET, yasal zorunluluk gereği cezayı ödemek zorunda kalırsa; PERSONEL, ödenen tutarı ilk yazılı bildirimde ŞİRKET'e nakden ödemeyi veya ilgili tutarın ücretinden mahsup edilmesi için her olay özelinde ayrıca muvafakatname vermeyi kabul ve taahhüt eder.</p>
<p><strong>6.3. Disiplin:</strong> PERSONEL'in kusurlu davranışı sonucu oluşan trafik cezalarını ödemekten imtina etmesi veya rücu işlemine onay vermemesi, işverene zarar verme kastı taşıdığından, İş Kanunu m. 25/II kapsamında haklı fesih nedeni sayılabilecektir.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 7: REKABET YASAĞI, GİZLİLİK VE CEZAİ ŞART</div><div class="doc-article-content">
<p><strong>7.1. Gizlilik ve Ticari Sır:</strong> PERSONEL; ŞİRKET'in maliyet yapıları, franchise koşulları, tedarikçi anlaşmaları, müşteri veritabanı, özel tarifeleri ve büyüme stratejileri gibi "Ticari Sır" niteliğindeki bilgilere vakıftır. Bu bilgilerin sözleşme süresince veya sona erdikten sonra üçüncü kişilerle paylaşılması, TCK m. 239 kapsamında suç teşkil eder ve tazminatsız derhal fesih nedenidir.</p>
<p><strong>7.2. Rekabet Yasağı (TBK 444-447):</strong> PERSONEL, iş sözleşmesi herhangi bir nedenle (işverenin haklı feshi veya personelin istifası/haksız feshi ile) sona erdikten sonra 2 (iki) yıl süreyle; son görev yaptığı il sınırları içerisinde, "Kahve Zinciri, Kafe-Restoran ve Gıda Perakendeciliği" sektöründe faaliyet gösteren rakip bir firmada çalışmamayı, ortak olmamayı veya danışmanlık yapmamayı taahhüt eder.</p>
<p><strong>7.3. Cezai Şart:</strong> PERSONEL'in işbu 7. maddede düzenlenen rekabet yasağını ihlal etmesi halinde; ŞİRKET'in uğradığı zararın miktarını ispat etmesine gerek kalmaksızın, PERSONEL, son brüt maaşının 6 (Altı) katı tutarında cezai şartı İŞVEREN'e derhal ve nakden ödemeyi kabul ve taahhüt eder. İŞVEREN'in bu miktarı aşan zararlarını talep hakkı ve TBK m. 446/2 uyarınca rekabet edici davranışa son verilmesini talep hakkı saklıdır.</p>
<p><strong>7.4. Ayartma Yasağı (Non-Solicitation):</strong> PERSONEL, işten ayrıldıktan sonra 2 yıl süreyle, ŞİRKET çalışanlarını yeni işyerine transfer etmeyeceğini, onlara iş teklifinde bulunmayacağını taahhüt eder.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 8: FİKRİ MÜLKİYET HAKLARININ DEVRİ</div><div class="doc-article-content">
<p>PERSONEL, işbu sözleşme kapsamında görevi sırasında veya görevi nedeniyle meydana getirdiği/getireceği her türlü fikir, sanat eseri, rapor, operasyonel süreç tasarımı, eğitim materyali, bilgisayar programı, veri tabanı ve iş geliştirme projeleri üzerindeki 5846 sayılı FSEK kapsamındaki tüm mali hakları (işleme, çoğaltma, yayma, temsil, umuma iletim) yer, süre ve sayı sınırı olmaksızın, işbu sözleşmedeki ücret karşılığında münhasıran ŞİRKET'e devrettiğini; bu hakların kullanımı ve devri konusunda ŞİRKET'in tam yetkili olduğunu kabul ve beyan eder.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 9: ÜCRETLİ UZAKLAŞTIRMA (GARDEN LEAVE)</div><div class="doc-article-content">
<p>İş sözleşmesinin herhangi bir nedenle feshi ihbarında bulunulması halinde (taraflardan hangisinin ihbar ettiğine bakılmaksızın); ŞİRKET, ihbar süresinin tamamında veya bir kısmında PERSONEL'i işyerine gelmekten, iş görme borcunu ifa etmekten ve şirket sistemlerine erişmekten muaf tutma (Garden Leave) hakkına sahiptir. Bu süre zarfında PERSONEL'in ücret ve yan hakları (prim hariç) ödenmeye devam eder. PERSONEL, bu süre içinde başka bir işverene hizmet veremez, rekabet edemez ve sadakat borcuna aykırı davranamaz.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 10: KİŞİSEL VERİLERİN KORUNMASI (KVKK) VE İZLEME</div><div class="doc-article-content">
<p><strong>10.1.</strong> ŞİRKET; iş sağlığı güvenliği, operasyonel verimlilik, şirket varlıklarının korunması ve yasal yükümlülükler (işin ifası ve meşru menfaat) sebepleriyle; işyerlerinde, ofislerde kamera ile görüntü kaydı ve şirket araçlarında GPS (Konum) takip sistemi kullanmaktadır.</p>
<p><strong>10.2.</strong> PERSONEL, bu veri işleme faaliyetleri, kameraların bulunduğu yerler ve araç takip sisteminin çalışma prensipleri hakkında kendisine ayrıca tebliğ edilen "Çalışan Aydınlatma Metni" ile 6698 sayılı KVKK kapsamında detaylıca bilgilendirildiğini, mesai saatleri dışında şirket aracının kullanımı durumunda da (özel kullanım izni varsa) GPS kaydının devam edebileceğini bildiğini beyan eder.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 11: MOBBİNG VE ETİK HAT</div><div class="doc-article-content">
<p>PERSONEL, işyerinde karşılaşabileceği her türlü etik dışı davranış, ayrımcılık veya psikolojik taciz (mobbing) iddiasını, yasal yollara başvurmadan önce ivedilikle Şirket Yönetim Kurulu'na veya varsa "Etik Hattı"na yazılı olarak bildirmeyi kabul eder. ŞİRKET, bu başvuruları gizlilik içinde inceleyip sonuçlandıracağını taahhüt eder.</p>
</div></div>

<div class="doc-article"><div class="doc-article-title">MADDE 12: SON HÜKÜMLER VE YÜRÜRLÜK</div><div class="doc-article-content">
<p><strong>12.1.</strong> İşbu sözleşmede hüküm bulunmayan hallerde 4857 sayılı İş Kanunu, 6098 sayılı TBK ve ilgili mevzuat hükümleri uygulanır.</p>
<p><strong>12.2.</strong> Uyuşmazlık halinde İstanbul (Çağlayan) Mahkemeleri ve İcra Daireleri yetkilidir.</p>
<p><strong>12.3.</strong> İki sayfadan ve 12 maddeden ibaret işbu sözleşme, {{BASLAMA}} tarihinde taraflarca okunarak, içeriği müzakere edilip tam olarak anlaşılarak serbest iradeleriyle imzalanmış ve bir nüshası PERSONEL'e teslim edilmiştir.</p>
</div></div>`
    },
    fesihDayanaklari: {
        hakli: [
            {v:'25_2_a',t:'25/II-a: Yanıltma'},
            {v:'25_2_b',t:'25/II-b: Hakaret'},
            {v:'25_2_d',t:'25/II-d: Sataşma/Sarhoşluk'},
            {v:'25_2_e',t:'25/II-e: Güveni kötüye kullanma'},
            {v:'25_2_g',t:'25/II-g: Devamsızlık'},
            {v:'25_2_h',t:'25/II-h: Görev reddi'},
            {v:'25_2_i',t:'25/II-ı: İş güvenliği'}
        ],
        gecerli: [
            {v:'18_y',t:'Yetersizlik'},
            {v:'18_d',t:'Davranış'},
            {v:'18_i',t:'İşletme gerekleri'}
        ],
        deneme: [{v:'deneme',t:'Deneme değerlendirmesi'}]
    },
    documentTemplates: {
        tutanak: {
            olay: '<p><strong>OLAY:</strong></p><p>{{TARIH}} tarihinde saat {{SAAT}} sıralarında, {{SUBE}} şubesinde görevli {{PERSONEL}} ({{TC}}) aşağıda açıklanan olayı gerçekleştirmiştir:</p><p style="background:#f9f9f9;padding:15px;border-left:4px solid #ff8674;margin:15px 0">{{ACIKLAMA}}</p><p>İşbu tutanak, olayın tespiti amacıyla düzenlenmiş olup, ilgili personele tebliğ edilmiştir.</p>',
            devamsizlik: '<p><strong>DEVAMSIZLIK TESPİTİ:</strong></p><p>{{PERSONEL}} ({{TC}}), {{TARIH}} tarihinde {{SUBE}} şubesinde göreve gelmemiştir.</p><p>Yapılan araştırmada herhangi bir mazeret bildirimi tespit edilememiştir.</p><p style="background:#fef9e7;padding:12px;border-radius:6px;margin:15px 0">⚠️ İş Kanunu Md. 25/II-g: Ardı ardına 2 iş günü veya ay içinde 3 iş günü devamsızlık haklı fesih sebebidir.</p>',
            kasa: '<p><strong>KASA SAYIM TESPİTİ:</strong></p><p>{{TARIH}} tarihinde saat {{SAAT}} itibarıyla {{SUBE}} şubesinde yapılan kasa sayımında aşağıdaki tespit yapılmıştır:</p><p>Sorumlu personel: {{PERSONEL}} ({{TC}})</p><p style="background:#f9f9f9;padding:15px;border-left:4px solid #ff8674;margin:15px 0">{{ACIKLAMA}}</p><p>TBK Md. 407/2 uyarınca personelin yazılı muvafakati ile ücretinden kesinti yapılabilir.</p>',
            imza: '<p><strong>İMZADAN İMTİNA TESPİTİ:</strong></p><p>{{TARIH}} tarihinde {{PERSONEL}} ({{TC}}) adlı personele tebliğ edilmek istenen belgeyi imzalamaktan imtina etmiştir.</p><p>İmtina edilen belge: {{ACIKLAMA}}</p><p style="background:#ebf5fb;padding:12px;border-radius:6px;margin:15px 0">📌 Yargıtay içtihadı gereği, 2 şahit huzurunda düzenlenen imtina tutanağı tebliğ yerine geçer.</p>'
        },
        savunma: '<p>Şirketimiz {{SUBE}} şubesinde {{POZISYON}} olarak görev yapmaktasınız.</p><p><strong>{{TARIH}}</strong> tarihinde gerçekleştirdiğiniz aşağıdaki davranış/eylem nedeniyle savunmanız istenmektedir:</p><p style="background:#f9f9f9;padding:15px;border-left:4px solid #ff8674;margin:15px 0">{{ACIKLAMA}}</p><p>Bu davranış <strong>{{DAYANAK}}</strong> kapsamında değerlendirilmektedir.</p><p style="background:#fef9e7;padding:12px;border-radius:6px;margin:15px 0">⚠️ Savunmanızı bu yazının tebliğinden itibaren <strong>{{SURE}}</strong> içinde yazılı olarak vermeniz gerekmektedir. Süresinde savunma verilmemesi, savunma hakkından vazgeçilmiş sayılacaktır.</p>',
        fesih: {
            hakli: '<p>{{TARIH}} tarihinde gerçekleştirdiğiniz aşağıdaki eylem/davranış nedeniyle iş sözleşmeniz <strong>4857 sayılı İş Kanunu\'nun {{DAYANAK}} maddesi</strong> uyarınca haklı nedenle ve derhal feshedilmiştir:</p><p style="background:#f9f9f9;padding:15px;border-left:4px solid #c0392b;margin:15px 0">{{ACIKLAMA}}</p><p>Bu fesih işlemi sonucunda kıdem ve ihbar tazminatı ödenmeyecektir.</p>',
            gecerli: '<p>Aşağıda belirtilen nedenlerle iş sözleşmeniz <strong>4857 sayılı İş Kanunu\'nun 17. ve 18. maddeleri</strong> uyarınca geçerli nedenle feshedilmiştir:</p><p style="background:#f9f9f9;padding:15px;border-left:4px solid #ff8674;margin:15px 0">{{ACIKLAMA}}</p><p>İhbar süreniz: {{IHBAR_SURE}}<br>Son çalışma gününüz: {{SON_GUN}}</p><p>Kıdem tazminatı ve diğer yasal haklarınız ödenecektir.</p>',
            deneme: '<p>{{BASLAMA}} tarihinde başlayan iş sözleşmeniz, 2 aylık deneme süresi içinde <strong>4857 sayılı İş Kanunu\'nun 15. maddesi</strong> uyarınca sona erdirilmiştir.</p><p style="background:#f9f9f9;padding:15px;border-left:4px solid #3498db;margin:15px 0">{{ACIKLAMA}}</p><p>Deneme süresi içinde fesih bildirim süresine ve tazminata tabi değildir.</p>'
        },
        borc: '<p>Ben {{PERSONEL}}, {{TARIH}} tarihli olayda kusurum nedeniyle <strong>{{TUTAR}} TL</strong> borcu kabul ediyorum.</p><p style="background:#f9f9f9;padding:15px;border-left:4px solid #ff8674;margin:15px 0">{{ACIKLAMA}}</p><p>TBK Md. 407/2 uyarınca bu borcun {{ODEME_SEKLI}} kabul ve taahhüt ediyorum.</p><p>Baskı altında olmadığımı, borcumu ve kesinti koşullarını anladığımı beyan ederim.</p>'
    }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof firebase !== 'undefined') {
        ...
        // Auth State Listener - Kullanıcı oturum durumunu takip et
        firebase.auth().onAuthStateChanged(async (user) => {
            if (user) {
                ...
                try {
                    // Önce config'i yükle (email mapping için)
                    await loadSystemConfig();
                    
                    // 1. ÖNCE PERSONEL koleksiyonundan email ile bul
                    ...
                    if (!personelSnapshot.empty) {
                        ...
                        if (personelData.role) {
                            ...
                            console.log('✅ Personel koleksiyonundan kullanıcı bulundu:', currentUser.name, currentUser.role);
                        }
                    }
                    
                    // 2. Personel'de bulunamadıysa users koleksiyonuna bak
                    if (!foundInPersonel) {
                        const userDoc = await db.collection('users').doc(user.uid).get();
                        if (userDoc.exists) {
                            currentUser = { id: user.uid, email: user.email, ...userDoc.data() };
                            console.log('✅ Users koleksiyonundan kullanıcı bulundu');
                        } else {
                            // 3. Email mapping'den bak
                            ...
                            currentUser = {
                                id: user.uid,
                                email: user.email,
                                name: emailMatch?.name || (isAdmin ? 'Yönetici' : user.email.split('@')[0]),
                                role: emailMatch?.role || (isAdmin ? 'yonetici' : 'barista'),
                                branch: emailMatch?.branch || (isAdmin ? 'all' : '')
                            };
                            console.log('✅ Email mapping veya varsayılan kullanıcı:', currentUser.name, currentUser.role);
                        }
                    }
                    ...
                    // Config yükle ve Dashboard verilerini yükle
                    loadSystemConfig().then(async () => {
                        console.log('Config yüklendi, personel cache yükleniyor...');
                        await loadPersonelDisplayCache();
                        console.log('Cache yüklendi, dashboard yükleniyor...');
                        loadTodayChecks();
                    });
                    
                    // Önizleme modunu kontrol et
                    checkPreviewMode();
                ...
                // Kullanıcı çıkış yapmış - Login sayfasına yönlendir
...
async function logout() {
    try {
        // Çıkış aktivitesini logla (currentUser silinmeden önce)
        ...
        if (currentUser?.sessionId) {
            try {
                ...
                console.error('Oturum sonlandırma hatası:', e);
            }
        }
...
function initUI() {
    ...
    // Belge şablonları için varsayılanı yükle
...
function setDefaultDates() {
    ...
    // Tutanak varsayılan tarih/saatleri
...
// Şube select populate - kullanıcı yetkisine göre filtrele
function populateSubeSelect(selectId) {
    ...
    // Mağaza müdürü sadece kendi şubesini görsün
    ...
    // Mağaza müdürü ise kendi şubesini seç
    if (!isAdmin && userBranch) {
        sel.value = userBranch;
    } else {
        // Admin/Bölge Müdürü için default: Tuzla Port
...
function renderPositionCards() {
    ...
    container.innerHTML = STATE.positions.map((p, i) => `
        <div class="type-card${i === 0 ? ' selected' : ''}" data-type="${p.id}" onclick="selectType('sozlesme','${p.id}',this)">
            <div class="type-card-icon">${p.type === 'Mavi Yaka' ? '☕' : p.type === 'Gri Yaka' ? '📋' : p.type === 'Beyaz Yaka' ? '🏪' : '🌍'}</div>
...
// ==================== VARSAYILAN VERİLERİ KONTROL ET ====================
function initDefaultData() {
    // Varsayılan verileri kontrol et ve oluştur
    initializeDefaultData();
}

async function initializeDefaultData() {
    try {
        // Varsayılan puantaj kurallarını kontrol et
        ...
        if (rulesSnapshot.empty) {
            ...
            console.log('Varsayılan puantaj kuralları oluşturuldu');
        }
        
        // Varsayılan gelişmiş izinleri kontrol et
        ...
        if (advPermsSnapshot.empty) {
            ...
            console.log('Varsayılan gelişmiş izinler oluşturuldu');
        }
    } catch (error) {
...
async function handleLogin(event) {
    // Login artık login.html'de yapılıyor
    if (event) event.preventDefault();
    window.location.replace('login.html');
}

// Şifremi unuttum - login.html'e yönlendir
...
function applyUserPermissions() {
    ...
    // Sidebar'da kullanıcı bilgilerini güncelle
    ...
    if (userRoleEl) {
        const roleNames = {
            ...
            'magaza_mudur_yardimcisi': 'Mağaza Müdür Yardımcısı',
            'barista': 'Barista'
        };
...
function applyBaristaRestrictions() {
    ...
    // Belgeler ve Formlar menülerini tamamen gizle
    ...
    // Nav gruplarını kontrol et - boş olanları gizle
    ...
    // Dashboard section'ları kontrol et - Belgeler bölümünü tamamen gizle
    ...
    // Personel ve Ayarlar gizle
    const navPersonel = document.querySelector('.nav-item[data-page="personel"]');
    const navAdmin = document.querySelector('.nav-item[data-page="admin"]');
    const navKatalog = document.querySelector('.nav-item[data-page="katalog"]');
    if (navPersonel) navPersonel.style.display = 'none';
    if (navAdmin) navAdmin.style.display = 'none';
    if (navKatalog) navKatalog.style.display = 'none';
    ...
    // Dashboard'da FORMLAR bölümünü gizle (ID ile)
    const formlarSection = document.getElementById('dashboard-formlar-section');
    if (formlarSection) formlarSection.style.display = 'none';
    
    console.log('✅ Barista kısıtlamaları uygulandı');
}
...
async function loadFirebaseData() {
    ...
    // Puantaj şube seçeneklerini doldur - kullanıcı yetkisine göre filtrele
...
function goTo(page) {
    // page-xxx formatında geldiyse düzelt
    ...
    // Tüm belgeler ve formlar için şube seçimi (Tuzla Port default)
    ...
    // Dashboard için bugünkü checkler
    if (page === 'dashboard') {
        loadTodayChecks();
    }
}
...
async function executeConfirmCallback() {
    ...
    if (_confirmCallback) {
        try {
            ...
            console.error('Confirm callback hatası:', error);
            toast('İşlem hatası: ' + error.message, 'error');
        }
...
function selectFesihType(type, el) {
    ...
    sel.innerHTML = '<option value="">Seçiniz</option>' + opts.map(d => `<option value="${d.v}">${d.t}</option>`).join('');
}
...
// Benzersiz Belge Numaralandırma: YIL-ŞUBEKODU-GGAASS
function docNo(prefix, sube) {
    ...
    if (sube && STATE.branchCodes) {
        ...
        // Fallback: İlk 3 harf
        subeKod = sube.replace(/[^A-Za-zÇĞİÖŞÜçğıöşü]/g, '').substring(0, 3).toUpperCase();
        if (subeKod === 'ŞİŞ' || subeKod === 'SIS') subeKod = 'SIS';
...
function downloadPDF() {
    ...
    // Tüm doc-article elementlerine page-break-inside: avoid ekle
    ...
    // Tüm doc-article-title elementlerine page-break-after: avoid ekle
    ...
    html2pdf().set({
        margin: [15, 15, 20, 15], // üst, sağ, alt, sol
        filename: title.replace(/[^a-zA-Z0-9ğüşıöçĞÜŞİÖÇ\s-]/g, '') + '.pdf',
...
function generateSozlesme() {
    ...
    // Pozisyona göre el yazısı beyan metinleri
    const beyanlar = {
        barista: '"İşbu iş sözleşmesini, eklerini ve KVKK Aydınlatma Metnini elden teslim aldım, okudum, içeriğini ve yükümlülüklerimi tam olarak anladım ve hür irademle kabul ettim."',
        mudur_yardimcisi: '"İşbu iş sözleşmesini, eklerini ve KVKK Aydınlatma Metnini elden teslim aldım, okudum. İşverenin işleyişini, görev tanımımı, ücret ve fazla mesai uygulamalarını, gizlilik ve rekabet yasağı hükümlerini tam olarak anladım ve hür irademle kabul ettim."',
        magaza_muduru: '"Sözleşmeyi okudum, bir suretini teslim aldım. İşveren Vekili statüsünü, fazla mesai düzenlemesini ve rekabet yasağı hükümlerini anladım, hür irademle kabul ediyorum."',
        bolge_muduru: '"İşbu sözleşmeyi okudum, içeriğini, İşveren Vekili statümün sonuçlarını ve rekabet yasağı yükümlülüklerimi anladım, bir suretini elden teslim aldım."'
    };
    const elYazisiMetni = beyanlar[type] || '"İşbu iş sözleşmesini okudum, anladım, bir suretini teslim aldım."';
    ...
    const html = `<div class="doc">
        ${docHeader('BELİRSİZ SÜRELİ İŞ SÖZLEŞMESİ')}
        ...
        <div class="doc-section" style="margin-bottom:15px;border-bottom:1px solid #ccc;padding-bottom:8px">
            <table style="width:100%;font-size:10pt">
                <tr><td><strong>Sözleşme Tarihi:</strong> ${baslama}</td><td style="text-align:right"><strong>Sözleşme No:</strong> ${docNo('SOZ', sube)}</td></tr>
            </table>
        </div>
        
        <div class="doc-article"><div class="doc-article-title">MADDE 1: TARAFLAR</div>
        <div class="doc-article-content" style="font-size:9pt">
            <p style="font-weight:bold;margin:8px 0 5px 0">1.1. İŞVEREN:</p>
            <p style="margin:2px 0"><strong>Ünvanı:</strong> ${STATE.company.name}</p>
            ...
            <p style="font-size:8pt;color:#666;margin:5px 0">(Bundan sonra "İŞVEREN" olarak anılacaktır.)</p>
            
            <p style="font-weight:bold;margin:12px 0 5px 0">1.2. PERSONEL (İŞÇİ):</p>
            <p style="margin:2px 0"><strong>Adı Soyadı:</strong> ${ad || '______________________'}</p>
            ...
            <p style="font-size:8pt;color:#666;margin:5px 0">(Bundan sonra "PERSONEL" olarak anılacaktır.)</p>
        </div></div>
            ...
            <table style="width:100%;border:none;">
                <tr>
                    <td style="width:50%;text-align:center;vertical-align:bottom;padding:15px;border:none;">
                        <p style="margin-bottom:50px">(Kaşe / İmza)</p>
                        <div style="border-top:1px solid #000;width:70%;margin:0 auto;padding-top:8px">
                            <p style="font-weight:bold;margin:0">İŞVEREN</p>
                        </div>
                    </td>
                    <td style="width:50%;text-align:center;vertical-align:bottom;padding:15px;border:none;">
                        <p style="margin-bottom:20px">(İmza)</p>
                        <div style="border-top:1px solid #000;width:70%;margin:0 auto;padding-top:8px">
                            <p style="font-weight:bold;margin:0">PERSONEL</p>
                            <p style="font-size:9pt;margin:3px 0 0 0">(Adı Soyadı)</p>
                        </div>
                    </td>
                </tr>
            </table>
        ...
        <div style="margin-top:25px;padding:15px;border:1px solid #000;page-break-inside:avoid;">
            <p style="font-size:9pt;margin-bottom:8px;font-style:italic">(Aşağıdaki alan Personel tarafından kendi el yazısı ile doldurulacaktır)</p>
            <p style="font-weight:bold;margin-bottom:8px;font-size:10pt">EL YAZISI İLE BEYAN:</p>
            ...
            <table style="width:100%;margin-top:10px;border:none;font-size:10pt">
                <tr>
                    ...
                    <td style="border:none;text-align:right"><strong>İmza:</strong> ________________</td>
                </tr>
            </table>
        </div>
    </div>`;
    
    showPreview(posName + ' İş Sözleşmesi - ' + ad, html);
}

// ==================== TUTANAK ====================
function generateTutanak() {
    const type = selectedTypes.tutanak || 'olay';
    const titles = {olay: 'OLAY TESPİT TUTANAĞI', devamsizlik: 'DEVAMSIZLIK TUTANAĞI', kasa: 'KASA AÇIĞI TUTANAĞI', imza: 'İMZADAN İMTİNA TUTANAĞI'};
    ...
    const html = `<div class="doc">
        ...
        <div class="doc-section">
            <p><strong>Tutanak No:</strong> ${docNo('TUT', sube)} | <strong>Düzenleme:</strong> ${duzTarih} ${duzSaat} | <strong>Şube:</strong> ${sube}</p>
            <p><strong>Personel:</strong> ${personel} (${pozisyon})</p>
            <p><strong>Olay Tarihi:</strong> ${tarih} ${saat}${type === 'kasa' && kasa ? ' | <strong>Açık:</strong> ' + kasa + ' TL' : ''}</p>
        </div>
        <div class="doc-section">
            <div class="doc-section-title">OLAY AÇIKLAMASI</div>
            <p style="background:#f9f9f9;padding:15px;border-left:4px solid #ff8674">${aciklama}</p>
        </div>
        <p style="margin:20px 0">İşbu tutanak şahitler huzurunda düzenlenmiştir.</p>
        <div class="doc-section">
            <div class="doc-section-title">ŞAHİTLER</div>
            <p>1. ${sahit1} _________________ | 2. ${sahit2} _________________</p>
        </div>
        ${docSignature(
            {title: 'DÜZENLEYEN', name: duzenleyen, role: gorev},
            {title: 'PERSONEL', name: personel, role: '(İmza / İmtina)'}
        )}
        <p style="margin-top:20px;font-size:9pt;color:#666"><strong>NOT:</strong> İmzadan imtina halinde şahitler tarafından not edilir.</p>
    </div>`;
...
function generateSavunma() {
    ...
    const yetkili = getVal('sav_yetkili') || 'İnsan Kaynakları';
    
    const html = `<div class="doc">
        ${docHeader('SAVUNMA İSTEME BELGESİ')}
        <div class="doc-section">
            <p><strong>Tarih:</strong> ${duzTarih} ${duzSaat} | <strong>Belge No:</strong> ${docNo('SAV', sube)}</p>
            <p><strong>Sayın:</strong> ${personel} (${pozisyon}) - ${sube}</p>
        </div>
        <div class="doc-section">
            <p>4857 sayılı İş Kanunu'nun 19. maddesi gereğince savunma hakkınız tanınmaktadır.</p>
            <p style="margin-top:15px"><strong>İsnat Edilen Eylem:</strong></p>
            ...
            <p style="margin-top:15px"><strong>Savunma Süresi:</strong> Tebliğden itibaren <strong>${sure} iş günü</strong></p>
            <p style="margin-top:15px;background:#fef9e7;padding:12px;border-radius:6px">⚠️ Süre içinde savunma verilmezse feragat edilmiş sayılır.</p>
        </div>
        <div class="doc-section" style="margin-top:20px;font-size:10pt;color:#666">
            <strong>Yasal Dayanak:</strong> İş K. Md. 19: "Savunması alınmadan işçinin sözleşmesi feshedilemez."
        </div>
        ${docSignature(
            {title: 'TEBLİĞ EDEN', name: yetkili, role: 'İmza'},
            {title: 'PERSONEL', name: personel, role: 'Tebliğ Tarihi: ___/___/___'}
        )}
    </div>`;
    
    showPreview('Savunma İsteme - ' + personel, html);
}

// ==================== FESİH ====================
function generateFesih() {
    const type = selectedTypes.fesih || 'hakli';
    const titles = {hakli: 'HAKLI NEDENLE FESİH BİLDİRİMİ', gecerli: 'GEÇERLİ NEDENLE FESİH BİLDİRİMİ', deneme: 'DENEME SÜRESİ FESHİ'};
    const infos = {
        hakli: 'İş Kanunu 25/II uyarınca sözleşmeniz derhal ve tazminatsız feshedilmiştir.',
        gecerli: 'İş Kanunu 18. madde uyarınca geçerli nedenle feshedilmiştir. Tazminat hakları saklıdır.',
        deneme: 'Deneme süresi içinde bildirimsiz ve tazminatsız feshedilmiştir.'
    };
    ...
    const html = `<div class="doc">
        ${docHeader(titles[type])}
        <div class="doc-section">
            <p><strong>Tarih:</strong> ${fesTarih} | <strong>Belge No:</strong> ${docNo('FES', sube)} | <strong>Şube:</strong> ${sube}</p>
            <p><strong>Sayın:</strong> ${personel} (${pozisyon})</p>
            <p><strong>İşe Giriş Tarihi:</strong> ${giris}</p>
        </div>
        <div class="doc-section">
            <div class="doc-section-title">FESİH GEREKÇESİ</div>
            <p style="background:#fdedec;padding:15px;border-left:4px solid #e74c3c">${gerekce}</p>
        </div>
        <div class="doc-section">
            <p>${infos[type]}</p>
        </div>
        ${type === 'hakli' ? '<p style="background:#fef9e7;padding:12px;border-radius:6px;margin:20px 0">⚠️ İş Kanunu Md. 26 uyarınca 6 iş günü içinde kullanılmıştır.</p>' : ''}
        ${docSignature(
            {title: 'İŞVEREN', name: '(KAŞE)', role: 'İmza'},
            {title: 'PERSONEL', name: personel, role: 'Tebliğ: ___/___/___'}
        )}
    </div>`;
...
function generateIstifa() {
    ...
    const neden = getVal('istifa_neden') || 'Kişisel nedenlerden dolayı';
    
    const ihbarText = {
        '0': 'derhal geçerli olmak üzere',
        '2': '2 haftalık ihbar süresine uyarak',
        '4': '4 haftalık ihbar süresine uyarak',
        '6': '6 haftalık ihbar süresine uyarak',
        '8': '8 haftalık ihbar süresine uyarak'
    };
    
    const html = `<div class="doc">
        ${docHeader('İSTİFA DİLEKÇESİ')}
        <div class="doc-section" style="text-align:right;margin-bottom:20px">
            <p><strong>Tarih:</strong> ${tarih}</p>
        </div>
        <div class="doc-section">
            <p style="margin-bottom:20px"><strong>TAMASLAN KAFE RESTORAN VE GIDA HİZMETLERİ TİC. LTD. ŞTİ.</strong></p>
            <p><strong>Şube:</strong> ${sube}</p>
        </div>
        <div class="doc-section" style="margin-top:30px">
            <p>Sayın Yetkili,</p>
            <p style="margin-top:15px">Şirketinizde <strong>${giris}</strong> tarihinden beri <strong>${pozisyon}</strong> pozisyonunda çalışmaktayım.</p>
            <p style="margin-top:15px;background:#f8f9fa;padding:15px;border-radius:8px">${neden}</p>
            <p style="margin-top:15px">Yukarıda belirttiğim nedenlerle, İş Kanunu hükümlerine uygun olarak <strong>${ihbarText[ihbar]}</strong> görevimden istifa ediyorum.</p>
            <p style="margin-top:15px"><strong>Son Çalışma Tarihim:</strong> ${sonTarih}</p>
            <p style="margin-top:20px">Gereğini arz ederim.</p>
        </div>
        <div style="margin-top:40px;text-align:right">
            <p><strong>${personel}</strong></p>
            <p style="font-size:10pt;color:#666">TC: ${tc}</p>
            <p style="margin-top:30px">İmza: _____________________</p>
        </div>
        <div style="margin-top:40px;padding-top:20px;border-top:1px dashed #ccc">
            <p><strong>İşveren Onayı:</strong></p>
            <p style="margin-top:10px">Tebliğ Alan: _____________________ Tarih: ___/___/___</p>
        </div>
    </div>`;
    
    showPreview('İstifa Dilekçesi - ' + personel, html);
}
...
function generateIbraname() {
    ...
    // Ödeme kalemleri
    const kalemler = [];
    if (document.getElementById('ibra_maas')?.checked) kalemler.push('Maaş Alacağı');
    if (document.getElementById('ibra_kidem')?.checked) kalemler.push('Kıdem Tazminatı');
    if (document.getElementById('ibra_ihbar')?.checked) kalemler.push('İhbar Tazminatı');
    if (document.getElementById('ibra_izin')?.checked) kalemler.push('Kullanılmayan Yıllık İzin');
    if (document.getElementById('ibra_fazla')?.checked) kalemler.push('Fazla Mesai Ücreti');
    if (document.getElementById('ibra_diger')?.checked) kalemler.push('Diğer Alacaklar');
    
    const kalemlerText = kalemler.length > 0 ? kalemler.join(', ') : 'Tüm alacaklar';
    
    const html = `<div class="doc">
        ${docHeader('İBRANAME')}
        <p style="text-align:center;color:#666;margin-top:-15px;margin-bottom:20px">TBK Md. 420 Uyarınca</p>
        <div class="doc-section">
            <p><strong>Tarih:</strong> ${tarih} | <strong>Belge No:</strong> ${docNo('IBR', sube)} | <strong>Şube:</strong> ${sube}</p>
        </div>
        <div class="doc-section">
            <table style="width:100%;border-collapse:collapse;margin:15px 0">
                ...
                <tr><td style="padding:8px;border:1px solid #ddd;background:#f9f9f9"><strong>İşe Giriş</strong></td><td style="padding:8px;border:1px solid #ddd">${giris}</td></tr>
                <tr><td style="padding:8px;border:1px solid #ddd;background:#f9f9f9"><strong>İşten Ayrılış</strong></td><td style="padding:8px;border:1px solid #ddd">${cikis}</td></tr>
        ...
        <div class="doc-section">
            <div class="doc-section-title">ÖDEME KALEMLERİ</div>
            <p style="background:#e8f5e9;padding:15px;border-radius:8px">${kalemlerText}</p>
            <p style="margin-top:15px;font-size:14pt"><strong>TOPLAM ÖDEME: ${tutar} TL</strong></p>
        </div>
        <div class="doc-section" style="background:#fff3e0;padding:15px;border-radius:8px;margin-top:20px">
            <p>Ben ${personel}, <strong>${giris}</strong> ile <strong>${cikis}</strong> tarihleri arasında ${sube} şubesinde çalıştım.</p>
            <p style="margin-top:10px">Yukarıda belirtilen <strong>${kalemlerText}</strong> dahil tüm işçilik alacaklarımın toplamı olan <strong>${tutar} TL</strong>'yi IBAN hesabıma aldım.</p>
            <p style="margin-top:10px">İşverenden hiçbir alacağım kalmadığını, ibra ettiğimi beyan ederim.</p>
        </div>
        <div style="background:#e3f2fd;padding:12px;border-radius:6px;font-size:10pt;margin:20px 0">
            <strong>TBK Md. 420:</strong> İşçi ibrası yazılı olmalı ve banka kanalıyla ödenmelidir. İmza tarihinden itibaren 1 ay içinde itiraz hakkı saklıdır.
        </div>
        ${docSignature(
            {title: 'İŞVEREN', name: yetkili, role: '(KAŞE / İmza)'},
            {title: 'PERSONEL', name: personel, role: 'El yazısı: Okudum, aldım, ibra ediyorum'}
        )}
    </div>`;
    
    showPreview('İbraname - ' + personel, html);
}

// ==================== BORÇ İKRAR ====================
function generateBorc() {
    const turLabels = {kasa: 'Kasa Açığı', zimmet: 'Zimmet Hasarı', stok: 'Stok Kaybı'};
    const odemeLabels = {tek: 'maaştan tek seferde mahsup edilmesini', taksit: 'maaştan taksitlerle mahsup edilmesini'};
    ...
    const html = `<div class="doc">
        ${docHeader('BORÇ İKRAR VE MAHSUP MUVAFAKATNAMESİ')}
        <p style="text-align:center;color:#666;margin-top:-15px;margin-bottom:20px">TBK Md. 407/2 Uyarınca</p>
        <div class="doc-section">
            <p><strong>Tarih:</strong> ${duzTarih} | <strong>Belge No:</strong> ${docNo('BRC', sube)} | <strong>Şube:</strong> ${sube}</p>
            <p><strong>Personel:</strong> ${personel}${tc ? ' | TC: ' + tc : ''}</p>
            <p><strong>Borç Türü:</strong> ${turLabels[tur]} | <strong>Tutar:</strong> ${tutar} TL | <strong>Olay:</strong> ${tarih}</p>
        </div>
        <div class="doc-section">
            <div class="doc-section-title">OLAY AÇIKLAMASI</div>
            <p style="background:#f9f9f9;padding:15px;border-left:4px solid #ff8674">${aciklama}</p>
        ...
        <div class="doc-section">
            <p>Ben ${personel}, ${tarih} tarihli olayda kusurum nedeniyle <strong>${tutar} TL</strong> borcumu kabul ediyor, TBK 407/2 uyarınca ${odemeLabels[odeme]} kendi irademle taahhüt ediyorum.</p>
            <p style="margin-top:10px">Baskı altında değilim, borcumu ve kesinti koşullarını anladığımı beyan ederim.</p>
        </div>
        <div style="background:#ebf5fb;padding:12px;border-radius:6px;font-size:10pt;margin:20px 0">
            <strong>TBK 407/2:</strong> İşveren, işçiden alacağını işçi rızası olmadan takas edemez.
        </div>
        ${docSignature(
            {title: 'İŞVEREN', name: '(KAŞE)', role: 'İmza'},
            {title: 'PERSONEL', name: personel, role: 'El yazısı: Okudum, kabul ediyorum'}
        )}
        ${sahit ? '<p style="margin-top:20px"><strong>Şahit:</strong> ' + sahit + ' _________________</p>' : ''}
    </div>`;
    
    showPreview('Borç İkrar - ' + personel, html);
}
...
function generateAvans() {
    ...
    // Tutar formatla ve yazıya çevir
    ...
    const html = `<div class="doc" style="font-size:11pt;line-height:1.5;-webkit-print-color-adjust:exact;print-color-adjust:exact">
        ...
        <table style="width:100%;border-collapse:collapse;margin:15px 0">
            ...
            <tr>
                <td style="padding:8px;border:1px solid #ddd;background-color:#f9f9f9;-webkit-print-color-adjust:exact"><strong>Şube</strong></td>
            ...
            <tr>
                ...
                <td style="padding:8px;border:1px solid #ddd" colspan="3"><strong>${tutarFormat} TL</strong> (${tutarYazi} Türk Lirası)</td>
            </tr>
            <tr>
                <td style="padding:8px;border:1px solid #ddd;background-color:#f9f9f9;-webkit-print-color-adjust:exact"><strong>Geri Ödeme</strong></td>
                <td style="padding:8px;border:1px solid #ddd" colspan="3">Gelecek ay maaşından tek seferde kesilecektir</td>
            </tr>
        </table>
        ...
        <div style="background-color:#f5f5f5;-webkit-print-color-adjust:exact;padding:10px;border-radius:4px;margin:15px 0">
            <strong>PERSONEL BEYANI:</strong> Ben ${personel}, yukarıda belirtilen tutarda avans talep ettiğimi, bu avansın onaylanması halinde gelecek ay maaşımdan tek seferde kesilerek mahsup edileceğini kabul ve taahhüt ederim.
        </div>
        <div style="background-color:#ebf5fb;-webkit-print-color-adjust:exact;padding:10px;border-radius:4px;font-size:10pt;margin:15px 0">
            <strong>YASAL BİLGİLENDİRME:</strong> 4857 sayılı İş Kanunu'nda avans ödenmesine ilişkin herhangi bir düzenleme bulunmamaktadır. Avans talebi tamamen işverenin inisiyatifinde olup, işveren avans verip vermemekte ve ödeme zamanını belirlemekte serbesttir. Talep, 7 iş günü içinde değerlendirilecek olup; onaylanması halinde en yakın ödeme döngüsünde, TBK Md. 407/2 uyarınca personelin yazılı muvafakati ile maaşından mahsup edilecektir. İşveren, gerekçe göstermeksizin talebi reddetme hakkını saklı tutar.
        </div>
        <table style="width:100%;margin-top:20px">
            <tr>
                <td style="width:50%;text-align:center;padding:15px;vertical-align:bottom">
                    <div style="margin-bottom:40px">${personel}</div>
                    <div style="border-top:1px solid #000;padding-top:8px"><strong>PERSONEL</strong><br><span style="font-size:10pt">Ad Soyad / İmza</span></div>
                </td>
                <td style="width:50%;text-align:center;padding:15px;vertical-align:bottom">
                    <div style="margin-bottom:15px">☐ ONAYLANDI &nbsp; ☐ REDDEDİLDİ</div>
                    <div style="margin-bottom:15px;font-size:10pt">Tarih: ___/___/________</div>
                    <div style="border-top:1px solid #000;padding-top:8px"><strong>İŞVEREN</strong><br><span style="font-size:10pt">Kaşe / İmza</span></div>
                </td>
            </tr>
        </table>
    </div>`;
...
// Sayıyı yazıya çevir fonksiyonu
...
function zimmetEkle() {
    ...
    div.innerHTML = `
        <div class="form-group" style="margin:0"><label class="form-label">Eşya Türü</label>
            <select class="form-select" id="zimmet_tur_${zimmetSayaci}">
                ...
                <option value="kart">Giriş Kartı</option>
                <option value="kiyafet">Üniforma / Kıyafet</option>
        ...
        <div class="form-group" style="margin:0"><label class="form-label">Marka / Model</label><input type="text" class="form-input" id="zimmet_marka_${zimmetSayaci}" placeholder="Örn: iPhone 17 Pro"></div>
        <div class="form-group" style="margin:0"><label class="form-label">Seri No / IMEI</label><input type="text" class="form-input" id="zimmet_seri_${zimmetSayaci}" placeholder="Varsa seri numarası"></div>
        <div class="form-group" style="margin:0"><label class="form-label">Tahmini Değer (TL)</label><input type="text" class="form-input" id="zimmet_deger_${zimmetSayaci}" placeholder="0"></div>
        <button class="btn btn-ghost" onclick="zimmetSil(${zimmetSayaci})" style="color:#e74c3c;padding:8px">🗑️</button>
    `;
...
function generateZimmet() {
    const islemLabels = {teslim: 'TESLİM (ZİMMET)', iade: 'İADE'};
    ...
    if (liste.length === 0) {
        alert('Lütfen en az bir eşya ekleyin.');
        return;
    }
    ...
    const html = `<div class="doc" style="font-size:11pt;line-height:1.5;-webkit-print-color-adjust:exact;print-color-adjust:exact">
        ${docHeader('DEMİRBAŞ TESLİM VE ZİMMET TUTANAĞI')}
        ...
        <div style="margin:20px 0">
            <strong style="font-size:12pt">ZİMMET EDİLEN EŞYALAR</strong>
            <table style="width:100%;border-collapse:collapse;margin:10px 0">
                <thead>
                    <tr style="background-color:#f0f0f0;-webkit-print-color-adjust:exact">
                        ...
                        <th style="padding:10px;border:1px solid #ddd">Eşya Türü</th>
                        ...
                        <th style="padding:10px;border:1px solid #ddd;width:100px">Değer</th>
                    </tr>
                </thead>
                <tbody>
                    ${esyaRows}
                ...
                <tfoot>
                    <tr style="background-color:#f9f9f9;-webkit-print-color-adjust:exact">
                        <td colspan="5" style="padding:10px;border:1px solid #ddd;text-align:right"><strong>TOPLAM DEĞER:</strong></td>
        ...
        <div style="background-color:#ebf5fb;-webkit-print-color-adjust:exact;padding:12px;border-radius:6px;font-size:10pt;margin:20px 0">
            <strong>PERSONEL TAAHHÜDÜ:</strong><br>
            Yukarıda belirtilen demirbaş/eşyaları eksiksiz ve çalışır durumda teslim aldığımı; bunları özenle kullanacağımı, işveren izni olmaksızın üçüncü kişilere devretmeyeceğimi, işten ayrılmam veya talep halinde derhal iade edeceğimi kabul ve taahhüt ederim.<br><br>
            <strong>Yasal Dayanak:</strong> TBK Md. 390 (özen borcu), TBK Md. 462 (teslim edilen eşyaların korunması), 4857 sayılı İş Kanunu Md. 25/II-e (güveni kötüye kullanma). Zimmet edilen eşyaların kaybı, hasarı veya iadeden imtina halinde, güncel rayiç bedeli üzerinden tazmin yükümlülüğü doğar.
        </div>
        
        <table style="width:100%;margin-top:40px;border:none;">
            <tr>
                <td style="width:50%;text-align:center;vertical-align:bottom;padding:20px;border:none;">
                    <p style="margin-bottom:60px">(Kaşe / İmza)</p>
                    <div style="border-top:1px solid #000;width:70%;margin:0 auto;padding-top:10px">
                        <p style="font-weight:bold">TESLİM EDEN</p>
                ...
                <td style="width:50%;text-align:center;vertical-align:bottom;padding:20px;border:none;">
                    ...
                    <p style="margin-bottom:20px">(İmza)</p>
                    <div style="border-top:1px solid #000;width:70%;margin:0 auto;padding-top:10px">
                        <p style="font-weight:bold">TESLİM ALAN</p>
                    </div>
                </td>
            </tr>
        </table>
...
// Sayfa yüklendiğinde varsayılan bir eşya satırı ekle
document.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => {
    ...
    list.innerHTML = items.map(v => `
        <div class="violation-item" onclick="showViolation('${v.id}')">
            <div class="violation-header">
                ...
                <span class="violation-badge ${v.risk}">${v.risk === 'low' ? 'Düşük' : v.risk === 'medium' ? 'Orta' : 'Yüksek'}</span>
            </div>
...
function showViolation(id) {
    ...
    const html = `<div class="doc">
        ...
        <div class="doc-section"><div class="doc-section-title">AÇIKLAMA</div><p>${v.desc}</p></div>
        <div class="doc-section"><div class="doc-section-title">İZLENECEK PROSEDÜR</div><ol style="margin-left:20px;line-height:2">${v.steps.map(s => '<li>' + s + '</li>').join('')}</ol></div>
        <div style="background:${colors[v.risk]};padding:15px;border-radius:6px"><strong>Risk:</strong> ${v.risk === 'low' ? 'DÜŞÜK - Uyarı ile çözülebilir' : v.risk === 'medium' ? 'ORTA - Savunma ve ihtar gerektirir' : 'YÜKSEK - Haklı fesih sebebi'}</div>
    </div>`;
...
function openAdminWithPin() {
    // Artık PIN sormuyoruz, giriş yapılmış kullanıcının yetkisini kontrol ediyoruz
    if (!currentUser) {
        toast('Lütfen önce giriş yapın', 'error');
        return;
    }
    ...
    // Diğer roller için gelişmiş izin kontrolü
...
function showAdminTab(tab, btn) {
    // Kaydedilmemiş belge şablonu değişikliği kontrolü
...
function renderAdminLists() {
    // Yönetici-only tab'ları kontrol et
    ...
    // Yönetici için özel kartları göster
        ...
        branchList.innerHTML = STATE.branches.map((b, i) => `
            <div class="admin-list-item">
                ...
                <div class="admin-list-item-actions">
                    <button class="admin-btn-icon admin-btn-edit" onclick="editBranch(${i})">✏️</button>
                    <button class="admin-btn-icon admin-btn-delete" onclick="deleteBranch(${i})">🗑️</button>
                </div>
            </div>
        `).join('');
    }
    ...
    if (posList) {
        posList.innerHTML = STATE.positions.map((p, i) => `
            <div class="admin-list-item">
                ...
                <div class="admin-list-item-actions">
                    <button class="admin-btn-icon admin-btn-edit" onclick="editPosition(${i})">✏️</button>
                    <button class="admin-btn-icon admin-btn-delete" onclick="deletePosition(${i})">🗑️</button>
                </div>
            </div>
        `).join('');
    }
    ...
    if (violList) {
        violList.innerHTML = STATE.violations.map((v, i) => `
            <div class="admin-list-item">
                ...
                <div class="admin-list-item-actions">
                    <button class="admin-btn-icon admin-btn-edit" onclick="editViolation(${i})">✏️</button>
                    <button class="admin-btn-icon admin-btn-delete" onclick="deleteViolation(${i})">🗑️</button>
                </div>
            </div>
        `).join('');
    }
...
async function cleanDuplicatePersonel() {
    ...
    resultsDiv.innerHTML = '<p style="color:var(--tx2)">⏳ Taranıyor...</p>';
    ...
    try {
        ...
        if (toDelete.length === 0) {
            resultsDiv.innerHTML = '<p style="color:var(--green)">✅ Duplicate personel bulunamadı.</p>';
            return;
        }
        
        // Silme işlemi için onay
        resultsDiv.innerHTML = `
            <p style="color:var(--yellow);margin-bottom:12px">⚠️ ${toDelete.length} duplicate personel bulundu:</p>
            ...
            <button class="btn btn-primary btn-sm" onclick="confirmDeleteDuplicates('${JSON.stringify(toDelete.map(p => p.id)).replace(/"/g, '&quot;')}')">Sil (${toDelete.length} kayıt)</button>
            <button class="btn btn-ghost btn-sm" onclick="document.getElementById('cleanupResults').style.display='none'">İptal</button>
        `;
    } catch (e) {
        resultsDiv.innerHTML = `<p style="color:var(--red)">❌ Hata: ${e.message}</p>`;
    }
}
...
async function confirmDeleteDuplicates(idsJson) {
    ...
    const resultsDiv = document.getElementById('cleanupResults');
    resultsDiv.innerHTML = '<p style="color:var(--tx2)">⏳ Siliniyor...</p>';
    ...
    try {
        ...
        await batch.commit();
        resultsDiv.innerHTML = `<p style="color:var(--green)">✅ ${ids.length} duplicate personel silindi.</p>`;
...
async function cleanDuplicateShifts() {
    ...
    try {
        ...
        snapshot.forEach(doc => {
            ...
            if (shiftMap[key]) {
                // Bu bir duplicate - daha eski olanı sil
        ...
        if (toDelete.length === 0) {
            resultsDiv.innerHTML = '<p style="color:var(--green)">✅ Duplicate shift bulunamadı.</p>';
            return;
        }
        
        resultsDiv.innerHTML = `
            <p style="color:var(--yellow);margin-bottom:12px">⚠️ ${toDelete.length} duplicate shift bulundu.</p>
            <button class="btn btn-primary btn-sm" onclick="confirmDeleteDuplicateShifts('${JSON.stringify(toDelete.map(p => p.id)).replace(/"/g, '&quot;')}')">Sil (${toDelete.length} kayıt)</button>
            <button class="btn btn-ghost btn-sm" onclick="document.getElementById('cleanupResults').style.display='none'">İptal</button>
        `;
    } catch (e) {
        resultsDiv.innerHTML = `<p style="color:var(--red)">❌ Hata: ${e.message}</p>`;
...
async function confirmDeleteDuplicateShifts(idsJson) {
    ...
    try {
        ...
        await batch.commit();
        resultsDiv.innerHTML = `<p style="color:var(--green)">✅ ${ids.length} duplicate shift silindi.</p>`;
...
// Sayfa yüklendiğinde önizleme durumunu kontrol et
    ...
    if (saved) {
        try {
            ...
            if (data.originalUser && data.previewUser) {
                ...
                // UI güncelle
                const roleNames = { 'barista': 'Barista', 'magaza_mudur_yardimcisi': 'Mağaza Müdür Yardımcısı', 'bolge_muduru': 'Bölge Müdürü', 'magaza_muduru': 'Mağaza Müdürü', 'yonetici': 'Yönetici' };
                
                document.getElementById('previewRoleName').textContent = roleNames[currentUser.role];
                document.getElementById('previewBranchName').textContent = currentUser.branch || 'Tüm Şubeler';
                ...
                const userNameEl = document.getElementById('userName');
                if (userNameEl) userNameEl.innerHTML = `<span style="background:var(--yellow);color:#000;padding:2px 6px;border-radius:4px;font-size:.7rem;margin-right:6px">ÖNİZLEME</span>${roleNames[currentUser.role]}`;
                
                const userRoleEl = document.getElementById('userRole');
                if (userRoleEl) userRoleEl.textContent = currentUser.branch || 'Tüm Şubeler';
...
function showPreviewTopBar() {
    ...
    bar.innerHTML = `
        <div style="display:flex;align-items:center;gap:12px">
            ...
            <span><strong>Önizleme Modu:</strong> ${roleNames[currentUser.role]} ${currentUser.branch && currentUser.branch !== 'all' ? '• ' + currentUser.branch : ''}</span>
        </div>
        <button onclick="stopRolePreview()" style="background:rgba(255,255,255,.2);border:1px solid rgba(255,255,255,.4);color:#fff;padding:6px 14px;border-radius:6px;cursor:pointer;font-size:.85rem;font-weight:500">✕ Önizlemeyi Sonlandır</button>
    `;
...
function startRolePreview() {
    ...
    if (!role) {
        toast('Lütfen bir rol seçin', 'error');
        return;
    }
    if (!branch && role === 'magaza_muduru') {
        toast('Mağaza Müdürü için şube seçimi gerekli', 'error');
        return;
    }
    
    // Mevcut kullanıcıyı sakla
    originalUser = { ...currentUser };
    isPreviewMode = true;
    
    // Önizleme kullanıcısını ayarla
    const roleNames = {
        'barista': 'Barista',
        'magaza_mudur_yardimcisi': 'Mağaza Müdür Yardımcısı',
        'bolge_muduru': 'Bölge Müdürü',
    ...
    currentUser = {
        ...
        name: `Önizleme: ${roleNames[role]}`
    };
    
    // UI güncelle
    document.getElementById('previewRoleName').textContent = roleNames[role];
    document.getElementById('previewBranchName').textContent = branch || 'Tüm Şubeler';
    document.getElementById('rolePreviewContent').style.display = 'none';
    document.getElementById('rolePreviewActive').style.display = 'block';
    
    // Sidebar'a önizleme badge'i ekle
    const userNameEl = document.getElementById('userName');
    if (userNameEl) {
        userNameEl.innerHTML = `<span style="background:var(--yellow);color:#000;padding:2px 6px;border-radius:4px;font-size:.7rem;margin-right:6px">ÖNİZLEME</span>${roleNames[role]}`;
    }
    const userRoleEl = document.getElementById('userRole');
    if (userRoleEl) {
        userRoleEl.textContent = branch || 'Tüm Şubeler';
    }
    ...
    toast(`${roleNames[role]} olarak önizleme başladı`, 'info');
}

function applyRolePreviewPermissions(role, branch) {
    // Barista için özel kısıtlamalar
    if (role === 'barista') {
    ...
    // Şube filtreleme (mağaza müdürü sadece kendi şubesini görür)
    if (role === 'magaza_muduru' && branch) {
        // Şube seçicileri kilitle
        document.querySelectorAll('select[id*="Sube"]').forEach(select => {
            select.value = branch;
            select.disabled = true;
        });
...
function stopRolePreview() {
    ...
    // Sidebar kullanıcı bilgisini geri yükle
    applyUserPermissions();
    ...
    // Tüm nav gruplarını göster
    document.querySelectorAll('.nav-group').forEach(group => {
        group.style.display = '';
    });
    
    // Tüm dashboard kartlarını göster
    document.querySelectorAll('.dashboard-card').forEach(card => {
        card.style.display = '';
    });
    
    // Tüm dashboard section'ları göster
    document.querySelectorAll('.dashboard-section').forEach(section => {
    ...
    toast('Önizleme sonlandırıldı, yönetici görünümüne dönüldü', 'success');
}
...
function changePin() {
    ...
    successModal('PIN başarıyla değiştirildi!');
}
...
function saveCompany() {
    ...
    saveState();
    document.getElementById('companyName').textContent = STATE.company.name;
    toast('Şirket bilgileri kaydedildi', 'success');
}
...
function addBranch() {
    showModal('Şube Ekle', `
        <div class="form-group">
            ...
            <input type="text" id="newBranchName" class="form-input" placeholder="Örn: Kadıköy">
        </div>
    `, `
        <button class="btn btn-ghost" onclick="closeModal()">İptal</button>
...
function deleteBranch(i) {
    confirmModal('Şube Sil', `"${STATE.branches[i]}" şubesini silmek istediğinize emin misiniz?`, () => {
...
function addPosition() {
    showModal('Pozisyon Ekle', `
        <div class="form-grid">
            ...
            <div class="form-group"><label class="form-label">Ad</label><input type="text" id="newPosName" class="form-input" placeholder="Örnek Pozisyon"></div>
...
function deletePosition(i) {
    confirmModal('Pozisyon Sil', `"${STATE.positions[i].name}" pozisyonunu silmek istediğinize emin misiniz?`, () => {
...
function addViolation() {
    showModal('İhlal Ekle', `
        <div class="form-grid">
            <div class="form-group"><label class="form-label">Kod</label><input type="text" id="newViolId" class="form-input" placeholder="V19"></div>
            <div class="form-group"><label class="form-label">Başlık</label><input type="text" id="newViolTitle" class="form-input"></div>
            <div class="form-group"><label class="form-label">Risk</label>
                <select id="newViolRisk" class="form-select"><option value="low">Düşük</option><option value="medium">Orta</option><option value="high">Yüksek</option></select>
            </div>
            <div class="form-group"><label class="form-label">Yasal Dayanak</label><input type="text" id="newViolLegal" class="form-input" placeholder="İş K. 25/II-x"></div>
            <div class="form-group full"><label class="form-label">Açıklama</label><input type="text" id="newViolDesc" class="form-input"></div>
            <div class="form-group full"><label class="form-label">Prosedür Adımları (virgülle ayırın)</label><input type="text" id="newViolSteps" class="form-input" placeholder="Tutanak,Savunma,İhtar"></div>
        </div>
    `, `
        <button class="btn btn-ghost" onclick="closeModal()">İptal</button>
...
function saveViolation() {
    ...
    if (v.id && v.title) {
        ...
        renderAdminLists();
        closeModal();
        toast('İhlal eklendi', 'success');
    }
}

function editViolation(i) {
    const v = STATE.violations[i];
    showModal('İhlal Düzenle', `
        <div class="form-grid">
            <div class="form-group"><label class="form-label">Kod</label><input type="text" id="editViolId" class="form-input" value="${v.id}"></div>
            <div class="form-group"><label class="form-label">Başlık</label><input type="text" id="editViolTitle" class="form-input" value="${v.title}"></div>
            <div class="form-group"><label class="form-label">Risk</label>
                <select id="editViolRisk" class="form-select">
                    <option value="low" ${v.risk==='low'?'selected':''}>Düşük</option>
                    <option value="medium" ${v.risk==='medium'?'selected':''}>Orta</option>
                    <option value="high" ${v.risk==='high'?'selected':''}>Yüksek</option>
                </select>
            </div>
            ...
            <div class="form-group full"><label class="form-label">Açıklama</label><input type="text" id="editViolDesc" class="form-input" value="${v.desc}"></div>
            <div class="form-group full"><label class="form-label">Prosedür Adımları</label><input type="text" id="editViolSteps" class="form-input" value="${v.steps.join(', ')}"></div>
        </div>
    `, `
        <button class="btn btn-ghost" onclick="closeModal()">İptal</button>
...
function deleteViolation(i) {
    confirmModal('İhlal Sil', `"${STATE.violations[i].title}" ihlalini silmek istediğinize emin misiniz?`, () => {
        STATE.violations.splice(i, 1);
        saveState();
        renderViolations();
        renderAdminLists();
        toast('İhlal silindi', 'success');
    }, 'danger');
}
...
function generateExportCode() {
    try {
        ...
        toast('Kod oluşturuldu', 'success');
    } catch(e) {
        toast('Kod oluşturulamadı', 'error');
    }
}
...
function importCode() {
    ...
    confirmModal('İçe Aktar', 'Mevcut tüm ayarlarınız değiştirilecek. Emin misiniz?', () => {
        try {
            ...
            successModal('Veriler başarıyla içe aktarıldı!');
        } catch(e) {
            toast('Geçersiz kod!', 'error');
        }
    }, 'danger');
}
...
function downloadBackup() {
    try {
        ...
        toast('Yedek oluşturulamadı', 'error');
    }
}
...
function uploadBackup(event) {
    ...
    if (!file.name.endsWith('.json')) {
        toast('Sadece .json dosyası yükleyebilirsiniz', 'error');
        return;
    }
    
    confirmModal('Yedek Yükle', `"${file.name}" dosyasını yüklemek istediğinize emin misiniz? Mevcut tüm ayarlarınız değiştirilecek.`, () => {
        ...
        reader.onload = (e) => {
            try {
                ...
                saveState();
                initUI();
                successModal('Yedek başarıyla yüklendi!');
            } catch(err) {
...
function saveDocTemplate() {
    ...
    // documentTemplates yoksa oluştur
...
function resetDocTemplate() {
    confirmModal('Varsayılana Dön', 'Bu şablonu varsayılan haline döndürmek istediğinize emin misiniz?', () => {
        let defaultTemplate = '';
...
function previewDocTemplate() {
    ...
    const preview = content
        .replace(/\{\{PERSONEL\}\}/g, 'Örnek Personel')
        .replace(/\{\{TC\}\}/g, '12345678901')
        .replace(/\{\{SUBE\}\}/g, 'Şişli (Merkez)')
        .replace(/\{\{TARIH\}\}/g, today())
        .replace(/\{\{SAAT\}\}/g, nowTime())
        .replace(/\{\{ACIKLAMA\}\}/g, 'Örnek açıklama metni burada görünecektir.')
        .replace(/\{\{DAYANAK\}\}/g, '25/II-e')
        .replace(/\{\{SURE\}\}/g, '1 iş günü')
        .replace(/\{\{POZISYON\}\}/g, 'Barista')
        .replace(/\{\{TUTAR\}\}/g, '500')
        .replace(/\{\{ODEME_SEKLI\}\}/g, 'maaştan tek seferde mahsup edilmesini')
        .replace(/\{\{IHBAR_SURE\}\}/g, '4 hafta')
        .replace(/\{\{SON_GUN\}\}/g, today())
        .replace(/\{\{BASLAMA\}\}/g, today());
    
    const categoryTitles = {
        tutanak: {olay: 'OLAY TESPİT TUTANAĞI', devamsizlik: 'DEVAMSIZLIK TUTANAĞI', kasa: 'KASA AÇIĞI TUTANAĞI', imza: 'İMZADAN İMTİNA TUTANAĞI'},
        savunma: 'SAVUNMA İSTEME YAZISI',
        fesih: {hakli: 'HAKLI FESİH BİLDİRİMİ', gecerli: 'GEÇERLİ FESİH BİLDİRİMİ', deneme: 'DENEME SÜRESİ FESHİ'},
        borc: 'BORÇ İKRAR VE MAHSUP MUVAFAKATNAMESİ'
    };
    ...
    const html = `<div class="doc">
        ...
        ${docSignature(
            {title: 'İŞVEREN', name: '(KAŞE)', role: 'İmza'},
            {title: 'PERSONEL', name: 'Örnek Personel', role: 'İmza'}
        )}
    </div>`;
    
    showPreview('Şablon Önizleme - ' + title, html);
}

function showUnsavedChangesModal(onSave, onDiscard) {
    openModal('Kaydedilmemiş Değişiklikler', `
        <div class="modal-icon warning">⚠️</div>
        <p class="modal-message">Şablonda kaydedilmemiş değişiklikler var.<br>Ne yapmak istersiniz?</p>
    `, `
        <button class="btn btn-ghost" onclick="closeModal()">İptal</button>
...
// Tarayıcı kapatma/yenileme koruması
window.addEventListener('beforeunload', (e) => {
    // Tarayıcı kapatılınca çıkış yap ayarı aktifse oturumu sonlandır
    const logoutOnClose = document.getElementById('sessionLogoutOnClose')?.checked;
    if (logoutOnClose && currentUser?.sessionId) {
        // Senkron olarak oturumu sonlandırmaya çalış (best effort)
...
// Visibility change - sayfa gizlendiğinde aktiviteyi duraklat
document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible' && currentUser?.sessionId) {
        // Sayfa tekrar görünür olduğunda aktiviteyi güncelle
...
async function loadPersonelList() {
    ...
    container.innerHTML = '<p style="color:var(--tx2);text-align:center;padding:20px">Yükleniyor...</p>';
    
    try {
        const snapshot = await db.collection('personel').orderBy('name').get();
        
        if (snapshot.empty) {
            container.innerHTML = '<p style="color:var(--tx2);text-align:center;padding:40px">Henüz personel kaydı yok. "Yeni Personel" butonuna tıklayarak ekleyin.</p>';
            return;
        }
        
        // Pozisyondan rol belirleme
        const positionToRole = {
            'Barista': 'barista',
            'Barista / Servis': 'barista',
            'Mağaza Müdürü': 'magaza_muduru',
            'Mağaza Müdür Yardımcısı': 'magaza_mudur_yardimcisi',
            'Bölge Müdürü': 'bolge_muduru'
        };
        
        // Rol görünümleri - SVG ikonlar
        const roleStyles = {
            'yonetici': { name: 'Yönetici', color: '#ff8674', icon: '<svg width="16" height="16" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z"/></svg>' },
            'bolge_muduru': { name: 'Bölge Müdürü', color: '#9b59b6', icon: '<svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 014 10 15.3 15.3 0 01-4 10 15.3 15.3 0 01-4-10 15.3 15.3 0 014-10z"/></svg>' },
            'magaza_muduru': { name: 'Mağaza Müdürü', color: '#008c95', icon: '<svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>' },
            'magaza_mudur_yardimcisi': { name: 'Müdür Yrd.', color: '#3498db', icon: '<svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>' },
            'barista': { name: 'Barista', color: '#27ae60', icon: '<svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M18 8h1a4 4 0 010 8h-1"/><path d="M2 8h16v9a4 4 0 01-4 4H6a4 4 0 01-4-4V8z"/><line x1="6" y1="1" x2="6" y2="4"/><line x1="10" y1="1" x2="10" y2="4"/><line x1="14" y1="1" x2="14" y2="4"/></svg>' }
        };
        ...
        snapshot.forEach(doc => {
            ...
            const role = p.role || positionToRole[p.position] || 'barista';
            const roleInfo = roleStyles[role] || { name: 'Personel', color: '#9ca3af', icon: '<svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>' };
            const hasAccess = !!(p.email && p.password);
            
            // Şube rengi
            const isMoonberry = (p.branch || '').includes('Şişli') || (p.branch || '').includes('Merkez');
            const branchColor = isMoonberry ? '#008c95' : '#ff8674';
            
            html += `
                <div style="background:var(--bg2);border:1px solid var(--cardBorder);border-left:3px solid ${branchColor};border-radius:10px;padding:14px;transition:all .15s" onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='0 4px 12px rgba(0,0,0,.08)'" onmouseout="this.style.transform='none';this.style.boxShadow='none'">
                    <!-- Header -->
                    <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px">
                        <div style="width:40px;height:40px;background:linear-gradient(135deg,${roleInfo.color},${roleInfo.color}88);border-radius:10px;display:flex;align-items:center;justify-content:center;color:#fff">
                            ${roleInfo.icon}
                        </div>
                        <div style="flex:1;min-width:0">
                            <div style="font-weight:600;color:var(--tx);font-size:.9rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${p.name || 'İsimsiz'}</div>
                            <div style="font-size:.75rem;color:var(--tx3)">${p.position || 'Pozisyon yok'}</div>
                        </div>
                        <span style="background:${roleInfo.color}15;color:${roleInfo.color};padding:3px 8px;border-radius:5px;font-size:.65rem;font-weight:600">${roleInfo.name}</span>
                    </div>
                    ...
                    <div style="display:flex;align-items:center;justify-content:space-between;padding-top:10px;border-top:1px solid var(--bg4)">
                        <div style="font-size:.7rem;color:${hasAccess ? '#27ae60' : 'var(--tx3)'}; display:flex;align-items:center;gap:4px">
                            ...
                            ${hasAccess ? 'Aktif' : (p.email ? 'Şifre yok' : 'E-posta yok')}
                        </div>
        ...
        const withAccess = snapshot.docs.filter(d => d.data().email && d.data().password).length;
        const byBranch = {};
        snapshot.forEach(doc => {
            const branch = doc.data().branch || 'Belirtilmemiş';
            byBranch[branch] = (byBranch[branch] || 0) + 1;
        });
        
        const statsHtml = `
            <div style="display:flex;gap:12px;margin-bottom:20px;flex-wrap:wrap">
                <div style="background:linear-gradient(135deg,#008c95,#00a8a8);padding:12px 20px;border-radius:10px;color:#fff;display:flex;align-items:center;gap:10px">
                    <span style="font-size:1.5rem">👥</span>
                    <div><div style="font-size:1.4rem;font-weight:700">${total}</div><div style="font-size:.75rem;opacity:.8">Toplam Personel</div></div>
                </div>
                <div style="background:linear-gradient(135deg,#27ae60,#2ecc71);padding:12px 20px;border-radius:10px;color:#fff;display:flex;align-items:center;gap:10px">
                    <span style="font-size:1.5rem">🔓</span>
                    <div><div style="font-size:1.4rem;font-weight:700">${withAccess}</div><div style="font-size:.75rem;opacity:.8">Giriş Yapabilen</div></div>
                </div>
                ${Object.entries(byBranch).map(([branch, count]) => `
                    <div style="background:var(--bg3);padding:12px 20px;border-radius:10px;display:flex;align-items:center;gap:10px">
                        <span style="font-size:1.2rem">📍</span>
                        <div><div style="font-size:1.1rem;font-weight:600;color:var(--tx)">${count}</div><div style="font-size:.7rem;color:var(--tx2)">${branch}</div></div>
                    </div>
                `).join('')}
            </div>
        `;
        ...
        container.innerHTML = '<p style="color:var(--red);text-align:center;padding:20px">Yükleme hatası: ' + error.message + '</p>';
    }
}
...
function showAddPersonelModal(editId = null) {
    ...
    // Brüt ücret sadece yönetici ve bölge müdürü görebilir
    const canSeeSalary = currentUser?.role === 'yonetici' || currentUser?.role === 'bolge_muduru';
    const salaryField = canSeeSalary 
        ? `<div class="form-group"><label class="form-label">Brüt Ücret (TL)</label><input type="number" id="personelSalary" class="form-input"></div>`
    ...
    const body = `
        ...
        <div class="form-grid">
            ...
            <div class="form-group"><label class="form-label">Shift Kod Adı</label><input type="text" id="personelCodeName" class="form-input" placeholder="Örn: Esin İ."><small style="color:var(--tx2);font-size:.7rem">Boş bırakılırsa Ad S. formatı kullanılır</small></div>
            ...
            <div class="form-group"><label class="form-label">Şube</label><select id="personelBranch" class="form-select">${STATE.branches.map(b => `<option value="${b}">${b}</option>`).join('')}</select></div>
            ...
            <div class="form-group"><label class="form-label">İşe Başlama</label><input type="date" id="personelStartDate" class="form-input"></div>
            ${salaryField}
        </div>
        
        <!-- SİSTEM GİRİŞİ -->
        <div style="margin-top:20px;padding:16px;background:linear-gradient(135deg,rgba(0,140,149,.08),rgba(255,134,116,.08));border-radius:12px;border:1px solid var(--cardBorder)">
            <div style="font-weight:600;color:var(--tx);margin-bottom:12px;display:flex;align-items:center;gap:8px">
                <svg width="18" height="18" fill="none" stroke="#008c95" stroke-width="2" viewBox="0 0 24 24"><path d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                Sistem Girişi
            </div>
            <div class="form-grid">
                <div class="form-group">
                    <label class="form-label">E-posta</label>
                    <input type="email" id="personelEmail" class="form-input" placeholder="ornek@moonberrycoffee.com">
                </div>
                <div class="form-group">
                    <label class="form-label">Şifre ${isEdit ? '(değiştirmek için girin)' : ''}</label>
                    <input type="password" id="personelPassword" class="form-input" minlength="6" placeholder="${isEdit ? 'Boş bırakılırsa değişmez' : 'En az 6 karakter'}">
                </div>
            </div>
            <div style="font-size:.75rem;color:var(--tx2);margin-top:8px">
                ⓘ E-posta ve şifre girilirse personel sisteme giriş yapabilir. Rol pozisyona göre otomatik belirlenir.
            </div>
        </div>
    `;
...
async function loadPersonelForEdit(id) {
    try {
        ...
        if (doc.exists) {
            ...
            // Şifre alanı - düzenleme modunda boş bırakılır (değişmeyecekse)
        }
    } catch (error) {
        console.error('Personel yükleme hatası:', error);
    }
...
async function savePersonel() {
    ...
    // Yeni kullanıcı ve email + şifre girildiyse şifre kontrolü
    if (email && password && password.length < 6) {
        toast('Şifre en az 6 karakter olmalı', 'error');
        return;
    }
    
    // Kod adı boşsa otomatik oluştur: "Ad S."
    if (!codeName) {
    ...
    // ====== YENİ: BENZERSİZ uniqueId OLUŞTUR ======
    // Format: busra_sogut (tam isim, Türkçe karakterler transliterate)
    ...
    const role = positionToRole[position] || 'barista';
    
    const data = {
        name,
        codeName,
        uniqueId,  // ← YENİ: Benzersiz ID
        tc,
    ...
    // ====== uniqueId ÇAKIŞMA KONTROLÜ ======
    if (!editId) {
        // Yeni personel ekleniyor, aynı uniqueId var mı kontrol et
        const existingCheck = await db.collection('personel').where('uniqueId', '==', uniqueId).get();
        if (!existingCheck.empty) {
            toast(`"${name}" ile aynı ID'ye sahip personel zaten var!`, 'error');
            return;
        }
    }
    
    // Şifre girilmişse kaydet
    if (password) {
    ...
    try {
        // Email ve şifre varsa Firebase Auth'ta kullanıcı oluştur
        if (email && password) {
            try {
                // Secondary app ile kullanıcı oluştur (mevcut oturumu bozmaz)
                ...
                console.log('Firebase Auth kullanıcısı oluşturuldu:', email);
                
                // Secondary app'i temizle
                await secondaryApp.delete();
                
                // Şifreyi Firestore'dan kaldır (artık Auth'ta)
                delete data.password;
                data.hasFirebaseAuth = true;
                
                toast('Kullanıcı hesabı oluşturuldu ✓', 'success');
            } catch (authError) {
                if (authError.code === 'auth/email-already-in-use') {
                    console.log('Kullanıcı zaten mevcut:', email);
                    // Şifreyi Firestore'da tut (login'de migrate edilecek)
                } else if (authError.code === 'auth/app/duplicate-app') {
                    // Secondary app zaten var, devam et
                    console.log('Secondary app zaten mevcut');
                } else {
                    console.error('Auth hatası:', authError);
                    // Şifreyi Firestore'da sakla, login'de migrate edilir
                }
            }
        }
        
        // Email varsa users koleksiyonunu da güncelle
        if (email) {
        ...
        if (editId) {
            ...
            // İlk isimler (Türkçe normalize)
            const oldFirstName = normalizeTurkishChars(oldName.split(' ')[0] || '');
            const newFirstName = normalizeTurkishChars(name.split(' ')[0] || '');
            
            console.log(`[Personel Güncelleme] Eski: "${oldName}" (${oldCodeName}) → Yeni: "${name}" (${codeName})`);
            
            // Şimdi güncelle
            await db.collection('personel').doc(editId).update(data);
            
            // ====== SHIFT'LERDE İSİM GÜNCELLEMESİ ======
            // Personel adı/kod adı değişince mevcut shift'leri güncelle
            try {
                const newDisplayName = getPersonelDisplayName(data);
                
                // TÜM shift'leri kontrol et (bölge müdürü tüm şubelerde olabilir)
                const shiftsSnap = await db.collection('shifts').get();
                let updatedCount = 0;
                ...
                for (const shiftDoc of shiftsSnap.docs) {
                    ...
                    // Branch kontrolü (bölge müdürü hariç tüm şubeler)
                    ...
                    // Bu personelin shift'teki key'ini bul - GENİŞLETİLMİŞ EŞLEŞME
                    for (const key of Object.keys(personelShifts)) {
                        const keyLower = key.toLowerCase();
                        const keyNorm = normalizeTurkishChars(key);
                        const keyFirst = keyNorm.split(/[_\s\.]/)[0]; // İlk isim
                        
                        // Eşleşme kontrolleri (eski ve yeni değerlerle)
                        const matches = 
                            // TAM EŞLEŞMEler
                            key === codeName ||
                            key === name ||
                            ...
                            // personelNames/personelFullNames EŞLEŞMEsi
                            ...
                            // İLK İSİM EŞLEŞMESİ (en önemli!)
                            (keyFirst && keyFirst.length > 2 && keyFirst === oldFirstName);
                        
                        if (matches) {
                            matchedKey = key;
                            console.log(`[Shift Eşleşme] ${shiftDoc.id}: key="${key}" → "${newDisplayName}"`);
                            break;
                        }
                    }
                    ...
                    if (needsUpdate) {
                        ...
                        updatedCount++;
                        console.log(`[Shift Güncelleme] ${shiftDoc.id}: "${matchedKey}" → "${newDisplayName}"`);
                    }
                }
                
                if (updatedCount > 0) {
                    console.log(`[Shift Güncelleme] Toplam ${updatedCount} shift güncellendi`);
                }
            } catch (shiftErr) {
                console.warn('Shift güncelleme hatası (kritik değil):', shiftErr);
            }
        ...
        // Shift sayfası açıksa tabloyu yenile
...
function editPersonel(id) {
    if (!id || id === 'undefined' || id === '') {
        toast('Geçersiz personel ID!', 'error');
        return;
    }
...
async function deletePersonel(id) {
    ...
    if (!id || id === 'undefined' || id === '') {
        toast('Geçersiz personel ID!', 'error');
        console.error('deletePersonel: Boş veya geçersiz ID:', id);
        return;
    }
    
    confirmModal('Personel Sil', 'Bu personeli silmek istediğinize emin misiniz?', async () => {
        try {
    ...
    try {
        ...
        if (doc.exists) {
            ...
            primSettingsCache = {
                ...
                warning: '-5 puana düşersen prim hakkını kaybedersin!',
                motivation: 'Prim sisteminde başarılı olmanın en önemli adımı check listelerini zamanında doldurmak. Her vaktinde tamamlanan check sana +1 puan kazandırır. Gün içinde 3 check varsa, hepsini vaktinde yaparsan günde +3 puan biriktirirsin.',
                motivation2: 'Ciro hedefini tutturmanın yolu ek ürün satışından geçer. Özellikle Cuma, Cumartesi ve Pazar günleri müşteri yoğunluğu yüksek. Kasada her müşteriye kurabiye, çikolata veya sezonluk içecek önermek, kişi başı harcamayı artırır. Hafta sonu 500 müşteriye ortalama 5 TL ek ürün satışı = 2.500 TL ekstra ciro demek!',
                quote: 'Checklist\'i doldur, ek ürün öner, prim cebinde kalsın.'
            };
        }
    } catch (e) {
        console.error('Prim ayarları yüklenemedi:', e);
    ...
    container.innerHTML = `
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:20px">
            <div style="background:var(--bg2);border-radius:10px;padding:14px">
                <div style="font-size:.8rem;font-weight:600;color:var(--tx);margin-bottom:10px">Check Puanları</div>
                <div style="display:flex;flex-direction:column;gap:6px;font-size:.75rem">
                    ...
                    <div style="display:flex;justify-content:space-between"><span style="color:var(--tx2)">Geç</span><span style="color:#e67e22;font-weight:600">-1</span></div>
                    <div style="display:flex;justify-content:space-between"><span style="color:var(--tx2)">Yapılmayan</span><span style="color:#e74c3c;font-weight:600">-2</span></div>
                    <div style="display:flex;justify-content:space-between"><span style="color:var(--tx2)">Platform eksik</span><span style="color:#9b59b6;font-weight:600">-1</span></div>
                </div>
            </div>
            <div style="background:var(--bg2);border-radius:10px;padding:14px">
                <div style="font-size:.8rem;font-weight:600;color:var(--tx);margin-bottom:10px">Yönetici Değerlendirmesi</div>
                <div style="display:flex;flex-direction:column;gap:6px;font-size:.75rem">
                    <div style="display:flex;justify-content:space-between"><span style="color:var(--tx2)">★ Ciddi ihlal</span><span style="color:#c0392b;font-weight:600">-3</span></div>
                    <div style="display:flex;justify-content:space-between"><span style="color:var(--tx2)">★★ Orta ihlal</span><span style="color:#d35400;font-weight:600">-2</span></div>
                    <div style="display:flex;justify-content:space-between"><span style="color:var(--tx2)">★★★★ İyi performans</span><span style="color:#27ae60;font-weight:600">+1</span></div>
                    <div style="display:flex;justify-content:space-between"><span style="color:var(--tx2)">★★★★★ Mükemmel</span><span style="color:#2ecc71;font-weight:600">+2</span></div>
                </div>
            </div>
        </div>
        ...
        <div style="background:linear-gradient(135deg,rgba(39,174,96,.08),rgba(46,204,113,.04));border:1px solid rgba(39,174,96,.2);border-radius:12px;padding:16px;margin-bottom:16px">
            ...
            <div style="background:rgba(231,76,60,.1);border-radius:8px;padding:10px;display:flex;align-items:center;gap:10px">
                ...
                <span style="font-size:.75rem;color:#c0392b"><strong>Dikkat:</strong> ${s.warning || '-5 puana düşersen prim hakkını kaybedersin!'}</span>
            </div>
        </div>
        
        <!-- MOTİVASYON -->
        <div style="background:var(--bg2);border-radius:10px;padding:16px">
            <div style="font-size:.85rem;font-weight:600;color:var(--tx);margin-bottom:10px">Primi Nasıl Kazanırsın?</div>
...
// Prim Ayarlarını Düzenle
function editPrimSettings() {
    const s = primSettingsCache || {};
    const tiersJson = JSON.stringify(s.tiers || [], null, 2);
    
    const body = `
        <div style="display:flex;flex-direction:column;gap:16px">
            <div class="form-group">
                <label class="form-label">Dönem (Ay/Yıl)</label>
                <input type="text" id="primMonth" class="form-input" value="${s.month || ''}" placeholder="Ocak 2026">
            </div>
            <div class="form-group">
                <label class="form-label">Prim Kademeleri (JSON)</label>
                <textarea id="primTiers" class="form-input" rows="6" style="font-family:monospace;font-size:.8rem">${tiersJson}</textarea>
                <div style="font-size:.7rem;color:var(--tx3);margin-top:4px">Format: [{"minCiro": "55-65K", "prim": "500 TL"}, ...]</div>
            </div>
            <div class="form-group">
                <label class="form-label">Uyarı Mesajı</label>
                <input type="text" id="primWarning" class="form-input" value="${s.warning || ''}">
            </div>
            <div class="form-group">
                <label class="form-label">Motivasyon Yazısı 1</label>
                <textarea id="primMotivation" class="form-input" rows="3">${s.motivation || ''}</textarea>
            </div>
            <div class="form-group">
                <label class="form-label">Motivasyon Yazısı 2</label>
                <textarea id="primMotivation2" class="form-input" rows="3">${s.motivation2 || ''}</textarea>
            </div>
            <div class="form-group">
                <label class="form-label">Slogan</label>
                <input type="text" id="primQuote" class="form-input" value="${s.quote || ''}">
            </div>
        </div>
    `;
    ...
    const footer = `
        <button class="btn btn-ghost" onclick="closeModal()">İptal</button>
        <button class="btn btn-primary" onclick="savePrimSettings()">Kaydet</button>
    `;
    
    showModal('Prim Sistemi Düzenle', body, footer);
}

// Prim Ayarlarını Kaydet
async function savePrimSettings() {
    try {
        let tiers = [];
        try {
            tiers = JSON.parse(document.getElementById('primTiers').value);
        } catch (e) {
            toast('Prim kademeleri JSON formatı hatalı', 'error');
            return;
        }
        ...
        const settings = {
            ...
            tiers: tiers,
            warning: document.getElementById('primWarning').value,
            motivation: document.getElementById('primMotivation').value,
            motivation2: document.getElementById('primMotivation2').value,
            quote: document.getElementById('primQuote').value,
            updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
        ...
        closeModal();
        toast('Prim ayarları kaydedildi', 'success');
        loadPrimSettings();
    } catch (e) {
        console.error('Prim ayarları kaydetme hatası:', e);
...
async function loadPuantajRules() {
    try {
        ...
        console.error('Kural yükleme hatası:', e);
    }
}
...
async function loadPuantaj() {
    ...
    container.innerHTML = '<div style="text-align:center;padding:40px;color:var(--tx2)">Yükleniyor...</div>';
    
    // Prim sistemi ve kuralları yükle
    loadPrimSettings();
    ...
    try {
        ...
        if (personelSnapshot.empty) {
            container.innerHTML = '<div style="text-align:center;padding:60px;color:var(--tx2)"><div style="font-size:3rem;margin-bottom:16px;opacity:.5">👥</div>Bu şubede kayıtlı personel yok.</div>';
            return;
        }
        ...
        // === OTOMATİK PUANLARI DA ÇEK (Check Sisteminden) ===
        ...
        // Seed formatı - Türkçe karakterler _ olur (eski veriler için)
        ...
        try {
            ...
            console.log(`[Puantaj] ${sube} için ${totalAutoRecords} otomatik kayıt bulundu`);
            console.log('[Puantaj] autoNotesData keys:', Object.keys(autoNotesData));
        } catch (e) {
            console.error('Otomatik puanlar yüklenemedi:', e);
        }
        ...
        personelSnapshot.forEach(doc => {
            ...
            // Otomatik puanları bul - ÇOKLU FORMAT EŞLEŞTİRME
            const firstName = p.name?.split(' ')[0] || '';
            const lastName = p.name?.split(' ').slice(1).join(' ') || '';
            
            // Tüm olası key formatlarını dene (öncelik sırasına göre)
            const possibleKeys = [
                ...
                normalizeNameSeed(p.name),                 // 5. ESKİ: Seed format (b__ra_s____t)
                normalizeNameSeed(firstName),              // 6. ESKİ: Seed ilk isim (b__ra)
                p.name,                                    // 7. Tam isim: "BÜŞRA SÖĞÜT"
                p.name?.toLowerCase(),                     // 8. Küçük harf: "büşra söğüt"
                firstName,                                 // 9. İlk isim: "BÜŞRA"
                firstName?.toLowerCase(),                  // 10. Küçük: "büşra"
            ].filter(k => k);
            ...
            for (const key of possibleKeys) {
                if (autoNotesData[key] && autoNotesData[key].length > 0) {
                    ...
                    console.log(`[Puantaj] ${p.name} için eşleşme bulundu: key="${key}", ${otomatikPuanlar.length} kayıt`);
                    break;
                }
            }
            ...
            // Otomatik puanlar (Check sisteminden)
            ...
            // Toplam puan = Manuel + Otomatik + Reset Bonus
            const toplamPuan = manuelPuan + otomatikPuan + primResetBonus;
            ...
            personelList.push({ 
                ...
                name: p.name,
                codeName: p.codeName,
                displayName: getPersonelDisplayName(p),  // codeName varsa onu, yoksa "Büşra S."
                tc: p.tc, 
                position: p.position, 
                toplamPuan,
                manuelPuan,
        ...
        // === AYLIK LİDERLİK TABLOSU ===
        // Puanlara göre sırala ve yıldız hesapla
        ...
        // === İSTATİSTİKSEL YILDIZ HESAPLAMA (Z-Score) ===
        // Ortalama hesapla
        const puanlar = personelList.map(p => p.toplamPuan);
        const n = puanlar.length;
        const ortalama = n > 0 ? puanlar.reduce((a, b) => a + b, 0) / n : 0;
        
        // Standart sapma hesapla
        const varyans = n > 0 ? puanlar.reduce((acc, p) => acc + Math.pow(p - ortalama, 2), 0) / n : 0;
        const stdSapma = Math.sqrt(varyans) || 1; // 0'a bölmeyi önle
        
        // Z-score'a göre yıldız ata (Normal dağılım)
        // z >= 1.0  → 5 yıldız (üst ~16%)
        // z >= 0.25 → 4 yıldız (üst-orta ~40%)
        // z >= -0.25 → 3 yıldız (orta ~20%)
        // z >= -1.0 → 2 yıldız (alt-orta ~40%)
        // z < -1.0  → 1 yıldız (alt ~16%)
        personelList.forEach(p => {
            const zScore = (p.toplamPuan - ortalama) / stdSapma;
            p.zScore = zScore;
            
            if (zScore >= 1.0) p.performansYildiz = 5;
            else if (zScore >= 0.25) p.performansYildiz = 4;
            else if (zScore >= -0.25) p.performansYildiz = 3;
            else if (zScore >= -1.0) p.performansYildiz = 2;
            else p.performansYildiz = 1;
        });
        
        // İstatistik cache'e kaydet
        window.puantajStats = { ortalama: ortalama.toFixed(1), stdSapma: stdSapma.toFixed(1), n };
        
        // Personel Kartları - Düzenli Grid, Marka Renkleri
        // Her iki şubede de Moonberry teal rengi
        const brandColor = '#008c95';
        const brandBg = 'rgba(0,140,149,.08)';
        
        // === AYLIK ŞAMPIYONLAR ===
        if (personelList.length > 1 && !isBarista) {
            const topPerformer = sortedByPuan[0];
            const mostImproved = sortedByPuan.find(p => p.pozitifOlay > 0);
            const needsAttention = sortedByNegatif[0];
                    ...
                    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px">
                        ${topPerformer && topPerformer.toplamPuan > 0 ? `
                            <div style="background:linear-gradient(135deg,rgba(255,215,0,.15),rgba(255,215,0,.05));border:1px solid rgba(255,215,0,.3);border-radius:12px;padding:14px;text-align:center">
                                ...
                                <div style="font-size:.7rem;color:var(--tx2);margin-bottom:4px">En Yüksek Puan</div>
                        ...
                        ` : ''}
                        ${mostImproved && mostImproved.pozitifOlay >= 2 ? `
                            <div style="background:linear-gradient(135deg,rgba(39,174,96,.15),rgba(39,174,96,.05));border:1px solid rgba(39,174,96,.3);border-radius:12px;padding:14px;text-align:center">
                                <div style="font-size:1.5rem;margin-bottom:4px">⭐</div>
                                <div style="font-size:.7rem;color:var(--tx2);margin-bottom:4px">En Çok Başarı</div>
                                <div style="font-weight:700;color:var(--tx);font-size:.9rem">${mostImproved.displayName}</div>
                                <div style="font-size:.8rem;color:#27ae60;font-weight:600">${mostImproved.pozitifOlay} başarı</div>
                            </div>
                        ` : ''}
        ...
        personelList.forEach(p => {
            ...
            const cardBorder = isDanger ? '#e74c3c' : (isRisk ? '#f39c12' : brandColor);
            
            // Yıldız gösterimi - BÜYÜTÜLDÜ
            const yildizlar = '★'.repeat(p.performansYildiz || 3) + '☆'.repeat(5 - (p.performansYildiz || 3));
            const yildizColor = p.performansYildiz >= 4 ? '#f1c40f' : (p.performansYildiz >= 3 ? '#95a5a6' : '#e74c3c');
            
            html += `
                <div style="background:linear-gradient(135deg,${brandBg},var(--cardBg));border:2px solid ${cardBorder}30;border-left:4px solid ${cardBorder};border-radius:12px;padding:16px;transition:all .2s" onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='0 4px 12px rgba(0,0,0,.08)'" onmouseout="this.style.transform='none';this.style.boxShadow='none'">
                    <!-- Header - TIKLANABİLİR -->
                    <div onclick="showPersonelPuanDetay('${p.id}', '${p.name.replace(/'/g, "\\'")}')" style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;cursor:pointer;padding:8px;margin:-8px;border-radius:8px;transition:background .15s" onmouseover="this.style.background='var(--bg3)'" onmouseout="this.style.background='transparent'">
                        <div style="display:flex;align-items:center;gap:10px">
                            <div style="width:36px;height:36px;background:linear-gradient(135deg,${brandColor},${brandColor}99);border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:.9rem;color:#fff;font-weight:600">
                                ${p.name.charAt(0).toUpperCase()}
                            </div>
                            <div>
                                <div style="display:flex;align-items:center;gap:8px">
                                    <span style="font-weight:600;font-size:.95rem;color:var(--tx)">${p.displayName}</span>
                                    <span style="font-size:.9rem;color:${yildizColor};letter-spacing:1px">${yildizlar}</span>
                                </div>
                                <div style="font-size:.75rem;color:var(--tx3)">${p.position || 'Barista'}</div>
                            </div>
                        </div>
                        <div style="text-align:right">
                            <div style="font-size:1.4rem;font-weight:800;color:${puanColor};line-height:1">${puanSign}${p.toplamPuan}</div>
                            <div style="font-size:.65rem;color:var(--tx3);text-transform:uppercase">puan</div>
                        </div>
                    </div>
                    ...
                    <div style="display:flex;gap:8px;margin-bottom:12px;flex-wrap:wrap">
                        ...
                        ${p.negatifOlay === 0 && p.pozitifOlay === 0 ? `<span style="font-size:.65rem;padding:3px 6px;background:${brandBg};color:var(--tx3);border-radius:4px">Henüz kayıt yok</span>` : ''}
                    </div>
                    
                    ${isDanger ? `
                        <div style="background:rgba(231,76,60,.12);border-radius:8px;padding:8px 12px;margin-bottom:12px;display:flex;align-items:center;justify-content:space-between;gap:8px">
                            <div style="display:flex;align-items:center;gap:8px">
                                <svg width="14" height="14" fill="none" stroke="#e74c3c" stroke-width="2" viewBox="0 0 24 24"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
                                <span style="font-size:.75rem;color:#c0392b;font-weight:600">Prim eşiği aşıldı</span>
                            </div>
                            <button onclick="event.stopPropagation();resetPersonelPrim('${p.id}', '${p.name}')" style="padding:4px 10px;background:#e74c3c;color:#fff;border:none;border-radius:6px;font-size:.7rem;font-weight:600;cursor:pointer;transition:all .15s" onmouseover="this.style.opacity='0.8'" onmouseout="this.style.opacity='1'">
                                Primi Sıfırla
                            </button>
                        </div>
                    ` : ''}
        ...
        html += `<div style="font-weight:600;margin-bottom:16px;color:var(--tx);font-size:1rem;display:flex;align-items:center;gap:8px"><svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2"/><rect x="9" y="3" width="6" height="4" rx="1"/></svg> Yönetici Değerlendirmeleri</div>`;
        ...
        personelList.forEach(p => {
            if (p.puanlar && p.puanlar.length > 0) {
                ...
                p.puanlar.forEach(puan => {
                    ...
                    // Yönetici adını göster
                    const yonetici = puan.createdBy || 'Bilinmiyor';
                    const yoneticiKisa = yonetici.includes('@') ? yonetici.split('@')[0] : yonetici;
                    ...
                    html += `
                        <div style="display:flex;justify-content:space-between;align-items:center;padding:12px;background:${bgColor};border:1px solid ${borderColor};border-radius:10px;margin-bottom:8px">
                            <div style="display:flex;align-items:center;gap:12px">
                                <span style="font-size:.85rem;color:#f1c40f">${stars}</span>
                                <div>
                                    <div style="color:var(--tx);font-size:.85rem;font-weight:500">${puan.note || '-'}</div>
                                    <div style="color:var(--tx3);font-size:.7rem">${tarih} · ${yoneticiKisa}</div>
                                </div>
                            </div>
                            <div style="display:flex;align-items:center;gap:10px">
        ...
        if (!hasRecords) {
            html += `<div style="text-align:center;padding:40px;color:var(--tx3)">Bu ay henüz kayıt eklenmemiş</div>`;
        }
        html += `</div>`;
        
        // Otomatik Check Puanları bölümü
        const autoNotesHtml = await loadAutoCheckNotes(sube, ay, yil, personelList);
        html += autoNotesHtml;
        
        container.innerHTML = html;
    } catch (error) {
        console.error('Puantaj yükleme hatası:', error);
        container.innerHTML = '<div style="text-align:center;padding:40px;color:var(--red)">Yükleme hatası!</div>';
    }
}

// Otomatik Check puanlarını yükle - Günlük Görünüm
    ...
    try {
        ...
        notesSnapshot.forEach(doc => {
            ...
            if (note.date >= startDate && note.date <= endDate) {
                filteredDocs.push({ id: doc.id, ...note });
            }
        });
        ...
        // Cache'e kaydet (detay modal için)
                ...
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;padding-bottom:12px;border-bottom:1px solid var(--bg4)">
                    <div style="display:flex;align-items:center;gap:10px">
                        ...
                        <div>
                            ...
                            <div style="font-size:.75rem;color:var(--tx3)">${filteredDocs.length} kayıt · ${Object.keys(notesByPerson).length} personel</div>
                        </div>
                    </div>
            ...
            html += `
                <div onclick="showPersonelPuanDetay('${personelId}', '${personName.replace(/'/g, "\\'")}')" style="background:linear-gradient(135deg,var(--bg3),var(--bg2));border:1px solid var(--bg4);border-radius:10px;padding:12px;cursor:pointer;transition:all .15s" onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='0 4px 12px rgba(0,0,0,.08)'" onmouseout="this.style.transform='none';this.style.boxShadow='none'">
                    ...
                    <div style="display:flex;gap:6px;flex-wrap:wrap">
                        ...
                        ${late > 0 ? `<span style="font-size:.65rem;color:#f39c12;background:rgba(243,156,18,.1);padding:2px 6px;border-radius:4px">${late} geç</span>` : ''}
        ...
        sortedDates.forEach(date => {
            ...
            notes.forEach(note => {
                const person = personelList.find(p => p.id === note.personelId);
                ...
                let aciklama = '';
                const checkSaat = note.checkTime || '';
                const checkTip = note.checkType || note.note || 'Check';
                
                if (note.status === 'missed') {
                    aciklama = `${checkSaat ? checkSaat + ' ' : ''}${checkTip} tamamlanmadı`;
                } else if (note.status === 'late') {
                    aciklama = `${checkSaat ? checkSaat + ' ' : ''}${checkTip} geç yapıldı`;
                } else {
                    aciklama = `${checkSaat ? checkSaat + ' ' : ''}${checkTip} vaktinde tamamlandı`;
                }
                ...
                html += `
                    <div style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:${statusBg};border-radius:8px;margin-bottom:6px">
                        ...
                        <div style="flex:1;min-width:0">
                            <div style="font-size:.8rem;font-weight:600;color:var(--tx)">${personName}</div>
                            <div style="font-size:.75rem;color:var(--tx2)">${aciklama}</div>
                        </div>
                        ${canManagePoints ? `<button onclick="event.stopPropagation();deleteAutoNote('${note.id}')" style="background:rgba(231,76,60,.15);border:none;color:#e74c3c;cursor:pointer;padding:6px 10px;border-radius:6px;font-size:.7rem;font-weight:500;transition:all .15s" onmouseover="this.style.background='rgba(231,76,60,.25)'" onmouseout="this.style.background='rgba(231,76,60,.15)'">Sil</button>` : ''}
                    </div>
                `;
            });
        ...
        console.error('Otomatik notlar yüklenemedi:', e);
    }
...
function showPersonelPuanDetay(personelId, personName) {
    // Önce puantajPersonelCache'den veri al (otomatik puanlar dahil)
    const personelData = window.puantajPersonelCache?.find(p => p.id === personelId);
    
    if (!personelData) {
        toast('Personel verisi bulunamadı', 'error');
        return;
    }
    
    const notes = personelData.otomatikPuanlar || [];
    const manuelPuanlar = personelData.puanlar || [];
    
    const canManage = ['yonetici', 'bolge_muduru', 'magaza_muduru'].includes(currentUser?.role);
    
    // İstatistikler - doğrudan personelData'dan al
    ...
    const yildizlar = '★'.repeat(personelData.performansYildiz || 3) + '☆'.repeat(5 - (personelData.performansYildiz || 3));
    const yildizColor = personelData.performansYildiz >= 4 ? '#f1c40f' : (personelData.performansYildiz >= 3 ? '#95a5a6' : '#e74c3c');
        ...
        <div style="max-height:70vh;overflow-y:auto">
            ...
            <div style="background:linear-gradient(135deg,rgba(0,140,149,.1),rgba(255,134,116,.05));border-radius:12px;padding:16px;margin-bottom:16px">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
                    <div>
                        <div style="font-size:1.1rem;font-weight:700;color:var(--tx)">${formatDisplayName(personName)}</div>
                        <div style="font-size:1.1rem;color:${yildizColor};letter-spacing:2px">${yildizlar}</div>
                    </div>
                    <div style="text-align:right">
            ...
            <div style="margin-bottom:16px">
                <div style="font-size:.85rem;font-weight:600;color:var(--tx);margin-bottom:10px;display:flex;align-items:center;gap:6px">
                    ...
                    Check Sisteminden (${notes.length})
                </div>
    `;
    
    // Tarihe göre sırala ve göster
    ...
    sortedNotes.forEach(note => {
        ...
        const tarih = `${dateObj.getDate()} ${ayAdi[dateObj.getMonth() + 1]}`;
        const noteColor = note.points > 0 ? '#27ae60' : (note.points < 0 ? '#e74c3c' : 'var(--tx3)');
        const statusBg = note.status === 'onTime' ? 'rgba(39,174,96,.08)' : (note.status === 'late' ? 'rgba(243,156,18,.08)' : 'rgba(231,76,60,.08)');
        const statusText = note.status === 'onTime' ? 'Vaktinde' : (note.status === 'late' ? 'Geç' : 'Eksik');
        const checkSaat = note.checkTime || '';
        const checkTip = note.checkType || 'Platform Check';
        
        body += `
            <div style="display:flex;align-items:center;gap:10px;padding:10px;background:${statusBg};border-radius:8px;margin-bottom:6px">
                <div style="min-width:60px;font-size:.7rem;color:var(--tx3)">${tarih}<br>${gun}</div>
                <div style="flex:1">
                    <div style="font-size:.8rem;color:var(--tx);font-weight:500">${checkTip} ${checkSaat ? '· ' + checkSaat : ''}</div>
                    <div style="font-size:.7rem;color:var(--tx2)">${statusText}</div>
                </div>
                <div style="font-weight:700;color:${noteColor};font-size:.85rem">${note.points > 0 ? '+' : ''}${note.points}</div>
                ${canManage ? `<button onclick="deleteAutoNote('${note.id}');closeModal();setTimeout(loadPuantaj,500)" style="background:rgba(231,76,60,.1);border:none;color:#e74c3c;cursor:pointer;padding:4px 8px;border-radius:4px;font-size:.65rem">Sil</button>` : ''}
            </div>
        `;
    });
    ...
    if (manuelPuanlar.length > 0) {
        body += `
            <div>
                <div style="font-size:.85rem;font-weight:600;color:var(--tx);margin-bottom:10px;display:flex;align-items:center;gap:6px">
                    <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
                    Manuel Değerlendirmeler (${manuelPuanlar.length})
                </div>
        `;
        
        manuelPuanlar.forEach(puan => {
            if (puan.type === 'prim_reset') return;
            const tarih = puan.eventDate || puan.createdAt?.toDate?.()?.toLocaleDateString('tr-TR') || '-';
            const yildiz = puan.score || 3;
            const puanEtki = yildiz === 1 ? -3 : (yildiz === 2 ? -2 : (yildiz === 3 ? -1 : (yildiz === 5 ? 2 : 1)));
            const puanColor = puanEtki < 0 ? '#e74c3c' : '#27ae60';
            
            body += `
                <div style="display:flex;align-items:center;gap:10px;padding:10px;background:${puanEtki < 0 ? 'rgba(231,76,60,.08)' : 'rgba(39,174,96,.08)'};border-radius:8px;margin-bottom:6px">
                    <div style="min-width:60px;font-size:.75rem;color:var(--tx3)">${tarih}</div>
                    <div style="flex:1">
                        <div style="font-size:.8rem;color:var(--tx)">${puan.note || '-'}</div>
                    </div>
                    <div style="font-weight:700;color:${puanColor};font-size:.85rem">${puanEtki > 0 ? '+' : ''}${puanEtki}</div>
                    ${canManage ? `<button onclick="deletePuantajEntry('${puan.id}');closeModal();setTimeout(loadPuantaj,500)" style="background:rgba(231,76,60,.1);border:none;color:#e74c3c;cursor:pointer;padding:4px 8px;border-radius:4px;font-size:.65rem">Sil</button>` : ''}
                </div>
            `;
        });
    ...
    if (canManage) {
        footer = `
            ...
            <button class="btn" style="background:#e74c3c;color:#fff" onclick="closeModal();resetPersonelPrim('${personelId}','${personName.replace(/'/g, "\\'")}')">Sıfırla</button>
            <button class="btn btn-primary" onclick="closeModal();showPuantajModal('${personelId}','${personName.replace(/'/g, "\\'")}')">Değerlendir</button>
        `;
    }
    
    showModal(`${formatDisplayName(personName)} - Puan Detayı`, body, footer);
}

// Otomatik puantaj notu sil
async function deleteAutoNote(noteId) {
    // Yetki kontrolü
    const canManagePoints = ['yonetici', 'bolge_muduru', 'magaza_muduru'].includes(currentUser?.role);
    if (!canManagePoints) {
    ...
    confirmModal('Puantaj Kaydını Sil', 'Bu otomatik puan kaydını silmek istediğinize emin misiniz?', async () => {
        try {
            await db.collection('puantajNotes').doc(noteId).delete();
            toast('Puan kaydı silindi', 'success');
            loadPuantaj(); // Sayfayı yenile
        } catch (e) {
...
// Detaylı kural açıklamaları modalı
function showPuantajRulesDetail() {
    showModal('Performans Kuralları', `
        <div style="max-height:60vh;overflow-y:auto">
            <div style="margin-bottom:20px">
                <div style="font-weight:600;color:var(--tx);margin-bottom:10px;font-size:.95rem">Checklist Puanları</div>
                <div style="background:var(--bg3);border-radius:10px;padding:14px">
                    <div style="display:grid;gap:8px;font-size:.85rem">
                        ...
                        <div style="display:flex;justify-content:space-between;color:var(--tx)">
                            <span>Geç tamamlanan check (pencere sonrası)</span>
                            <span style="color:#f39c12;font-weight:600">-2 puan</span>
                        </div>
                        <div style="display:flex;justify-content:space-between;color:var(--tx)">
                            <span>Yapılmayan/doldurulmayan check</span>
            ...
            <div style="margin-bottom:20px">
                ...
                <div style="background:var(--bg3);border-radius:10px;padding:14px">
                    ...
                    <div style="display:flex;justify-content:space-between;color:var(--tx);font-size:.85rem">
                        <span>Her yapılmayan saat için</span>
            ...
            <div style="margin-bottom:20px">
                <div style="font-weight:600;color:var(--tx);margin-bottom:10px;font-size:.95rem">Temizlik Çizelgesi</div>
                <div style="background:var(--bg3);border-radius:10px;padding:14px;font-size:.85rem">
                    <div style="margin-bottom:8px;color:var(--tx)"><strong>Açılışçı:</strong> 10:00-11:00 shift başlangıcı → 15:00'a kadar tamamlanmalı</div>
                    <div style="margin-bottom:8px;color:var(--tx)"><strong>Aracı:</strong> 11:00-16:00 shift başlangıcı → 19:00'a kadar tamamlanmalı</div>
                    <div style="color:var(--tx)"><strong>Kapanışçı:</strong> 16:00+ shift başlangıcı → 02:00'a kadar tamamlanmalı</div>
                </div>
            </div>
            
            <div style="margin-bottom:20px">
                <div style="font-weight:600;color:var(--tx);margin-bottom:10px;font-size:.95rem">Manuel Değerlendirme (Yönetici)</div>
                <div style="background:var(--bg3);border-radius:10px;padding:14px">
                    <div style="display:grid;gap:6px;font-size:.85rem">
                        <div style="display:flex;justify-content:space-between;color:var(--tx)">
                            <span>⭐ Ciddi ihlal</span>
                            <span style="color:#e74c3c;font-weight:600">-3 puan</span>
                        </div>
                        <div style="display:flex;justify-content:space-between;color:var(--tx)">
                            <span>⭐⭐ Orta ihlal</span>
            ...
            <div style="background:rgba(231,76,60,.1);border:1px solid rgba(231,76,60,.2);border-radius:10px;padding:14px">
                <div style="font-weight:600;color:#e74c3c;margin-bottom:6px;font-size:.9rem">Prim Eşiği</div>
                <div style="font-size:.85rem;color:var(--tx)">Toplam puan <strong>-5</strong> veya altına düşerse, o ay prim hakkı kaybedilir.</div>
                <div style="font-size:.8rem;color:var(--tx3);margin-top:6px">Her ay başı puanlar sıfırlanır.</div>
            </div>
        </div>
    `, '<button class="btn btn-primary" onclick="closeModal()">Anladım</button>');
}

function showPuantajModal(personelId, name, type = 'all') {
    const title = `${name} - Manuel Değerlendirme`;
    
    // Tüm kurallar (cache'ten)
    const kurallar = puantajRulesCache || [];
    
    let kuralOptions = '<option value="">-- Kural Seçin (Opsiyonel) --</option>';
    kurallar.forEach(k => {
        kuralOptions += `<option value="${k.id}" data-title="${k.title}">${k.title} (${k.effect || ''})</option>`;
    });
    kuralOptions += '<option value="other">Diğer (Manuel Giriş)</option>';
    
    // Tüm yıldız seçenekleri - 5'li grid
    const starOptions = `
        <div class="star-option" onclick="selectPuantajStar(1, this)" style="padding:12px 6px;background:rgba(192,57,43,.12);border:2px solid transparent;border-radius:12px;cursor:pointer;text-align:center;transition:all .2s">
            <div style="font-size:1.1rem;margin-bottom:4px;color:#c0392b">★</div>
            <div style="font-size:.75rem;color:#c0392b;font-weight:700">Ciddi İhlal</div>
            <div style="font-size:.8rem;color:#e74c3c;font-weight:600;margin-top:2px">-3</div>
        </div>
        <div class="star-option" onclick="selectPuantajStar(2, this)" style="padding:12px 6px;background:rgba(211,84,0,.12);border:2px solid transparent;border-radius:12px;cursor:pointer;text-align:center;transition:all .2s">
            <div style="font-size:1.1rem;margin-bottom:4px;color:#d35400">★★</div>
            <div style="font-size:.75rem;color:#d35400;font-weight:700">Orta İhlal</div>
            <div style="font-size:.8rem;color:#e67e22;font-weight:600;margin-top:2px">-2</div>
        </div>
        <div class="star-option" onclick="selectPuantajStar(3, this)" style="padding:12px 6px;background:rgba(241,196,15,.12);border:2px solid transparent;border-radius:12px;cursor:pointer;text-align:center;transition:all .2s">
            <div style="font-size:1.1rem;margin-bottom:4px;color:#f39c12">★★★</div>
            <div style="font-size:.75rem;color:#f39c12;font-weight:700">Hafif Uyarı</div>
            <div style="font-size:.8rem;color:#f1c40f;font-weight:600;margin-top:2px">-1</div>
        </div>
        <div class="star-option" onclick="selectPuantajStar(4, this)" style="padding:12px 6px;background:rgba(39,174,96,.12);border:2px solid transparent;border-radius:12px;cursor:pointer;text-align:center;transition:all .2s">
            <div style="font-size:1.1rem;margin-bottom:4px;color:#27ae60">★★★★</div>
            <div style="font-size:.75rem;color:#27ae60;font-weight:700">İyi Performans</div>
            <div style="font-size:.8rem;color:#2ecc71;font-weight:600;margin-top:2px">+1</div>
        </div>
        <div class="star-option" onclick="selectPuantajStar(5, this)" style="padding:12px 6px;background:rgba(46,204,113,.12);border:2px solid transparent;border-radius:12px;cursor:pointer;text-align:center;transition:all .2s">
            <div style="font-size:1.1rem;margin-bottom:4px;color:#2ecc71">★★★★★</div>
            <div style="font-size:.75rem;color:#2ecc71;font-weight:700">Mükemmel</div>
            <div style="font-size:.8rem;color:#27ae60;font-weight:600;margin-top:2px">+2</div>
        </div>
    `;
    ...
    const body = `
        <div style="padding:8px 0">
            <div style="text-align:center;margin-bottom:16px">
                <svg width="32" height="32" fill="none" stroke="#008c95" stroke-width="2" viewBox="0 0 24 24"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
                <div style="color:#008c95;font-weight:700;font-size:1rem;margin-top:6px">Manuel Değerlendirme</div>
            </div>
            ...
            <div style="margin-bottom:16px">
                <label style="display:block;font-size:.85rem;color:var(--tx2);margin-bottom:8px;font-weight:500">Kural Seç</label>
            ...
            <div style="margin-bottom:16px">
                <label style="display:block;font-size:.85rem;color:var(--tx2);margin-bottom:8px;font-weight:500">Açıklama <span style="color:#e74c3c">*</span></label>
                <textarea id="puantajNote" class="form-input" rows="3" placeholder="Değerlendirme açıklaması yazın..." style="background:var(--bg3);color:var(--tx);border-color:var(--bg4);resize:vertical;min-height:80px;font-family:inherit"></textarea>
            </div>
            
            <div style="margin-bottom:16px">
                <label style="display:block;font-size:.85rem;color:var(--tx2);margin-bottom:8px;font-weight:500">Olay Tarihi</label>
...
// Kural seçildiğinde açıklamaya otomatik yaz
        ...
        if (currentUser?.role === 'barista') {
            document.getElementById('puantajArsivContainer').innerHTML = `
                <div style="text-align:center;padding:60px 20px;grid-column:1/-1">
                    ...
                    <div style="font-size:1.1rem;font-weight:600;color:var(--tx);margin-bottom:8px">Arşiv Erişimi Kısıtlı</div>
                    <div style="font-size:.85rem;color:var(--tx3);max-width:300px;margin:0 auto">Bu bölüm sadece yöneticiler tarafından görüntülenebilir.</div>
                </div>
            `;
...
async function savePuantajFromModal() {
    ...
    if (score === 0) {
        toast('Lütfen yıldız seviyesi seçin', 'error');
        return;
    }
    
    if (!note) {
        toast('Lütfen açıklama girin', 'error');
        return;
    }
...
// Prim Sıfırlama Fonksiyonu
async function resetPersonelPrim(personelId, name) {
    // Yetki kontrolü
    const canReset = ['yonetici', 'bolge_muduru', 'magaza_muduru'].includes(currentUser?.role);
    if (!canReset) {
        toast('Bu işlem için yetkiniz yok', 'error');
        return;
    }
    ...
    const telafiPuani = mevcutPuan < 0 ? Math.abs(mevcutPuan) : 0;
    ...
    confirmModal(
        ...
        `<div style="text-align:center">
            ...
            <div style="font-size:.85rem;color:var(--tx2);margin-bottom:16px">${ay}/${yil} Dönemi</div>
            <div style="background:var(--bg3);border-radius:10px;padding:12px;margin-bottom:16px">
                ...
                <div style="display:flex;justify-content:space-between;margin-bottom:8px">
                    <span style="color:var(--tx2)">Telafi Puanı:</span>
                    <span style="font-weight:700;color:#27ae60">+${telafiPuani}</span>
                </div>
            ...
            <div style="font-size:.8rem;color:var(--tx3)">Tüm kayıtlar geçmişte saklanır.</div>
        </div>`,
        async () => {
            try {
                // Telafi puanı ekle (mevcut negatifi sıfırla)
                await db.collection('puantaj').add({
                    personelId,
                    personelName: name,
                    period,
                    ...
                    score: 5,
                    type: 'prim_reset',
                    resetBonus: telafiPuani,
                    note: `Prim sıfırlama (${mevcutPuan} → 0) - ${currentUser?.name || 'Yönetici'}`,
                    eventDate: new Date().toISOString().split('T')[0],
                    createdAt: firebase.firestore.FieldValue.serverTimestamp(),
                    createdBy: currentUser?.email || currentUser?.username || 'admin'
                });
                
                toast(`${name} puanı sıfırlandı`, 'success');
                loadPuantaj();
            } catch (error) {
                console.error('Prim sıfırlama hatası:', error);
                toast('Sıfırlama hatası: ' + error.message, 'error');
            }
        },
        'warning'
    );
...
async function loadPuantajAnaliz() {
    ...
    container.innerHTML = '<div style="text-align:center;padding:40px;color:var(--tx2)">Analiz yükleniyor...</div>';
    ...
    try {
        ...
        snapshot.forEach(doc => {
            ...
            // Yeni puan sistemi: 1★=-3, 2★=-2, 3★=-1, 4★=+1, 5★=+2
            if (yildiz === 1) {
        ...
        html += `
            <div style="background:linear-gradient(135deg,rgba(39,174,96,.15),rgba(46,204,113,.1));border:1px solid rgba(39,174,96,.3);border-radius:16px;padding:20px;text-align:center">
                ...
                <div style="font-size:.75rem;color:var(--tx2);text-transform:uppercase;margin-bottom:4px">Ayın En İyisi</div>
        ...
        html += `
            <div style="background:linear-gradient(135deg,rgba(231,76,60,.15),rgba(192,57,43,.1));border:1px solid rgba(231,76,60,.3);border-radius:16px;padding:20px;text-align:center">
                ...
                <div style="font-size:.75rem;color:var(--tx2);text-transform:uppercase;margin-bottom:4px">Düşük Performans</div>
        ...
        html += `
            <div style="background:var(--cardBg);border:1px solid var(--cardBorder);border-radius:16px;padding:20px;text-align:center">
                ...
                <div style="font-size:.75rem;color:var(--tx2);text-transform:uppercase;margin-bottom:4px">Toplam Kayıt</div>
                    ...
                    <div style="display:flex;justify-content:space-between;align-items:center;padding:14px;background:${primStatus ? 'rgba(231,76,60,.08)' : 'var(--bg3)'};border-radius:10px;margin-bottom:8px">
                        ...
                        <div style="display:flex;align-items:center;gap:16px">
                            ...
                            <div style="min-width:60px;text-align:right">
                                ...
                                ${primStatus ? '<div style="font-size:.65rem;color:#e74c3c">PRİM YOK</div>' : ''}
                            </div>
                        </div>
                    </div>
                `;
...
// Arşiv yükleme - Gelişmiş versiyon
async function loadPuantajArsiv() {
    ...
    container.innerHTML = '<div style="text-align:center;padding:40px;color:var(--tx2);grid-column:1/-1">Arşiv yükleniyor...</div>';
    ...
    try {
        ...
        snapshot.forEach(doc => {
            ...
            let puan = 0;
            if (yildiz === 1) { puan = -3; toplamNegatif++; donemler[period].negatif++; }
            else if (yildiz === 2) { puan = -2; toplamNegatif++; donemler[period].negatif++; }
            else if (yildiz === 3) { puan = -1; toplamNegatif++; donemler[period].negatif++; }
            else if (yildiz === 4) { puan = 1; toplamPozitif++; donemler[period].pozitif++; }
            else if (yildiz === 5) { puan = 2; toplamPozitif++; donemler[period].pozitif++; }
        ...
        if (summaryContainer) {
            summaryContainer.innerHTML = `
                <div style="background:linear-gradient(135deg,rgba(0,140,149,.1),rgba(0,140,149,.05));border:1px solid rgba(0,140,149,.2);border-radius:12px;padding:16px;text-align:center">
                    ...
                    <div style="font-size:.75rem;color:var(--tx2)">Toplam Kayıt</div>
                </div>
                ...
                <div style="background:linear-gradient(135deg,rgba(243,156,18,.1),rgba(243,156,18,.05));border:1px solid rgba(243,156,18,.2);border-radius:12px;padding:16px;text-align:center">
                    ...
                    <div style="font-size:.75rem;color:var(--tx2)">Prim Kaybı</div>
                </div>
            `;
        }
        
        // Trend Chart (Aylık bar chart)
        if (trendChart) {
            const aylar = ['01','02','03','04','05','06','07','08','09','10','11','12'];
            const ayAdilar = ['Oca','Şub','Mar','Nis','May','Haz','Tem','Ağu','Eyl','Eki','Kas','Ara'];
            let maxKayit = 1;
            aylar.forEach(ay => {
                const period = `${yil}-${ay}`;
            ...
            aylar.forEach((ay, i) => {
                ...
                // Negatif (kırmızı) ve pozitif (yeşil) yığılı bar
                const negHeight = kayit > 0 ? (negatif / kayit) * height : 0;
                const posHeight = height - negHeight;
                
                if (posHeight > 0) {
                    barHtml += `<rect x="${x}" y="${150 - height}" width="20" height="${posHeight}" fill="#27ae60" rx="3" style="cursor:pointer" onclick="showArsivDetail('${period}')"><title>${ayAdilar[i]}: ${kayit} kayıt</title></rect>`;
                }
                if (negHeight > 0) {
        ...
        if (pieChart) {
            ...
            if (total > 0) {
                ...
                [1,2,3,4,5].forEach((star, i) => {
                    ...
                    pieHtml += `<path d="M80,80 L${x1},${y1} A60,60 0 ${largeArc},1 ${x2},${y2} Z" fill="${colors[i]}" style="cursor:pointer" onclick="alert('${labels[i]}: ${value} kayıt (${Math.round(percentage*100)}%)')"><title>${labels[i]}: ${value}</title></path>`;
        ...
        if (sortedDonemler.length === 0) {
            html = '<div style="text-align:center;padding:60px;color:var(--tx3);grid-column:1/-1">Bu yıla ait arşivlenmiş dönem yok</div>';
        } else {
            sortedDonemler.forEach(([period, data]) => {
                ...
                const isMoonberry = sube.includes('Şişli') || sube.includes('Merkez');
                    ...
                    <div onclick="showArsivDetail('${period}')" style="background:var(--cardBg);border:1px solid var(--cardBorder);border-left:4px solid ${brandColor};border-radius:12px;padding:16px;transition:all .2s;cursor:pointer" onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='0 4px 12px rgba(0,0,0,.08)'" onmouseout="this.style.transform='none';this.style.boxShadow='none'">
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
                            <div>
                                ...
                                <div style="font-size:.75rem;color:var(--tx3)">${personelSayisi} personel · ${data.kayitSayisi} kayıt</div>
                            </div>
                            <div style="text-align:right">
        ...
        container.innerHTML = '<div style="text-align:center;padding:40px;color:var(--red);grid-column:1/-1">Yükleme hatası!</div>';
    }
}

// Arşiv Detay Gösterimi
function showArsivDetail(period) {
    ...
    // Personel listesi (puana göre sırala)
    ...
    let html = `
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
            <div>
                <div style="font-size:1.2rem;font-weight:700;color:var(--tx)">${ayAdi} ${yil} Detayı</div>
                <div style="font-size:.85rem;color:var(--tx2)">${personelList.length} personel · ${data.kayitSayisi} kayıt</div>
            </div>
        ...
        html += `
            <div style="background:${isDanger ? 'rgba(231,76,60,.05)' : 'var(--bg3)'};border:1px solid ${isDanger ? 'rgba(231,76,60,.2)' : 'var(--cardBorder)'};border-radius:10px;padding:12px">
                ...
                <div style="display:flex;align-items:center;justify-content:space-between">
                    ...
                    ${isDanger ? '<span style="font-size:.65rem;background:#e74c3c;color:#fff;padding:2px 6px;border-radius:4px">PRİM YOK</span>' : ''}
                </div>
            </div>
        `;
    });
    ...
    html += `
        <div style="margin-top:20px">
            <div style="font-weight:600;color:var(--tx);margin-bottom:12px">Son Kayıtlar</div>
    ...
    sonKayitlar.forEach(k => {
        ...
        html += `
            <div style="display:flex;align-items:center;gap:12px;padding:10px;background:var(--bg3);border-radius:8px;margin-bottom:6px">
                ...
                <div style="flex:1">
                    <div style="font-size:.85rem;color:var(--tx);font-weight:500">${k.note || 'Açıklama yok'}</div>
...
function showPuantajRules() {
    db.collection('puantajRules').get().then(snapshot => {
        ...
        html += '</div>';
        showModal('⭐ Puantaj Kuralları', html, '<button class="btn btn-ghost" onclick="closeModal()">Kapat</button>');
    });
}
...
async function loadUserList() {
    ...
    try {
        ...
        // Personel listesini al (isim eşleştirmesi için)
        ...
        const roleLabels = {
            ...
            'bolge_muduru': '🏢 Bölge Müdürü',
            'magaza_muduru': '🏪 Mağaza Müdürü',
            'magaza_mudur_yardimcisi': '🏪 Müdür Yardımcısı',
            'mudur_yardimcisi': '🏪 Müdür Yardımcısı',
            'barista': '☕ Barista'
        };
        
        if (usersSnapshot.empty) {
            container.innerHTML = '<p style="color:var(--tx2);text-align:center;padding:20px">Henüz kullanıcı yok.</p>';
            return;
        }
        
        let html = '<div style="font-size:.75rem;font-weight:600;color:var(--tx2);margin-bottom:12px;text-transform:uppercase">🔓 Sisteme Giriş Yapabilen Kullanıcılar</div>';
        ...
        usersSnapshot.forEach(doc => {
            ...
            const branchText = displayBranch && displayBranch !== 'all' ? displayBranch : 'Tüm Şubeler';
            ...
            html += `
                <div style="display:flex;justify-content:space-between;align-items:center;padding:12px;background:var(--bg3);border-radius:var(--radiusSm);margin-bottom:8px;border-left:3px solid ${isAdmin ? '#9b59b6' : '#27ae60'}">
                    <div>
                        ...
                        <div style="font-size:.8rem;color:var(--tx2)">@${userId} • ${branchText}</div>
                    </div>
                    <div style="display:flex;gap:8px">
                        ${!isAdmin ? `
                            <button class="btn btn-sm btn-ghost" onclick="editUserDirect('${userId}')" title="Düzenle">✏️</button>
                            <button class="btn btn-sm" style="background:var(--red);color:#fff" onclick="removeUserLogin('${userId}')" title="Sil">🗑️</button>
        ...
        // Personel listesinde olup users'da olmayan kişiler
        let pendingHtml = '';
        ...
        for (const p of personnelList) {
            ...
            if (!found) {
                hasPending = true;
                const roleLabel = roleLabels[p.role] || `📋 ${p.role || 'Bilinmiyor'}`;
                const branchText = p.branch && p.branch !== 'all' ? p.branch : 'Tüm Şubeler';
                ...
                pendingHtml += `
                    <div style="display:flex;justify-content:space-between;align-items:center;padding:12px;background:var(--bg3);border-radius:var(--radiusSm);margin-bottom:8px;opacity:.7">
                        <div>
                            ...
                            <div style="font-size:.8rem;color:var(--tx2)">${p.email} • ${branchText}</div>
                        </div>
                        <div>
                            <button class="btn btn-sm btn-primary" onclick="showLoginModal('${username}', '${p.name}', '${p.email}', '${p.role}', '${p.branch}')" title="Login Tanımla">🔑 Login Ekle</button>
                        </div>
                    </div>
                `;
            }
        ...
        if (hasPending) {
            html += '<div style="font-size:.75rem;font-weight:600;color:var(--tx2);margin:20px 0 12px;text-transform:uppercase">🔒 Giriş Tanımlı Değil (Personel Listesinden)</div>';
            html += pendingHtml;
        }
        ...
        container.innerHTML = '<p style="color:var(--red);text-align:center;padding:20px">Yükleme hatası!</p>';
    }
}

// Kullanıcı düzenleme (direkt users collection)
...
function showLoginModal(username, name, email, role, branch) {
    ...
    const body = `
        <div style="padding:8px 0">
            <div style="background:var(--bg3);border-radius:12px;padding:16px;margin-bottom:20px">
                ...
                <div style="font-size:.85rem;color:var(--tx2)">${email || 'E-posta tanımlı değil'}</div>
            </div>
            ...
            <div class="form-group">
                <label class="form-label">Kullanıcı Adı (E-posta'nın @ öncesi)</label>
                ...
                <div style="font-size:.75rem;color:var(--tx3);margin-top:4px">Giriş yaparken kullanılacak: kullanici@moonberrycoffee.com</div>
            </div>
            
            <div class="form-group">
                <label class="form-label">Şifre ${isEdit ? '(boş bırakılırsa değişmez)' : ''}</label>
    ...
    const footer = `
        <button class="btn btn-ghost" onclick="closeModal()">İptal</button>
        <button class="btn btn-primary" onclick="saveUserLogin(${isEdit})">💾 Kaydet</button>
    `;
...
async function saveUserLogin(isEdit) {
    ...
    if (!username) {
        toast('Kullanıcı adı gerekli', 'error');
        return;
    }
    ...
    try {
        const userData = {
            name: name,
            role: role || 'barista',
            branch: branch || 'Tuzla Port',
            updatedAt: firebase.firestore.FieldValue.serverTimestamp()
        };
        
        if (password) {
            userData.password = password; // Gerçek uygulamada hash'lenmeli
        }
        ...
        toast(isEdit ? 'Login güncellendi' : 'Login oluşturuldu', 'success');
        closeModal();
        loadUserList();
...
async function removeUserLogin(username) {
    if (!confirm('Bu kullanıcının giriş yetkisi kaldırılacak. Devam etmek istiyor musunuz?')) {
        return;
    }
    
    try {
        await db.collection('users').doc(username).delete();
        toast('Login kaldırıldı', 'success');
        loadUserList();
    } catch (error) {
        console.error('Silme hatası:', error);
...
function showNewUserModal() {
    const roleOptions = `
        ...
        <option value="magaza_mudur_yardimcisi">🏪 Müdür Yardımcısı</option>
        <option value="magaza_muduru">🏪 Mağaza Müdürü</option>
        <option value="bolge_muduru">🏢 Bölge Müdürü</option>
        <option value="yonetici">👑 Yönetici</option>
    `;
    
    const branchOptions = (STATE.branches || ['Tuzla Port', 'Şişli (Merkez)'])
    ...
    const body = `
        <div style="padding:8px 0">
            <div class="form-group">
                ...
                <div style="font-size:.75rem;color:var(--tx3);margin-top:4px">Giriş: kullanici@moonberrycoffee.com</div>
            </div>
    ...
    const footer = `
        <button class="btn btn-ghost" onclick="closeModal()">İptal</button>
        <button class="btn btn-primary" onclick="createNewUser()">💾 Oluştur</button>
    `;
    
    showModal('➕ Yeni Kullanıcı', body, footer);
}
...
async function createNewUser() {
    ...
    const branch = document.getElementById('newUserBranch')?.value || 'Tuzla Port';
    
    if (!username) { toast('Kullanıcı adı gerekli', 'error'); return; }
    if (!name) { toast('Görünen ad gerekli', 'error'); return; }
    if (!password || password.length < 6) { toast('Şifre en az 6 karakter olmalı', 'error'); return; }
    
    // Geçersiz karakterler kontrolü
    if (!/^[a-z0-9._]+$/.test(username)) {
        toast('Kullanıcı adı sadece küçük harf, rakam, nokta ve alt çizgi içerebilir', 'error');
        return;
    }
    ...
    try {
        ...
        if (existingDoc.exists) {
            toast('Bu kullanıcı adı zaten mevcut', 'error');
            return;
        }
...
// Personel listesindeki rolleri users collection'ına senkronize et
async function syncRolesFromPersonnel() {
    if (!confirm('Personel listesindeki roller, mevcut kullanıcılara uygulanacak. Devam etmek istiyor musunuz?')) {
        return;
    }
    ...
    try {
        ...
        if (!personnelDoc.exists) {
            toast('Personel listesi bulunamadı', 'error');
            return;
        }
        
        const personnelList = personnelDoc.data().list || [];
        
        // Email'e göre rol eşleştirmesi oluştur
        const roleMap = {};
        personnelList.forEach(p => {
            if (p.email && p.role) {
                // Email'in @ öncesini kullanıcı adı olarak kullan
                const username = p.email.split('@')[0];
                roleMap[username] = { role: p.role, branch: p.branch, name: p.name };
            }
        });
        
        // Users collection'ını güncelle
        const usersSnapshot = await db.collection('users').get();
        let updated = 0;
        ...
        for (const doc of usersSnapshot.docs) {
            ...
            // Email veya kullanıcı adı ile eşleştir
            let match = roleMap[userId];
            
            // Eğer direkt eşleşme yoksa, email'den dene
            ...
            if (match && match.role) {
                ...
                if (userData.role !== match.role) {
                    ...
                    updated++;
                    console.log(`${userId}: ${userData.role} → ${match.role}`);
                }
            }
        }
        
        toast(`${updated} kullanıcı güncellendi`, 'success');
        loadUserList();
        
    } catch (error) {
        console.error('Senkronizasyon hatası:', error);
        toast('Senkronizasyon hatası!', 'error');
    }
}
...
function showAddUserModal(editId = null) {
    ...
    const body = `
        ...
        <div class="form-grid">
            <div class="form-group"><label class="form-label">Kullanıcı Adı *</label><input type="text" id="modalUserName" class="form-input" ${isEdit ? 'readonly' : ''} placeholder="ornek.kullanici"></div>
            <div class="form-group"><label class="form-label">Görünen Ad</label><input type="text" id="modalUserDisplayName" class="form-input" placeholder="Ahmet Yılmaz"></div>
            <div class="form-group"><label class="form-label">Şifre ${isEdit ? '(boş bırakılırsa değişmez)' : '*'}</label><input type="password" id="modalUserPassword" class="form-input" minlength="6"></div>
            <div class="form-group">
                <label class="form-label">Rol *</label>
                <select id="modalUserRoleSelect" class="form-select">
                    <option value="barista">☕ Barista</option>
                    <option value="magaza_mudur_yardimcisi">🏪 Mağaza Müdür Yardımcısı</option>
                    <option value="magaza_muduru">🏪 Mağaza Müdürü</option>
                    <option value="bolge_muduru">🏢 Bölge Müdürü</option>
                    <option value="yonetici">👑 Yönetici</option>
                </select>
            ...
            <div class="form-group">
                <label class="form-label">Şube Erişimi</label>
...
async function loadUserForEdit(id) {
    try {
        ...
        if (doc.exists) {
            ...
            // Rol select'ini ayarla - önce select elementini al
            const roleSelect = document.getElementById('modalUserRoleSelect');
            if (roleSelect && u.role) {
                // Eğer rol seçeneklerde yoksa, option olarak ekle
...
async function saveUser() {
    ...
    if (!username) {
        toast('Kullanıcı adı zorunludur', 'error');
        return;
    }
    
    if (!editId && !password) {
        toast('Şifre zorunludur', 'error');
        return;
    }
    ...
    try {
        if (editId) {
            ...
            if (existing.exists) {
                toast('Bu kullanıcı adı zaten kullanılıyor', 'error');
                return;
            }
...
async function deleteUser(id) {
    if (id === 'admin') {
        toast('Admin hesabı silinemez', 'error');
        return;
    }
    
    confirmModal('Kullanıcı Sil', 'Bu kullanıcıyı silmek istediğinize emin misiniz?', async () => {
        try {
...
// ==================== GELİŞMİŞ İZİN YÖNETİMİ ====================
const PERMISSION_STRUCTURE = {
    pages: [
        { id: 'dashboard', label: 'Dashboard' },
        { id: 'personel', label: 'Personel' },
        { id: 'puantaj', label: 'Puantaj' },
        { id: 'shift', label: 'Shift Planı' },
        { id: 'checklist', label: 'Checklist' },
        { id: 'katalog', label: 'İhlal Kataloğu' }
    ],
    documents: [
        { id: 'sozlesme', label: 'İş Sözleşmesi' },
        { id: 'tutanak', label: 'Tutanak' },
        { id: 'savunma', label: 'Savunma İsteme' },
        { id: 'fesih', label: 'Fesih' },
        { id: 'istifa', label: 'İstifa' },
        { id: 'ibraname', label: 'İbraname' }
    ],
    forms: [
        { id: 'borc', label: 'Borç İkrar' },
    ...
    admin: [
        ...
        { id: 'admin_puantaj', label: 'Puantaj Kuralları' },
        { id: 'admin_subeler', label: 'Şube Yönetimi' },
        { id: 'admin_pozisyonlar', label: 'Pozisyonlar' },
        { id: 'admin_ihlaller', label: 'İhlal Düzenleme' },
        { id: 'admin_sablonlar', label: 'Şablonlar' },
        { id: 'admin_oturumlar', label: 'Oturum Ayarları' }
    ],
    special: [
        { id: 'print_shift', label: 'Shift planını yazdırabilir' },
        { id: 'create_shift_week', label: 'Yeni shift haftası oluşturabilir' },
        { id: 'view_checklist_history', label: 'Geçmiş checklisti görebilir' },
        { id: 'fill_others_checklist', label: 'Başkasının checklistini doldurabilir' },
        { id: 'view_salary', label: 'Maaş bilgilerini görebilir' },
        { id: 'export_data', label: 'Veri dışa aktarabilir' },
        { id: 'delete_records', label: 'Kayıt silebilir' },
        { id: 'assign_password', label: 'Şifre atayabilir' }
    ],
    limits: [
        { id: 'shift_history_weeks', label: 'Shift geçmiş limiti (hafta)', type: 'number', options: [1, 5, 52, 999] }
    ]
};

const DEFAULT_ROLE_PERMISSIONS = {
    barista: {
        pages: { dashboard: { view: true, edit: false }, personel: { view: false, edit: false }, puantaj: { view: true, edit: false }, shift: { view: true, edit: false }, checklist: { view: true, edit: true }, katalog: { view: false, edit: false } },
        ...
        admin: { admin_genel: false, admin_kullanicilar: false, admin_puantaj: false, admin_subeler: false, admin_pozisyonlar: false, admin_ihlaller: false, admin_sablonlar: false, admin_oturumlar: false },
        special: { print_shift: true, create_shift_week: false, view_checklist_history: false, fill_others_checklist: false, view_salary: false, export_data: false, delete_records: false, assign_password: false },
    ...
    magaza_mudur_yardimcisi: {
        ...
        admin: { admin_genel: false, admin_kullanicilar: false, admin_puantaj: false, admin_subeler: false, admin_pozisyonlar: false, admin_ihlaller: true, admin_sablonlar: false, admin_oturumlar: false },
        special: { print_shift: true, create_shift_week: true, view_checklist_history: true, fill_others_checklist: true, view_salary: false, export_data: true, delete_records: false, assign_password: false },
    ...
    bolge_muduru: {
        ...
        forms: { borc: { view: true, edit: true }, avans: { view: true, edit: true }, zimmet: { view: true, edit: true } },
        admin: { admin_genel: true, admin_kullanicilar: false, admin_puantaj: true, admin_subeler: true, admin_pozisyonlar: true, admin_ihlaller: true, admin_sablonlar: true, admin_oturumlar: true },
        special: { print_shift: true, create_shift_week: true, view_checklist_history: true, fill_others_checklist: true, view_salary: true, export_data: true, delete_records: true, assign_password: true },
        limits: { shift_history_weeks: 999 }
    }
};

// Pozisyon seçimi için yeni fonksiyonlar
let selectedRoleForPerm = null;

function selectRoleForPermissions(role) {
    selectedRoleForPerm = role;
    
    const roleInfo = {
        barista: { icon: '🧑‍🍳', name: 'Barista / Servis', desc: 'Günlük operasyon yetkileri' },
        magaza_mudur_yardimcisi: { icon: '👔', name: 'Mağaza Müdür Yardımcısı', desc: 'Yardımcı yönetim yetkileri' },
        magaza_muduru: { icon: '🏪', name: 'Mağaza Müdürü', desc: 'Şube yönetimi yetkileri' },
        bolge_muduru: { icon: '🌍', name: 'Bölge Müdürü', desc: 'Çoklu şube yönetimi yetkileri' }
    };
    
    const info = roleInfo[role];
    if (!info) {
        toast('Bilinmeyen rol: ' + role, 'error');
        return;
    }
    document.getElementById('selectedRoleIcon').textContent = info.icon;
    document.getElementById('selectedRoleName').textContent = info.name;
    document.getElementById('selectedRoleDesc').textContent = info.desc;
...
async function loadRolePermissions(role) {
    // Firebase'den izinleri al veya varsayılanı kullan
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 12px;background:var(--bg2);border-radius:8px;border:1px solid var(--cardBorder)">
                ...
                <div style="display:flex;gap:12px">
                    <label style="display:flex;align-items:center;gap:4px;cursor:pointer">
                        ...
                        <span style="font-size:.75rem;color:var(--tx2)">Gör</span>
                    </label>
                    <label style="display:flex;align-items:center;gap:4px;cursor:pointer">
                        ...
                        <span style="font-size:.75rem;color:var(--tx2)">Düzenle</span>
                    </label>
                </div>
            </div>
        `;
        ...
        html += `
            <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 12px;background:var(--bg2);border-radius:8px;border:1px solid var(--cardBorder)">
                ...
                <label style="display:flex;align-items:center;gap:4px;cursor:pointer">
                    ...
                    <span style="font-size:.75rem;color:var(--tx2)">Erişim</span>
                </label>
            </div>
        `;
    });
        ...
        html += `
            <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 12px;background:var(--bg2);border-radius:8px;border:1px solid var(--cardBorder)">
                ...
                <label style="display:flex;align-items:center;gap:4px;cursor:pointer">
                    ...
                    <span style="font-size:.75rem;color:var(--tx2)">İzin</span>
                </label>
            </div>
        `;
    });
...
function renderLimitsPermissions(perms) {
    ...
    const limitLabels = {
        'shift_history_weeks': { label: 'Shift Geçmiş Görüntüleme', unit: 'hafta', options: [1, 5, 12, 999] }
    };
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 12px;background:var(--bg2);border-radius:8px;border:1px solid var(--cardBorder)">
                    ...
                    <div style="display:flex;align-items:center;gap:8px">
                        <select class="perm-limit" data-id="${l.id}" style="padding:6px 10px;border-radius:6px;border:1px solid var(--cardBorder);background:var(--bg);color:var(--tx);font-size:.8rem">
                            ${info.options.map(opt => `<option value="${opt}" ${value === opt ? 'selected' : ''}>${opt === 999 ? 'Sınırsız' : opt + ' ' + info.unit}</option>`).join('')}
                        </select>
                    </div>
                </div>
            `;
    ...
    container.innerHTML = html || '<p style="color:var(--tx2);font-size:.8rem;text-align:center">Limit ayarı yok</p>';
}

async function saveAdvancedPermissions() {
    const role = selectedRoleForPerm || document.getElementById('advRoleSelect')?.value;
    if (!role) {
        toast('Lütfen bir pozisyon seçin', 'error');
        return;
    }
    ...
    try {
        ...
        toast('Yetkiler başarıyla kaydedildi! ✓', 'success');
        hideRolePermissions();
    } catch (error) {
...
function resetAdvancedPermissions() {
    ...
    if (!confirm(`"${role}" rolünün izinleri varsayılana döndürülecek. Emin misiniz?`)) return;
    ...
    toast('Varsayılan izinler yüklendi. Kaydetmeyi unutmayın!', 'success');
}
...
// Kullanıcı girişinde izinleri yükle
async function loadUserAdvancedPermissions() {
    ...
    try {
        ...
        console.error('Kullanıcı izin yükleme hatası:', error);
    }
}

// İzinleri UI'a uygula
function applyAdvancedPermissions() {
    ...
    // İhlal Kataloğu özel kontrolü
    const katalogPerm = perms.pages?.katalog;
    if (katalogPerm && !katalogPerm.view) {
        const navKatalog = document.querySelector('.nav-item[data-page="katalog"]');
        if (navKatalog) navKatalog.style.display = 'none';
    }
    ...
    // Belgeler bölümünü kontrol et
    const hasAnyDocAccess = Object.values(perms.documents || {}).some(d => d.view);
    if (!hasAnyDocAccess) {
        // Sidebar'daki Belgeler menüsünü gizle
        const navBelgeler = document.getElementById('nav-belgeler');
        if (navBelgeler) navBelgeler.style.display = 'none';
        
        // Dashboard'daki Belgeler section'ını gizle
        document.querySelectorAll('.dashboard-section').forEach(section => {
            const header = section.querySelector('div');
            if (header && header.textContent.includes('Belgeler')) {
                section.style.display = 'none';
    ...
    // Formlar menüsünü kontrol et
    const hasAnyFormAccess = Object.values(perms.forms || {}).some(f => f.view);
    if (!hasAnyFormAccess) {
        const navFormlar = document.getElementById('nav-formlar');
        if (navFormlar) navFormlar.style.display = 'none';
        
        // Dashboard'daki Formlar section'ını gizle
    ...
    // Özel izinleri uygula
    if (!perms.special?.print_shift) {
        // Print butonlarını gizle
    ...
    // Salt okunur badge ekle (düzenleme izni yoksa)
...
async function loadSessionSettings() {
    try {
        ...
        console.error('Oturum ayarları yükleme hatası:', error);
    }
}
...
async function saveSessionSettings() {
    ...
    try {
        ...
        toast('Oturum ayarları kaydedildi', 'success');
    } catch (error) {
        console.error('Oturum ayarları kayıt hatası:', error);
        toast('Kayıt hatası!', 'error');
    }
}
...
function resetSessionTimer() {
    ...
    // Ayarları al ve timer'ları yeniden başlat
...
function showSessionWarning() {
    ...
    const body = `
        <div style="text-align:center;padding:20px">
            ...
            <div style="font-size:1.2rem;font-weight:600;color:var(--tx);margin-bottom:12px">Oturum Süresi Doluyor!</div>
            <div style="color:var(--tx2);margin-bottom:20px">
                <span id="sessionCountdown">${seconds}</span> saniye içinde otomatik çıkış yapılacak.
            </div>
            <div style="font-size:.85rem;color:var(--tx3)">Devam etmek için herhangi bir yere tıklayın.</div>
        </div>
    `;
    
    showModal('⏰ Oturum Uyarısı', body, `
        <button class="btn btn-ghost" onclick="sessionAutoLogout()">Çıkış Yap</button>
    ...
    try {
        ...
        if (snapshot.empty) {
            container.innerHTML = `
                <div style="text-align:center;padding:40px;color:var(--tx2)">
                    <div style="font-size:2rem;margin-bottom:12px">✨</div>
                    <div>Aktif oturum bulunamadı</div>
                </div>
            `;
            return;
        }
                ...
                <table style="width:100%;border-collapse:collapse;font-size:.85rem">
                    <thead>
                        <tr style="background:var(--bg3)">
                            <th style="padding:12px;text-align:left;color:var(--tx);font-weight:600;border-bottom:1px solid var(--bg4)">Kullanıcı</th>
                            ...
                            <th style="padding:12px;text-align:left;color:var(--tx);font-weight:600;border-bottom:1px solid var(--bg4)">Giriş Zamanı</th>
                            <th style="padding:12px;text-align:left;color:var(--tx);font-weight:600;border-bottom:1px solid var(--bg4)">Son Aktivite</th>
                            <th style="padding:12px;text-align:center;color:var(--tx);font-weight:600;border-bottom:1px solid var(--bg4)">İşlem</th>
                        </tr>
                    </thead>
                    <tbody>
        `;
            ...
            html += `
                <tr style="border-bottom:1px solid var(--bg4);transition:background .2s" onmouseover="this.style.background='var(--bg3)'" onmouseout="this.style.background='transparent'">
                    ...
                    <td style="padding:12px;text-align:center">
                        ${isCurrentSession ? 
                            `<button onclick="terminateSession('${doc.id}')" style="background:rgba(243,156,18,.1);border:1px solid rgba(243,156,18,.3);color:#f39c12;padding:6px 12px;border-radius:6px;cursor:pointer;font-size:.8rem;transition:all .2s" onmouseover="this.style.background='rgba(243,156,18,.2)'" onmouseout="this.style.background='rgba(243,156,18,.1)'">⚠️ Çıkış Yap</button>` :
                            `<button onclick="terminateSession('${doc.id}')" style="background:rgba(231,76,60,.1);border:none;color:#e74c3c;padding:6px 12px;border-radius:6px;cursor:pointer;font-size:.8rem;transition:all .2s" onmouseover="this.style.background='rgba(231,76,60,.2)'" onmouseout="this.style.background='rgba(231,76,60,.1)'">🚫 Sonlandır</button>`
                        }
                    </td>
                </tr>
            `;
        ...
        container.innerHTML = `
            <div style="text-align:center;padding:40px;color:var(--red)">
                ...
                <div>Yükleme hatası: ${error.message}</div>
            </div>
        `;
    }
}
    ...
    try {
        ...
        if (snapshot.empty) {
            container.innerHTML = `
                <div style="text-align:center;padding:40px;color:var(--tx2)">
                    <div style="font-size:2rem;margin-bottom:12px">📊</div>
                    <div>Henüz oturum geçmişi yok</div>
                </div>
            `;
            return;
        }
        ...
        snapshot.forEach(doc => {
            ...
            const statusText = s.status === 'active' ? 'Aktif' : (s.status === 'timeout' ? 'Zaman Aşımı' : 'Sonlandı');
            const statusIcon = s.status === 'active' ? '🟢' : (s.status === 'timeout' ? '🟡' : '⚪');
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 16px;background:var(--bg3);border-radius:10px">
                    <div style="display:flex;align-items:center;gap:12px">
                        ...
                        <div>
                            ...
                            <div style="font-size:.75rem;color:var(--tx3)">${s.device || 'Bilinmiyor'} • ${s.ip || '-'}</div>
                        </div>
                    </div>
                    <div style="text-align:right">
        ...
        console.error('Oturum geçmişi yükleme hatası:', error);
    }
}

// Oturum sonlandır
async function terminateSession(sessionId) {
    // Kendi oturumunu mu sonlandırıyor?
    const isOwnSession = sessionId === currentUser?.sessionId;
    
    if (isOwnSession) {
        if (!confirm('KENDİ OTURUMUNUZU sonlandırmak üzeresiniz. Çıkış yapılacak. Devam etmek istiyor musunuz?')) return;
    } else {
    ...
    try {
        ...
        if (isOwnSession) {
            // Kendi oturumunu sonlandırdı - logout yap
            toast('Oturumunuz sonlandırıldı, çıkış yapılıyor...', 'warning');
            setTimeout(() => logout(), 1000);
        } else {
            toast('Oturum sonlandırıldı', 'success');
            loadActiveSessions();
            loadSessionHistory();
        }
    } catch (error) {
        console.error('Oturum sonlandırma hatası:', error);
        toast('Sonlandırma hatası!', 'error');
    }
}

// Tüm oturumları sonlandır (şifre değişikliğinde)
async function terminateAllSessions(userId, exceptCurrentSession = true) {
    try {
        ...
        console.error('Toplu oturum sonlandırma hatası:', error);
        return false;
    }
}
...
async function checkSessionValidity() {
    ...
    try {
        ...
        if (!storedSessionId) {
            // Session ID yok - yeni session oluştur
...
// Periyodik aktivite güncellemesi (5 dakikada bir)
setInterval(() => {
    if (currentUser?.sessionId) {
        updateSessionActivity();
    }
...
async function loadPuantajRulesAdmin() {
    ...
    try {
        ...
        if (snapshot.empty) {
            container.innerHTML = '<div style="text-align:center;padding:40px;color:var(--tx2)">Henüz kural tanımlanmamış</div>';
            return;
        }
                ...
                html += `
                    <div style="display:flex;justify-content:space-between;align-items:center;padding:14px;background:rgba(231,76,60,.05);border:1px solid rgba(231,76,60,.15);border-radius:12px;margin-bottom:8px">
                        ...
                        <div style="display:flex;gap:6px">
                            <button onclick="editPuantajRule('${r.id}')" style="background:var(--bg3);border:none;color:var(--tx);padding:8px 14px;border-radius:8px;cursor:pointer;font-size:.8rem;transition:all .2s" onmouseover="this.style.background='var(--bg4)'" onmouseout="this.style.background='var(--bg3)'">✏️ Düzenle</button>
                            <button onclick="deletePuantajRule('${r.id}')" style="background:rgba(255,107,107,.1);border:none;color:#e74c3c;padding:8px 14px;border-radius:8px;cursor:pointer;font-size:.8rem;transition:all .2s" onmouseover="this.style.background='rgba(255,107,107,.2)'" onmouseout="this.style.background='rgba(255,107,107,.1)'">🗑️ Sil</button>
                        </div>
                    </div>
                `;
            });
...
function showAddPuantajRuleModal(editId = null) {
    const isEdit = !!editId;
    const title = isEdit ? 'Kural Düzenle' : 'Yeni Puantaj Kuralı';
    
    const body = `
        <input type="hidden" id="ruleEditId" value="${editId || ''}">
        <div class="form-grid">
            <div class="form-group full">
                <label class="form-label">Kural Başlığı *</label>
                <input type="text" id="ruleTitle" class="form-input" placeholder="Örn: Check listi yanlış işaretleme, Geç kalma vb.">
            </div>
            <div class="form-group">
                <label class="form-label">Kategori *</label>
                <select id="ruleCategory" class="form-select">
                    <option value="ihlal">⚠️ İhlal (Negatif)</option>
                    <option value="basari">🏆 Başarı (Pozitif)</option>
                </select>
            </div>
            <div class="form-group">
                <label class="form-label">Puan Etkisi *</label>
                <select id="ruleEffect" class="form-select">
                    <optgroup label="── Negatif (İhlaller) ──">
                        <option value="-3">⭐ Ciddi İhlal (-3 Puan)</option>
                        <option value="-2">⭐⭐ Orta İhlal (-2 Puan)</option>
                        <option value="-1">⭐⭐⭐ Hafif Uyarı (-1 Puan)</option>
                    </optgroup>
                    <optgroup label="── Pozitif (Başarılar) ──">
                        <option value="+1">⭐⭐⭐⭐ İyi Performans (+1 Puan)</option>
                        <option value="+2">⭐⭐⭐⭐⭐ Mükemmel (+2 Puan)</option>
                    </optgroup>
                </select>
            </div>
            <div class="form-group full">
                <label class="form-label">Açıklama</label>
                <textarea id="ruleDesc" class="form-textarea" rows="3" placeholder="Kuralın detaylı açıklaması..." style="resize:vertical"></textarea>
            </div>
        </div>
        <div style="background:var(--bg3);border-radius:10px;padding:12px;margin-top:16px">
            <div style="font-size:.8rem;color:var(--tx2);display:flex;align-items:center;gap:8px">
                ...
                <span>Bu kural, İhlal/Başarı kaydederken seçilebilir olacak ve otomatik puan hesaplamasını etkileyecek.</span>
            </div>
        </div>
    `;
    ...
    const footer = `
        <button class="btn btn-ghost" onclick="closeModal()">İptal</button>
        <button class="btn btn-primary" onclick="savePuantajRule()">💾 Kaydet</button>
    `;
...
async function savePuantajRule() {
    ...
    // Yıldız sayısını etkiden hesapla
    let star = 3;
    if (effect === '-3') star = 1;
...
async function deletePuantajRule(id) {
    confirmModal('Kural Sil', 'Bu kuralı silmek istediğinize emin misiniz?', async () => {
        try {
...
function removePlatformCheckTime() {
    ...
    if (inputs.length > 1) {
        ...
        toast('En az bir saat olmalı', 'warning');
    }
}

// Check sistem ayarlarını kaydet
async function saveCheckSystemSettings() {
    ...
    try {
        ...
        toast('Check sistem ayarları kaydedildi ✓', 'success');
    } catch (error) {
        console.error('Check ayarları kayıt hatası:', error);
        toast('Kayıt hatası!', 'error');
    }
}

// Check sistem ayarlarını yükle
async function loadCheckSystemSettings() {
    try {
        ...
        console.error('Check ayarları yükleme hatası:', error);
    }
}

// ==================== TEMİZLİK ÇİZELGESİ AYARLARI ====================
async function saveTemizlikShiftRules() {
    ...
    try {
        ...
        toast('Temizlik çizelgesi ayarları kaydedildi', 'success');
    } catch (error) {
        console.error('Kayıt hatası:', error);
        toast('Kayıt hatası!', 'error');
    }
...
async function loadCheckRulesSettings() {
    try {
        ...
        if (doc.exists) {
            ...
            // Platform check saatlerini yükle
            ...
            // Temizlik shift kurallarını yükle
...
renderAdminLists = function() {
    ...
    // Yönetici değilse şirket bilgilerini ve şifre değiştirmeyi gizle
    if (currentUser && currentUser.role !== 'yonetici') {
...
function updateDarkModeIcon(theme) {
    ...
    // Logo güncelle (Dark: beyaz logo, Light: renkli logo)
    updateLogos(theme);
}
...
async function updateAdminCredentials() {
    ...
    if (newPassword && newPassword !== confirmPassword) { toast('Şifreler eşleşmiyor', 'error'); return; }
    if (newPassword && newPassword.length < 4) { toast('Şifre en az 4 karakter olmalı', 'error'); return; }
    ...
    try {
        ...
        toast('Giriş bilgileri güncellendi', 'success');
    ...
    } catch (e) { console.error(e); toast('Güncelleme hatası!', 'error'); }
}

// ==================== SHIFT SİSTEMİ ====================
// Shift tipleri ve saat aralıkları
...
// Excel renkleri (gerçek Excel'den alındı)
const SHIFT_COLORS = {
    'AÇILIŞ':  { bg: '#99CCFF', border: '#6699CC', text: '#003366' },  // Mavi
    'ARA':     { bg: '#FFFF99', border: '#CCCC00', text: '#666600' },  // Sarı
    'KAPANIŞ': { bg: '#FFC000', border: '#CC9900', text: '#663300' },  // Turuncu
    'OFF':     { bg: '#FF6666', border: '#CC0000', text: '#FFFFFF' },  // Kırmızı
    'Y.İ':     { bg: '#D9EAD3', border: '#6AA84F', text: '#274E13' },  // Yeşil
    'BÖLGE MÜDÜRÜ': { bg: '#E1BEE7', border: '#9C27B0', text: '#6A1B9A' },  // Mor
    'TUZLA':   { bg: '#E6B8AF', border: '#CC4125', text: '#660000' },  // Kırmızımsı
    'BÖLGE':   { bg: '#B4A7D6', border: '#8E7CC3', text: '#351C75' },  // Mor (eski)
    'RAPOR':   { bg: '#F4CCCC', border: '#E06666', text: '#990000' },  // Açık kırmızı
    'default': { bg: '#F3F3F3', border: '#CCCCCC', text: '#333333' }   // Varsayılan
};

// Shift duyuruları (Excel'den)
...
async function loadShiftList() {
    ...
    if (!selectedSube) {
        ...
        if (historyContainer) historyContainer.innerHTML = '<p style="color:var(--tx2);text-align:center;padding:20px">Mağaza seçerek geçmiş shiftleri görüntüleyin.</p>';
        return;
    }
    
    haftaSelect.innerHTML = '<option value="">⏳ Yükleniyor...</option>';
    if (historyContainer) historyContainer.innerHTML = '<p style="color:var(--tx2);text-align:center;padding:20px">⏳ Yükleniyor...</p>';
    ...
    try {
        ...
        // HAFTA LİMİTİ - Kullanıcı yetkisine göre
        // GÜNCELLEME: Gelecek hafta da her zaman görünsün (Cumartesi/Pazar girilen shift)
        const maxWeeks = window.userShiftHistoryWeeks || 999;
        const isYonetici = ['yonetici', 'bolge_muduru'].includes(currentUser?.role);
        
        // Bugünün ve gelecek haftanın başlangıcını bul
        ...
        if (!isYonetici && maxWeeks < 999) {
            // Limit uygula AMA gelecek hafta her zaman görünsün
            weeks = weeks.filter(w => {
                if (!w.weekStart) return false;
                
                // Gelecek hafta her zaman görünsün (Cumartesi/Pazar girildiyse)
        ...
        if (weeks.length === 0) {
            haftaSelect.innerHTML = '<option value="">-- Kayıtlı shift yok --</option>';
        } else {
            haftaSelect.innerHTML = '<option value="">-- Hafta Seçin (' + weeks.length + ' kayıt) --</option>';
            
            // Hangi haftayı otomatik seçeceğimizi bul
            ...
            weeks.forEach(w => {
                ...
                // Etiket oluştur
                let label = formatWeekWithYear(w.weekLabel, w.weekStart) + ' (' + cnt + ' kişi)';
                let badge = '';
                ...
                if (w.weekStart === todayWeekKey) {
                    ...
                    badge = ' 🔜 GELECEKhafta';
                    // Gelecek hafta varsa ve bu hafta yoksa, gelecek haftayı seç
            ...
            // OTOMATİK HAFTA SEÇİMİ
            if (autoSelectId) {
                haftaSelect.value = autoSelectId;
                // Seçili shift'i yükle
                setTimeout(() => selectShift(autoSelectId), 100);
                console.log(`✅ Otomatik shift seçildi: ${autoSelectType === 'current' ? 'Bu hafta' : 'Gelecek hafta'}`);
            }
        }
        
        // Geçmiş shiftleri göster
        ...
        if (historyContainer) historyContainer.innerHTML = '<p style="color:var(--red);text-align:center;padding:20px">❌ Yükleme hatası: ' + e.message + '</p>';
        toast('Shift listesi yüklenemedi: ' + e.message, 'error');
    }
}
...
function renderShiftHistory(weeks, branch, showAll = false) {
    ...
    if (!weeks || weeks.length === 0) {
        container.innerHTML = `
            <div style="text-align:center;padding:40px;background:var(--bg2);border-radius:12px">
                ...
                <div style="color:var(--tx2);margin-bottom:8px">Bu mağazada henüz shift kaydı yok.</div>
                <button class="btn btn-primary btn-sm" onclick="showNewShiftModal()" style="margin-top:12px">+ İlk Shift'i Oluştur</button>
            </div>`;
        return;
    }
    ...
    // Global'e kaydet (Daha Fazla için)
    ...
    let html = `
        <div style="margin-bottom:12px;display:flex;justify-content:space-between;align-items:center">
            ...
            <span style="font-size:.75rem;color:var(--tx2)">${showAll ? weeks.length : Math.min(3, weeks.length)}/${weeks.length} kayıt</span>
        </div>
    ...
    displayWeeks.forEach((w, idx) => {
        ...
        const badge = isFirst ? '<span style="font-size:.6rem;background:var(--accent);color:#fff;padding:2px 6px;border-radius:4px;margin-left:6px">EN YENİ</span>' : '';
        ...
        html += `
            <div onclick="selectShift('${w.id}')" 
                ...
                <div>
                    ...
                    <div style="font-size:.7rem;color:var(--tx2);margin-top:3px">👥 ${cnt} personel</div>
                </div>
                <div style="color:var(--accent);font-size:1.2rem;font-weight:bold">›</div>
            </div>`;
    });
    ...
    if (hasMore) {
        html += `
            <button onclick="renderShiftHistory(window._allShiftWeeks, window._shiftBranch, true)" 
                ...
                📂 Daha Fazla Yükle (${weeks.length - 3} daha)
            </button>`;
...
async function loadShiftWeek() {
    ...
    // Barista için silme butonunu gizle
    ...
    if (!haftaId) { 
        container.innerHTML = '<p style="color:var(--tx2);text-align:center;padding:40px">⬆️ Yukarıdan hafta seçin veya yeni hafta oluşturun.</p>'; 
        return; 
    }
    
    container.innerHTML = '<p style="color:var(--tx2);text-align:center;padding:40px">⏳ Yükleniyor...</p>';
    
    try {
        const doc = await db.collection('shifts').doc(haftaId).get();
        if (!doc.exists) { 
            container.innerHTML = '<p style="color:var(--red);text-align:center;padding:40px">❌ Shift bulunamadı.</p>'; 
            return; 
        }
        renderShiftTable(doc.data(), haftaId);
    } catch (e) { 
        console.error('Shift yükleme hatası:', e); 
        container.innerHTML = `<p style="color:var(--red);text-align:center;padding:40px">❌ Yükleme hatası: ${e.message}</p>`; 
    }
}
...
function renderShiftTable(shift, docId) {
    ...
    // Barista kontrolü - salt okunur mod
    const isBarista = currentUser?.role === 'barista';
    
    // Şube bazlı saat kuralları
    const isTuzla = (shift.branch || '').toLowerCase().includes('tuzla');
    const saatKurali = isTuzla 
        ? 'AÇILIŞ: 09:50-18:50 | Pzt-Per-Paz KAPANIŞ: 16:00-01:00 | Cum-Cmt KAPANIŞ: 16:30-01:30'
        : 'AÇILIŞ: 09:50-18:50 | KAPANIŞ: 16:30-01:30';
    
    // Sıralama: personelOrder varsa kullan, yoksa Object.keys
    ...
    // personelShifts'te olmayan isimleri çıkar
    ...
    if (personelNames.length === 0) {
        container.innerHTML = `
            <div style="text-align:center;padding:40px">
                ...
                <div style="color:var(--tx2);margin-bottom:16px">Bu haftaya henüz personel eklenmemiş.</div>
    ...
    let html = `<div id="shiftTablePrint">
        ...
        <div id="shiftPdfHeader" style="display:none;text-align:center;margin-bottom:20px;padding-bottom:16px;border-bottom:2px solid #ff8674">
            ...
            <p style="color:#666;margin:0;font-size:1rem">Shift Planı: ${weekLabelWithYear}</p>
        </div>
    ...
    personelNames.forEach((uniqueIdOrName, idx) => {
        ...
        // Görüntü adı: 1. personelNames map, 2. global cache lookup, 3. key kendisi
        let displayName = shift.personelNames?.[uniqueIdOrName];
        if (!displayName || displayName === uniqueIdOrName) {
            // Cache'ten doğru displayName'i al (codeName varsa onu kullanır)
            displayName = lookupPersonelDisplay(uniqueIdOrName, shift.branch);
        }
        
        // Sürükle-bırak için TR özellikleri (Barista için devre dışı)
        ...
        html += `<tr ${dragAttrs} data-idx="${idx}" data-key="${uniqueIdOrName}" style="background:${rowBg}">
            <td style="padding:12px;font-weight:600;color:#333;border:1px solid #ddd;background:#FFFF00;min-width:100px;${isBarista ? '' : 'cursor:grab'}">
                ${isBarista ? '' : '<span style="opacity:0.4;margin-right:4px">☰</span>'}${displayName}
            </td>`;
        ...
        days.forEach((_, dayIdx) => {
            ...
            // **** gibi değerler veya boş = boş göster
            ...
            // OFF günlerde de yıldız göster
            const isOff = cleanVal.toUpperCase() === 'OFF' || cleanVal.toUpperCase().includes('İZİN');
            const showStar = hasStar || isOff;
            
            // Yıldızlı veya OFF değer için border
            const borderStyle = showStar ? `3px solid #ff8674` : `2px solid ${colors.border}`;
            const starIndicator = showStar ? '⭐' : '';
            ...
            html += `<td style="padding:4px;text-align:center;border:1px solid #ddd;min-width:90px">
                <select ${isBarista ? 'disabled' : ''} onchange="updateShiftCell('${docId}','${uniqueIdOrName}',${dayIdx},this.value)" 
                    ...
                    <optgroup label="── Shift Türleri ──" style="background:#f5f5f5">
                        <option value="AÇILIŞ" ${displayVal === 'AÇILIŞ' && !hasStar ? 'selected' : ''} style="background:#99CCFF;color:#003366;font-weight:bold">AÇILIŞ</option>
                        <option value="ARA" ${displayVal === 'ARA' && !hasStar ? 'selected' : ''} style="background:#FFFF99;color:#666600;font-weight:bold">ARA</option>
                        <option value="KAPANIŞ" ${displayVal === 'KAPANIŞ' && !hasStar ? 'selected' : ''} style="background:#FFC000;color:#663300;font-weight:bold">KAPANIŞ</option>
                        <option value="OFF" ${displayVal === 'OFF' && !hasStar ? 'selected' : ''} style="background:#FF6666;color:#fff;font-weight:bold">OFF</option>
                        <option value="Y.İ" ${displayVal === 'Y.İ' && !hasStar ? 'selected' : ''} style="background:#D9EAD3;color:#274E13;font-weight:bold">Y.İ</option>
                        <option value="RAPOR" ${displayVal === 'RAPOR' && !hasStar ? 'selected' : ''} style="background:#F4CCCC;color:#990000;font-weight:bold">RAPOR</option>
                        <option value="BÖLGE MÜDÜRÜ" ${displayVal === 'BÖLGE MÜDÜRÜ' && !hasStar ? 'selected' : ''} style="background:#E1BEE7;color:#6A1B9A;font-weight:bold">BÖLGE MÜDÜRÜ</option>
                        <option value="BÖLGE MÜD" ${displayVal === 'BÖLGE MÜD' && !hasStar ? 'selected' : ''} style="background:#E1BEE7;color:#6A1B9A;font-weight:bold">BÖLGE MÜD</option>
                        <option value="BÖLGE M" ${displayVal === 'BÖLGE M' && !hasStar ? 'selected' : ''} style="background:#E1BEE7;color:#6A1B9A;font-weight:bold">BÖLGE M</option>
                        <option value="TUZLA" ${displayVal === 'TUZLA' && !hasStar ? 'selected' : ''} style="background:#E6B8AF;color:#660000;font-weight:bold">TUZLA</option>
                    </optgroup>
                    <optgroup label="── ⭐ Özel İstek ──" style="background:#fff3e0">
                        <option value="*AÇILIŞ" ${displayVal === 'AÇILIŞ' && hasStar ? 'selected' : ''} style="background:#99CCFF;color:#003366">⭐ AÇILIŞ</option>
                        <option value="*ARA" ${displayVal === 'ARA' && hasStar ? 'selected' : ''} style="background:#FFFF99;color:#666600">⭐ ARA</option>
                        <option value="*KAPANIŞ" ${displayVal === 'KAPANIŞ' && hasStar ? 'selected' : ''} style="background:#FFC000;color:#663300">⭐ KAPANIŞ</option>
                        <option value="*OFF" ${displayVal === 'OFF' && hasStar ? 'selected' : ''} style="background:#FF6666;color:#fff">⭐ OFF</option>
                        <option value="*Y.İ" ${displayVal === 'Y.İ' && hasStar ? 'selected' : ''} style="background:#D9EAD3;color:#274E13">⭐ Y.İ</option>
                        <option value="*RAPOR" ${displayVal === 'RAPOR' && hasStar ? 'selected' : ''} style="background:#F4CCCC;color:#990000">⭐ RAPOR</option>
                        <option value="*BÖLGE MÜDÜRÜ" ${displayVal === 'BÖLGE MÜDÜRÜ' && hasStar ? 'selected' : ''} style="background:#E1BEE7;color:#6A1B9A">⭐ BÖLGE MÜDÜRÜ</option>
                        <option value="*BÖLGE M" ${displayVal === 'BÖLGE M' && hasStar ? 'selected' : ''} style="background:#E1BEE7;color:#6A1B9A">⭐ BÖLGE M</option>
                        <option value="*TUZLA" ${displayVal === 'TUZLA' && hasStar ? 'selected' : ''} style="background:#E6B8AF;color:#660000">⭐ TUZLA</option>
                    </optgroup>
                    <optgroup label="── Saatler ──" style="background:#e3f2fd">
                        ${COMMON_TIMES.map(t => `<option value="${t}" ${cleanVal === t && !hasStar ? 'selected' : ''} style="background:#E8F4FD;color:#1E5B94">${t}</option>`).join('')}
                    </optgroup>
                    <optgroup label="── ⭐ Özel Saat ──" style="background:#fff8e1">
                        ${COMMON_TIMES.map(t => `<option value="*${t}" ${cleanVal === t && hasStar ? 'selected' : ''} style="background:#FFF3E0;color:#E65100">⭐ ${t}</option>`).join('')}
                    </optgroup>
                    ${cleanVal && !['AÇILIŞ','ARA','KAPANIŞ','OFF','Y.İ','RAPOR','BÖLGE MÜDÜRÜ','BÖLGE MÜD','BÖLGE M','TUZLA'].includes(displayVal) && !COMMON_TIMES.includes(cleanVal) ? `<option value="${val}" selected style="background:${colors.bg};color:${colors.text};font-weight:700">${starIndicator}${displayVal}</option>` : ''}
                    <optgroup label="── Diğer ──" style="background:#eceff1">
                        <option value="parttime" style="color:#546e7a">Özel Saat Gir...</option>
                    </optgroup>
                </select>
            </td>`;
        });
    ...
    // Personel Ekle/Çıkar butonları (shift tablosunun hemen altında) - Barista için gizle
    if (!isBarista) {
        html += `<div style="display:flex;gap:10px;margin:16px 0;justify-content:center">
            <button class="btn btn-primary btn-sm" onclick="addPersonelToShift('${docId}')" style="padding:8px 16px;font-size:.8rem">➕ Personel Ekle</button>
            <button class="btn btn-ghost btn-sm" onclick="removePersonelFromShiftModal('${docId}')" style="padding:8px 16px;font-size:.8rem">➖ Personel Çıkar</button>
        </div>`;
    }
    
    // Shift Kuralları bölümü (düzenlenebilir)
    html += `<div id="shiftAnnouncements" style="margin-top:20px;padding:16px;background:var(--bg2);border-radius:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
            <div style="font-weight:600;color:var(--tx);font-size:.9rem">📢 Shift Kuralları</div>
            ${!isBarista ? `<button class="btn btn-ghost btn-sm" onclick="editShiftRules('${docId}')" style="padding:4px 10px;font-size:.75rem">✏️ Düzenle</button>` : ''}
        </div>
    ...
    // Özel istekleri grupla (aynı personel + aynı neden = tek satır)
    ...
    html += `<div id="shiftSpecialRequests" style="margin-top:16px;padding:16px;background:#FFF8E1;border:2px solid #FFB300;border-radius:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
            <div style="font-weight:600;color:#E65100;font-size:.9rem">⭐ Özel İstek Günleri</div>
            ${!isBarista ? `<button class="btn btn-primary btn-sm" onclick="addSpecialRequest('${docId}')" style="padding:4px 10px;font-size:.75rem;background:#FF9800">+ Özel İstek Ekle</button>` : ''}
        </div>
        <div style="font-size:.75rem;color:#795548;margin-bottom:10px;padding:6px 10px;background:rgba(0,0,0,.05);border-radius:6px">
            SHIFT'E YILDIZ (*) SEMBOLÜ OLAN GÜNLER ÖZEL İSTEK GÜNLERİDİR.
        </div>
            ...
            ${groupedList.length > 0 ? groupedList.map((r, gi) => `
                <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;background:#fff;border-radius:8px;border:1px solid #FFB300">
                    <div>
                        ...
                        <span style="color:#888;margin:0 6px;font-size:.8rem">${r.days.length > 0 ? r.days.join(', ') : 'Tüm Hafta'}</span>
                        <span style="color:#666;margin:0 4px">→</span>
                        <span style="color:#333;font-weight:500">${r.reason}</span>
                    </div>
                    ${!isBarista ? `<button onclick="removeSpecialRequestGroup('${docId}', '${r.personel}', '${r.reason}')" style="background:none;border:none;color:#E53935;cursor:pointer;font-size:1rem" title="Bu isteği sil">×</button>` : ''}
                </div>
            `).join('') : `
                <div style="text-align:center;padding:16px;color:#795548;font-size:.85rem">
                    Henüz özel istek eklenmemiş.
                </div>
            `}
        </div>
    </div>`;
...
async function reorderShiftPersonel(docId, fromIdx, toIdx) {
    try {
        ...
        const shift = doc.data();
        const order = shift.personelOrder || Object.keys(shift.personelShifts || {});
        ...
        console.log(`[Sıralama] ${moved} taşındı: ${fromIdx} → ${toIdx}`);
...
function getShiftColorExcel(val) {
    ...
    // **** gibi değerler OFF olarak kabul edilsin
    ...
    // Kısmi eşleşme
    if (upperVal.includes('AÇILIŞ') || upperVal.includes('ACILIS')) return SHIFT_COLORS.AÇILIŞ;
    if (upperVal.includes('KAPANIŞ') || (upperVal.includes('KAPANIŞ') || upperVal.includes('KAPANIS'))) return SHIFT_COLORS.KAPANIŞ;
    if (upperVal.includes('OFF') || upperVal.includes('İZİN') || upperVal.includes('IZIN')) return SHIFT_COLORS.OFF;
    ...
    if (upperVal.includes('BÖLGE MÜDÜRÜ') || upperVal.includes('BOLGE MUDURU')) return SHIFT_COLORS['BÖLGE MÜDÜRÜ'];
    if (upperVal.includes('BÖLGE') || upperVal.includes('BOLGE')) return SHIFT_COLORS.BÖLGE;
    if (upperVal.includes('RAPOR')) return SHIFT_COLORS.RAPOR;
    if (upperVal.includes('Y.İ') || upperVal.includes('YILLIK')) return SHIFT_COLORS['Y.İ'];
    
    // Saat formatı (örn: 10:00 - 19:00)
...
// Shift değerini görüntüleme formatına çevir (**** -> OFF)
            ...
            <div style="padding:8px 0">
                ...
                <div style="margin-top:12px">
                    <label style="display:flex;align-items:center;gap:8px;cursor:pointer">
                        ...
                        <span>⭐ Özel istek olarak işaretle</span>
                    </label>
                </div>
            </div>
        `, `<button class="btn btn-ghost" onclick="closeModal();loadShiftWeek()">İptal</button>
...
async function toggleStar(docId, name, dayIdx, isChecked) {
    try {
        ...
        await db.collection('shifts').doc(docId).update({ personelShifts: data.personelShifts });
        toast(isChecked ? '⭐ Özel istek eklendi' : 'Özel istek kaldırıldı', 'success');
        loadShiftWeek();
    } catch (e) { 
        console.error(e); 
        toast('Hata!', 'error'); 
...
async function confirmParttime(docId, name, dayIdx) {
    ...
    if (!start || !end) { toast('Saat seçin', 'error'); return; }
    let value = `${start}/${end}`;
    ...
    try {
        ...
        await db.collection('shifts').doc(docId).update({ personelShifts: data.personelShifts });
        toast('✓ Part-time saat kaydedildi', 'success');
        loadShiftWeek();
    } catch (e) { 
        console.error(e); 
        toast('Güncelleme hatası!', 'error'); 
...
// Shift kurallarını düzenleme
function editShiftRules(docId) {
    showModal('Shift Kurallarını Düzenle', `
        <div style="padding:8px 0">
            <div class="form-group" style="margin-bottom:12px">
                <label class="form-label">Kural 1 (Yeşil)</label>
                <input type="text" id="rule1" class="form-input" value="AÇILIŞ PERSONELİ İŞE GEÇ KALMAMALIDIR!">
            </div>
            <div class="form-group" style="margin-bottom:12px">
                <label class="form-label">Kural 2 (Yeşil)</label>
                <input type="text" id="rule2" class="form-input" value="İZİN GÜNÜ YOKSA; YEMEK VE MOLA SÜRESİ SEÇİMİ PERSONELİN KENDİ SEÇİMİNDEDİR.">
            </div>
            <div class="form-group" style="margin-bottom:12px">
                <label class="form-label">Kural 3 (Sarı)</label>
                <input type="text" id="rule3" class="form-input" value="SHIFT'E YILDIZ (*) SEMBOLÜ OLAN GÜNLER ÖZEL İSTEK GÜNLERİDİR.">
            </div>
            <div class="form-group">
                <label class="form-label">Kural 4 (Kırmızı)</label>
                <input type="text" id="rule4" class="form-input" value="SHIFT YÖNETİCİLERİNİZE BİLGİ VERİLMEDEN HABERSİZ DEĞİŞTİRİLEMEZ!">
            </div>
        </div>
    `, `<button class="btn btn-ghost" onclick="closeModal()">İptal</button>
...
async function saveShiftRules(docId) {
    ...
    try {
        ...
        toast('Kurallar güncellendi', 'success');
        loadShiftWeek();
...
async function addSpecialRequest(docId) {
    ...
    let personelOpts = '<option value="">-- Personel Seçin --</option>';
    try {
        const doc = await db.collection('shifts').doc(docId).get();
        const data = doc.data();
    ...
    // Çoklu gün seçimi için checkbox'lar
    ...
    showModal('Özel İstek Ekle', `
        <div style="padding:8px 0">
            ...
            <div class="form-group" style="margin-bottom:12px">
                <label class="form-label">Günler (çoklu seçim yapabilirsiniz)</label>
                <div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:6px">
                    ${dayCheckboxes}
                </div>
                <div style="margin-top:6px;font-size:.75rem;color:var(--tx3)">Hiçbiri seçilmezse tüm hafta için geçerli olur.</div>
            </div>
            <div class="form-group">
                <label class="form-label">İstek Nedeni</label>
                <input type="text" id="reqReason" class="form-input" placeholder="Örn: OKUL, HASTANE, AİLE vb.">
            </div>
        </div>
    `, `<button class="btn btn-ghost" onclick="closeModal()">İptal</button>
...
async function saveSpecialRequest(docId) {
    ...
    try {
        ...
        // Her seçili gün için ayrı kayıt ekle (veya hiç seçilmediyse null ile tek kayıt)
        ...
        toast('Özel istek eklendi', 'success');
        loadShiftWeek();
    } catch (e) { console.error(e); toast('Hata!', 'error'); }
}

async function removeSpecialRequest(docId, idx) {
    if (!confirm('Bu özel isteği silmek istediğinize emin misiniz?')) return;
    try {
        const doc = await db.collection('shifts').doc(docId).get();
        const data = doc.data();
        const requests = data.specialRequests || [];
        ...
        await db.collection('shifts').doc(docId).update({ specialRequests: requests });
        toast('Özel istek silindi', 'success');
        loadShiftWeek();
    } catch (e) { console.error(e); toast('Hata!', 'error'); }
}

// Gruplu özel isteği sil (aynı personel + reason olan tüm kayıtlar)
async function removeSpecialRequestGroup(docId, personel, reason) {
    if (!confirm(`${personel} için "${reason}" isteğini silmek istediğinize emin misiniz?`)) return;
    try {
        const doc = await db.collection('shifts').doc(docId).get();
        const data = doc.data();
        const requests = data.specialRequests || [];
...
async function addPersonelToShift(docId) {
    // Mağazadaki personel listesini getir (şube bazlı)
    ...
    try {
        ...
        pSnap.forEach(d => {
            const p = d.data();
            if (p.branch === sube) {
                // getPersonelDisplayName: codeName varsa onu, yoksa "Ad S." formatı
                const displayName = getPersonelDisplayName(p);
                // uniqueId yoksa oluştur
                const uniqueId = p.uniqueId || normalizePersonelId(p.name);
                personelList.push({ 
                    name: p.name,
                    codeName: p.codeName,
        ...
        // Global'e kaydet (confirmAddPersonel'de kullanılacak)
        ...
        // Modal ile seçim - uniqueId'yi value olarak kullan
        ...
        showModal('Personel Ekle', `
            <div style="padding:8px 0">
                ...
                <div class="form-group">
                    <label class="form-label">Geçici Personel (Manuel)</label>
                    <input type="text" id="addPersonelManual" class="form-input" placeholder="Kod adı girin">
                </div>
            </div>
        `, `<button class="btn btn-ghost" onclick="closeModal()">İptal</button>
...
async function confirmAddPersonel(docId) {
    ...
    if (manual.value.trim()) {
        ...
        toast('Personel seçin veya girin', 'error');
    }
}
...
async function doAddPersonel(docId, displayName, fullName, uniqueId) {
    try {
        ...
        toast('Personel eklendi', 'success');
        loadShiftWeek();
    } catch (e) { console.error(e); toast('Ekleme hatası!', 'error'); }
}
...
async function removePersonelFromShift(docId) {
    try {
        ...
        showModal('Personel Çıkar', `
            <div style="padding:8px 0">
                <div class="form-group">
                    <label class="form-label">Çıkarılacak Personel</label>
                ...
                <div style="margin-top:12px;padding:12px;background:rgba(231,76,60,.1);border-radius:8px;color:var(--red);font-size:.85rem">
                    ⚠️ Bu işlem geri alınamaz. Personelin tüm shift verileri silinecek.
                </div>
            </div>
        `, `<button class="btn btn-ghost" onclick="closeModal()">İptal</button>
            <button class="btn btn-primary" style="background:var(--red)" onclick="confirmRemovePersonel('${docId}')">Çıkar</button>`);
    } catch (e) { console.error(e); }
}
...
async function confirmRemovePersonel(docId) {
    ...
    try {
        ...
        loadShiftWeek();
    } catch (e) { console.error(e); toast('Silme hatası!', 'error'); }
}
...
function showNewShiftModal() {
    ...
    // Hafta seçenekleri (sadece bu hafta + gelecek 8 hafta)
    const weekOpts = [];
    const today = new Date();
    for (let i = 0; i <= 8; i++) {
        ...
        weekOpts.push(`<option value="${monStr}|${sunStr}" ${isNext ? 'selected' : ''}>${label}${isCurrent ? ' (Bu Hafta)' : ''}${isNext ? ' ✓ Gelecek Hafta' : ''}</option>`);
    }
    
    showModal('Yeni Shift Haftası', `
        <div style="padding:8px 0">
            <div class="form-group" style="margin-bottom:16px">
            ...
            <div class="form-group">
                <label style="display:flex;align-items:center;gap:8px;cursor:pointer">
                    ...
                    <span>Önceki haftadan personel listesini kopyala</span>
                </label>
            </div>
        </div>
    `, `<button class="btn btn-ghost" onclick="closeModal()">İptal</button>
        <button class="btn btn-primary" onclick="createNewShift()">Oluştur</button>`);
}
...
async function createNewShift() {
    ...
    if (!sube || !weekVal) { toast('Tüm alanları doldurun', 'error'); return; }
    
    const [start, end] = weekVal.split('|');
    
    try {
        // Aynı hafta var mı kontrol
        ...
        // Önceki haftadan kopyala (TÜM shift verileri dahil)
        if (copy) {
            const allShifts = [];
            existingSnap.forEach(doc => {
                const d = doc.data();
        ...
        // Personel yoksa mağazadaki personelden al (rütbeye göre sırala)
        if (Object.keys(ps).length === 0) {
            ...
            pSnap.forEach(d => {
                const p = d.data();
                // Bölge müdürlerini ayrı tut - TÜM şubelere eklenecek
                if (p.role === 'bolge_muduru') {
                    bolgeMudurleri.push(p);
                }
                // Bu şubenin personelini ekle
                if (p.branch === sube) {
                    personelList.push(p);
                }
            });
            
            // Bölge müdürlerini listeye ekle (zaten yoksa)
            bolgeMudurleri.forEach(bm => {
        ...
        toast('Yeni hafta oluşturuldu!', 'success');
        closeModal();
        
        // Mağazayı seç ve listeyi yenile
        document.getElementById('shiftSube').value = sube;
        await loadShiftList();
        
        // Yeni haftayı seç ve tabloyu yükle
        setTimeout(() => {
            const haftaSelect = document.getElementById('shiftHafta');
            haftaSelect.value = newDoc.id;
            // Dropdown'da yeni eklenen seçenek görünsün
...
// PDF İndir - pdfmake ile renkli tablo
async function downloadShiftPDF() {
    ...
    if (!docId) { 
        toast('Önce bir shift seçin', 'error'); 
        return; 
    }
    ...
    try {
        ...
        console.error(e);
        toast('Shift verisi alınamadı', 'error');
        return;
    }
    ...
    const daysLong = ['Pazartesi', 'Salı', 'Çarşamba', 'Perşembe', 'Cuma', 'Cumartesi', 'Pazar'];
    
    // Şube bazlı saat kuralları
    const isTuzla = branch.toLowerCase().includes('tuzla');
    const saatKurali = isTuzla 
        ? 'AÇILIŞ: 09:50-18:50 | Pzt-Per-Paz: 16:00-01:00 | Cum-Cmt: 16:30-01:30'
        : 'AÇILIŞ: 09:50-18:50 | KAPANIŞ: 16:30-01:30';
    ...
    // Personel satırları - HER HÜCRE RENK ALACAK
    personelOrder.forEach((name, idx) => {
        if (!personelShifts[name]) return;
        
        // DOĞRU İSİM: personelNames mapping > lookup > name
        ...
        for (let dayIdx = 0; dayIdx < 7; dayIdx++) {
            ...
            if (isEmpty) {
                ...
                // Yıldız için basit ASCII * kullan (PDF uyumlu)
    ...
    const ozelIstekler = Object.values(groupedRequests).length > 0 
        ? Object.values(groupedRequests).map(r => {
            const dayStr = r.days.length > 0 ? r.days.join(', ') : 'Tüm Hafta';
            return `• ${r.personel}: ${dayStr} → ${r.reason}`;
        }).join('\n')
        : 'Bu hafta için özel istek yok';
    ...
    const docDefinition = {
        ...
        content: [
            // Başlık - Marka renkleri
            { text: 'Moonberry Coffee - ' + branch, fontSize: 16, bold: true, color: '#ff8674' },
            { text: 'Shift Planı: ' + weekLabel, fontSize: 10, color: '#008c95', margin: [0, 2, 0, 10] },
            ...
            // Özel İstekler
            { text: '', margin: [0, 8, 0, 0] },
            { text: 'Özel İstek Günleri:', fontSize: 9, bold: true, color: '#F57C00' },
...
async function deleteCurrentShift() {
    ...
    if (currentUser?.role !== 'yonetici') {
        toast('Sadece yönetici shift silebilir', 'error');
        return;
    }
    ...
    try {
        ...
        if (shiftsSnap.empty) {
            toast('Silinecek shift bulunamadı', 'error');
            return;
        }
        ...
        if (docId !== newestShiftId) {
            toast('Sadece en son oluşturulan hafta silinebilir!', 'error');
            return;
        }
    } catch (e) {
    ...
    // Onay iste
    if (!confirm(`"${weekLabel}" shift'ini silmek istediğinize emin misiniz?\n\nBu işlem geri alınamaz!`)) {
        return;
    }
    ...
    try {
        ...
        haftaSelect.value = '';
        document.getElementById('shiftTableContainer').innerHTML = '<p style="color:var(--tx2);text-align:center;padding:40px">⬆️ Yukarıdan hafta seçin.</p>';
        loadShiftList();
    } catch (e) {
        console.error('Silme hatası:', e);
...
async function printShift() {
    ...
    personelOrder.forEach(name => {
        ...
        for (let dayIdx = 0; dayIdx < 7; dayIdx++) {
            ...
            if (isEmpty) {
                ...
                const colors = getShiftColorExcel(cleanVal);
                const displayText = showStar ? '⭐' + cleanVal : cleanVal;
    ...
    const specialRequestsHtml = Object.values(groupedRequests).length > 0 
        ? Object.values(groupedRequests).map(r => {
            const dayStr = r.days.length > 0 ? r.days.join(', ') : 'Tüm Hafta';
            return `<span class="request-item">${r.personel}: ${dayStr} → ${r.reason}</span>`;
        }).join('') 
        : '<span class="empty-requests">Bu hafta için özel istek yok</span>';
        ...
        <body>
            <div class="header">
                ...
                <div>
                    ...
                    <p class="subtitle">Shift Planı: ${weekLabel}</p>
                </div>
            </div>
            <table>
                <thead>
            ...
            <div class="requests-box">
                <div class="requests-title">📌 Özel İstek Günleri</div>
                ${specialRequestsHtml}
            </div>
...
const EXCEL_PERSONEL = {
    'Şişli': ["ESİN", "ESMA", "ECE"],
    'Tuzla': ["BÜŞRA", "ESİN", "BALRA", "TUANA", "BARAN", "ELİF"]
};

// Firebase'e yükle
async function uploadExcelToFirebase() {
    if (currentUser?.role !== 'yonetici') {
        toast('Sadece yönetici yapabilir', 'error');
        return;
    }
    ...
    try {
        ...
        toast(`✓ ${pAdded} personel, ${sAdded} shift eklendi`, 'success');
        loadShiftList();
    ...
    btn.disabled = false;
    btn.innerHTML = '📥 Excel Verilerini Yükle';
}
...
async function logActivity(type, details = {}) {
    ...
    try {
        ...
        // Sessizce devam et - aktivite loglama kritik değil
...
async function loadActivityLogs() {
    ...
    if (userFilter && userFilter.options.length <= 1) {
        try {
            ...
            console.error('Kullanıcı listesi yüklenemedi:', e);
        }
    }
    ...
    try {
        ...
        const typeLabels = {
            'login': { icon: '🔐', label: 'Giriş Yaptı', color: '#27ae60' },
            'logout': { icon: '🚪', label: 'Çıkış Yaptı', color: '#95a5a6' },
            'page_view': { icon: '👁️', label: 'Sayfa Görüntüledi', color: '#3498db' },
            'document_create': { icon: '📄', label: 'Belge Oluşturdu', color: '#9b59b6' },
            'data_edit': { icon: '✏️', label: 'Veri Düzenledi', color: '#f39c12' },
            'data_delete': { icon: '🗑️', label: 'Veri Sildi', color: '#e74c3c' }
        };
        ...
        snapshot.forEach(doc => {
            ...
            const typeInfo = typeLabels[log.type] || { icon: '📌', label: log.type, color: '#95a5a6' };
        ...
        // Sessiz hata - aktivite logları kritik değil
        container.innerHTML = `
            <div style="text-align:center;padding:40px;color:var(--tx2)">
                <div style="font-size:2rem;margin-bottom:12px">📊</div>
                <div>Aktivite logları yüklenemedi</div>
                <div style="font-size:.8rem;margin-top:8px;color:var(--tx3)">Firebase ayarları kontrol edilebilir</div>
            </div>
        `;
    }
}

// getWeekOptions fonksiyonu (hafta listesi için)// getWeekOptions fonksiyonu (hafta listesi için)
...
// ==================== CHECKLIST MODULE ====================
// Checklist türleri - şubeye göre
// Checklist türleri - Firebase'den yüklenecek
let CHECKLIST_TYPES = {};
let SYSTEM_CONFIG = {};
let BRANCH_HOURS = {
    'Tuzla Port': { opening: '10:00', closing: { default: '01:00', friday: '01:30', saturday: '01:30' } },
    'Şişli (Merkez)': { opening: '10:00', closing: { default: '01:30' } }
};

// TEMİZLİK ÇİZELGESİ SHIFT TANIMLARI
const TEMIZLIK_SHIFT_RULES = {
    acilisci: {
        name: 'Açılışçı',
        shiftStartMin: '10:00',  // 10:00'dan başlayan
        shiftStartMax: '11:00',  // 11:00'a kadar başlayan (10:00-10:59)
        description: 'Sabah shifti başlatan personel (10:00-11:00 arası başlayanlar)'
    },
    araci: {
        name: 'Aracı',
        shiftStartMin: '11:00',  // 11:00'dan başlayan
        shiftStartMax: '16:00',  // 16:00'a kadar başlayan (11:00-15:59)
        description: 'Ara vardiya personeli (11:00-16:00 arası başlayanlar)'
    },
    kapanisci: {
        name: 'Kapanışçı',
        shiftStartMin: '16:00',  // 16:00'dan sonra başlayan
        shiftStartMax: '23:59',  // Gece yarısına kadar
        description: 'Akşam/kapanış vardiyası personeli (16:00+ başlayanlar)'
    }
};

// PLATFORM CHECK SAATLERİ
const PLATFORM_CHECK_TIMES = ['10:00', '12:00', '15:00', '18:00', '21:00', '00:00'];
...
let CHECK_RULES = {
    ...
    platform: {
        ...
        window: { before: 30, after: 30 }, // Her saat için ±30 dakika
        lateWindow: 30,
    ...
    aylik: {
        ...
        points: { onTime: 0, late: -1, missed: -1 },
        note: 'Her personel kendi shift türüne göre 1 kere yapmalı'
    },
    haftalik: {
        name: 'Haftalık Görevler',
        points: { done: 0, missed: -1 }
    }
};
let SHIFT_DURATION = 9; // saat

// Platform listesi - Firebase'den yüklenecek
let PLATFORMS = ['Trendyol', 'Yemek Sepeti', 'Getir Yemek', 'Migros'];

// WhatsApp Grup Linkleri - Firebase'den yüklenecek
let WHATSAPP_GROUPS = {};
...
// Mağaza kapanış saatini al (güne göre)
...
// Mağaza açılış saatini al
function getBranchOpeningTime(branch) {
    const hours = BRANCH_HOURS[branch];
    return hours?.opening || '10:00';
}

// Saat string'ini dakikaya çevir (00:00 = 1440 olarak hesapla gece için)
function timeToMinutes(timeStr, forNight = false) {
    const [h, m] = timeStr.split(':').map(Number);
    let minutes = h * 60 + m;
    if (forNight && h < 6) minutes += 1440; // Gece saatleri için ertesi güne ekle
    return minutes;
}

// Dakikayı saat string'ine çevir
...
// Kapanış shift başlangıç saatini hesapla
...
async function getCurrentShiftForPerson(personId, branch, date = new Date()) {
    ...
    try {
        ...
        console.warn('Shift planı okunamadı:', e);
    }
...
// Tarihi YYYY-MM-DD formatında döndür (LOCAL timezone)
...
// Check için zaman penceresi hesapla
    ...
    if (isCompleted) {
        if (completedAt) {
            ...
            if (completedMinutes <= window.endMinutes) {
                ...
                return { status: 'late', points: rules.points.late, label: 'Geç', color: '#f39c12' };
            }
        }
        return { status: 'completed', points: 0, label: 'Tamamlandı', color: '#27ae60' };
    }
    
    if (window.isMissed) {
        return { status: 'missed', points: rules.points.missed, label: 'Yapılmadı', color: '#e74c3c' };
    }
    
    if (window.isLate) {
        return { status: 'lateWindow', points: 0, label: 'Geç Penceresi', color: '#f39c12' };
    }
...
function getRemainingTime(checkType, targetTime, branch) {
    ...
    // Geç penceresi varsa onu kullan
    ...
    if (remaining <= 0) return { minutes: 0, text: 'Süre doldu', isExpired: true };
...
// Otomatik puantaj notu oluştur - TAM İSİM LOOKUP SİSTEMİ
// Personel cache (performans için)
...
// Kısa isimden tam personel bilgisi bul - GÜÇLENDİRİLMİŞ
async function findPersonelByDisplayName(displayName, branch) {
    ...
    // Helper: Türkçe karakterleri normalize et (karşılaştırma için)
    // ÖNEMLİ: Önce büyük Türkçe harfleri replace et, sonra toLowerCase!
    ...
    // 1. Tam isim eşleşmesi (case-insensitive)
    ...
    // 2. codeName eşleşmesi (case-insensitive + Türkçe normalize)
    ...
    // 4. İlk isim eşleşmesi (branch ile filtrele)
    ...
    // 5. "İsim S." formatı eşleştirmesi
    const match = searchName.match(/^(.+?)\s+([A-ZÇĞİÖŞÜa-zçğıöşü])\.?$/);
    if (match) {
    ...
    // 6. Kısmi eşleşme - codeName içinde ara (BALRA → Balra)
    found = personelList.find(p => {
    ...
    // 7. codeName'in ilk kısmı ile eşleştir (ESİN → "Esin İ." → esin)
    found = personelList.find(p => {
    ...
    // 8. İsmin herhangi bir parçasıyla eşleş
    found = personelList.find(p => {
...
async function createAutoPuantajNote(personelId, personelName, checkType, checkTime, status, points, branch, date) {
    ...
    // === TAM İSİM LOOKUP ===
    // personelName kısa isim olabilir ("Büşra S."), tam personeli bul
    ...
    if (personelInfo) {
        ...
        console.log(`[PuantajNote] ${personelName} → ${finalPersonelName} (${finalPersonelId})`);
    } else {
        // Bulunamadı, gelen değerleri kullan
        finalPersonelId = personelId;
        finalPersonelName = personelName;
        console.warn(`[PuantajNote] ${personelName} için tam isim bulunamadı, gelen değer kullanılıyor`);
    }
    ...
    if (status === 'onTime') {
        ...
        note = `${checkTime} ${checkName} geç dolduruldu`;
    } else if (status === 'missed') {
        note = `${checkTime} ${checkName} doldurulmadı`;
    }
    ...
    try {
        ...
        console.log('✅ Puantaj notu oluşturuldu:', noteId, points);
    } catch (e) {
        console.error('❌ Puantaj notu oluşturulamadı:', e.message);
    }
}

// Yapılmayan checkler için otomatik puan işle
async function processMissedChecks(branch, checkType, timeSlot, date, shiftPersonnel) {
    // Sadece yönetici/müdür görünümünde işle (tekrarlı işlemi önlemek için)
    ...
    // Her sorumlu personel için puan notu oluştur
    for (const personName of shiftPersonnel) {
        // TAM İSİM'den normalize edilmiş ID oluştur
        const personId = normalizePersonelId(personName);
        
        console.log(`[puantajNote] Kayıt: personName="${personName}" → personId="${personId}"`);
...
async function updateDashboardCheckCards() {
    // Bugünün tarihi (02:00 öncesi önceki gün)
    const now = new Date();
    ...
    try {
        ...
        for (const branch of branches) {
            ...
            // 1. Checklist (gunluk + acilis/kapanis)
            ...
            for (const shift of shifts) {
                temizlikTotal++;
                const docId = `${branchKey}_temizlik_${todayStr}_${shift}`;
                try {
                    const doc = await db.collection('checklistSubmissions').doc(docId).get();
            ...
            const platformTimes = PLATFORM_CHECK_TIMES || ['10:00', '12:00', '15:00', '18:00', '21:00', '00:00'];
            ...
            for (const time of platformTimes) {
                ...
                if (timeMinutes <= currentMinutes || (h === 0 && currentHour >= 22)) {
                    platformTotal++;
...
async function loadTodayChecks() {
    ...
    // ====== DASHBOARD KARTLARINI DOLDUR ======
    updateDashboardCheckCards();
    
    // GÜN SONU SAATİ: 02:00 - Bu saatten önce önceki gün sayılır
    const DAY_END_HOUR = 2;
    
    const now = new Date();
    // Eğer saat 02:00'den önceyse, önceki günü kullan
    let today = new Date(now);
    ...
    // Yönetici için tüm şubeler, diğerleri için kendi şubesi
    let branches = [];
    if (isAdmin || currentUser?.branch === 'all') {
        branches = STATE.branches || ['Tuzla Port', 'Şişli (Merkez)'];
    } else {
        branches = [currentUser?.branch || 'Tuzla Port'];
    }
    
    const primaryBranch = branches[0];
    
    // Karşılama kartını doldur
    if (welcomeCard) {
        ...
        else if (hour >= 18 || hour < DAY_END_HOUR) greeting = 'İyi akşamlar';
        
        document.getElementById('welcomeDate').textContent = dateStr;
        document.getElementById('welcomeMessage').textContent = `${greeting}, ${userName.split(' ')[0]}! 👋`;
        document.getElementById('welcomeBranch').innerHTML = `<span>📍</span> <span>${isAdmin ? 'Tüm Şubeler' : primaryBranch}</span>`;
        
        // Shift bilgisi
        let shiftInfo = isAdmin ? 'Yönetici' : '-';
        if (!isAdmin) {
            try {
                const weekStart = getWeekStart(today);
                const weekKey = formatDateLocal(weekStart);  // Timezone-safe
                const shiftDoc = await db.collection('shiftPlans').doc(`${primaryBranch}_${weekKey}`).get();
                if (shiftDoc.exists) {
                    const plan = shiftDoc.data();
                    const dayPlan = plan[todayStr];
                    if (dayPlan) {
                        for (const [shiftType, personnel] of Object.entries(dayPlan)) {
                            if (personnel && personnel.includes(currentUser?.id)) {
                                const shiftNames = { acilis: 'Açılış', kapanis: 'Kapanış', ara: 'Ara Vardiya' };
                                shiftInfo = shiftNames[shiftType] || shiftType;
                                break;
                            }
                        }
                    }
        ...
        document.getElementById('welcomeShift').innerHTML = `<span>⏰</span> <span>${shiftInfo}</span>`;
        
        welcomeCard.style.display = 'block';
    }
    
    // Bu ay puan özeti (sadece yönetici değilse)
    if (monthlyCard && currentUser?.id && !isAdmin) {
        try {
            const monthKey = todayStr.substring(0, 7);
            const startDate = monthKey + '-01';
            const endDate = monthKey + '-31';
            
            let manuelPuan = 0, ihlalSayisi = 0, basariSayisi = 0;
            
            const puantajSnapshot = await db.collection('puantaj')
                .where('personelId', '==', currentUser.id)
            ...
            puantajSnapshot.forEach(doc => {
                const d = doc.data();
                const yildiz = d.score || 3;
                if (yildiz <= 3) { manuelPuan -= (4 - yildiz); ihlalSayisi++; }
                else { manuelPuan += (yildiz - 3); basariSayisi++; }
            });
            
            let checkPuan = 0, checkVaktinde = 0, checkGec = 0;
            ...
            notesSnapshot.forEach(doc => {
                const n = doc.data();
                checkPuan += n.points || 0;
                if (n.points > 0) checkVaktinde++;
                else if (n.points < 0) checkGec++;
            });
            
            const toplamPuan = manuelPuan + checkPuan;
            const puanColor = toplamPuan >= 0 ? '#27ae60' : '#e74c3c';
            
            document.getElementById('monthlyPointsTotal').innerHTML = `<span style="color:${puanColor}">${toplamPuan >= 0 ? '+' : ''}${toplamPuan}</span>`;
            document.getElementById('monthlyPointsDetails').innerHTML = `
                <div style="background:rgba(39,174,96,.1);border-radius:10px;padding:12px;text-align:center">
                    <div style="font-size:1.3rem;font-weight:700;color:#27ae60">${checkVaktinde + basariSayisi}</div>
                    <div style="font-size:.7rem;color:#27ae60;font-weight:500">BAŞARI</div>
                </div>
                <div style="background:rgba(231,76,60,.1);border-radius:10px;padding:12px;text-align:center">
                    <div style="font-size:1.3rem;font-weight:700;color:#e74c3c">${checkGec + ihlalSayisi}</div>
                    <div style="font-size:.7rem;color:#e74c3c;font-weight:500">İHLAL</div>
                </div>
                <div style="background:var(--bg3);border-radius:10px;padding:12px;text-align:center">
                    <div style="font-size:1.3rem;font-weight:700;color:var(--tx)">${checkVaktinde}</div>
                    <div style="font-size:.7rem;color:var(--tx2);font-weight:500">VAKTİNDE CHECK</div>
                </div>
            `;
            monthlyCard.style.display = 'block';
        } catch (e) {
            console.warn('Aylık puan yüklenemedi:', e);
        }
    } else if (monthlyCard) {
        monthlyCard.style.display = 'none';
    }
    
    // Shift planlarını yükle - SADECE BU HAFTANIN SHİFT'İ
    const shiftData = {};
    ...
    try {
        ...
        // Günün index'i (Pazartesi = 0, Pazar = 6)
        let dayIndex = today.getDay() - 1;
        if (dayIndex < 0) dayIndex = 6;
        
        // Her şube için shift'i yükle
        // Önce personel koleksiyonunu yükle (tam isim eşleştirme için)
        const personelMap = {};
        try {
            const pSnap = await db.collection('personel').get();
            pSnap.forEach(doc => {
                ...
                // Tüm olası key formatlarını oluştur
                const keys = [
                    p.uniqueId,
                    p.codeName,
                    p.name,
                ...
                keys.forEach(k => {
                    ...
                    if (k?.toLowerCase) personelMap[k.toLowerCase()] = p.name;
                });
            });
            console.log('[Shift] personelMap oluşturuldu:', Object.keys(personelMap).length, 'key');
        } catch (e) {
            console.warn('Personel map yüklenemedi:', e);
        }
        ...
        for (const branch of branches) {
            try {
                ...
                // SADECE bu haftanın shift'ini kullan
                ...
                if (targetShift) {
                    ...
                    for (const [uniqueIdOrName, days] of Object.entries(personelShifts)) {
                        ...
                        // Tam ismi al: personelFullNames → personelMap → key (eski veriler için)
                        const fullName = personelFullNames[uniqueIdOrName] 
                            || personelMap[uniqueIdOrName] 
                            || personelMap[uniqueIdOrName?.toLowerCase()]
                            || uniqueIdOrName;
                        
                        if (cleanShift === 'A' || cleanShift === 'AÇILIŞ' || cleanShift.includes('AÇILI')) {
                            acilisPersonel.push(fullName);
                        } else if (cleanShift === 'K' || cleanShift === 'KAPANIŞ' || cleanShift.includes('KAPANI')) {
                            kapanisPersonel.push(fullName);
                        } else if (cleanShift && cleanShift !== 'İ' && cleanShift !== 'OFF' && cleanShift !== '-' && cleanShift !== 'X') {
                            araPersonel.push(fullName);
                        }
                    }
        ...
        console.warn('Shift yükleme hatası:', e);
    }
    
    // Shift'teki personel isimlerini al (tam isim döner)
    ...
    // Her şube için checkleri topla ve sırala
    for (const branch of branches) {
        const checkTypes = CHECKLIST_TYPES[branch] || [];
        const isMoonberry = branch.includes('Şişli') || branch.includes('Merkez');
        const brandColor = isMoonberry ? '#008c95' : '#ff8674';
        
        // Check'leri diziye topla
        const checksArray = [];
        
        // Check türlerini döngüle
        for (const checkType of checkTypes) {
            // Saat bazlı checkler (Günlük)
            if (checkType.timeSlots && checkType.timeSlots.length > 0) {
                for (const timeSlot of checkType.timeSlots) {
                    ...
                    // Yönetici hepsini görür, normal personel sadece aktif olanları
                    if (!isAdmin && window.isMissed) continue;
                    if (!isAdmin && window.isBeforeWindow) continue;
                    ...
                    const status = getCheckStatus(checkType.id, timeSlot, branch, isCompleted, submissionData?.submittedAt?.toDate());
                    
                    // Shift'ten sorumlu personeli bul
                    let responsiblePersonnel = [];
                    const checkHour = parseInt(timeSlot.split(':')[0]);
                    if (checkHour < 15) {
                        responsiblePersonnel = getShiftPersonnel(branch, 'acilis');
                    } else if (checkHour >= 20) {
                        responsiblePersonnel = getShiftPersonnel(branch, 'kapanis');
                    } else {
                        responsiblePersonnel = [...getShiftPersonnel(branch, 'acilis'), ...getShiftPersonnel(branch, 'ara')];
                    }
                    
                    // Sıralama için priority belirle (düşük = önce göster)
                    let priority = 50;
                    let sortMinutes = timeToMinutes(timeSlot);
                    
                    if (isCompleted) {
                        priority = 90; // Tamamlanan en altta
                    } else if (window.isMissed) {
                        priority = 80; // Yapılmayan altta
                    } else if (window.isLate) {
                        priority = 20; // Geç - acil
                    } else if (window.isInWindow) {
                        priority = 10; // Aktif pencerede - en acil
                    } else if (window.isBeforeWindow) {
                        priority = 30; // Henüz başlamamış
                    }
                    ...
                    // Yapılmayan check için otomatik puan işle
                    if (window.isMissed && !isCompleted && responsiblePersonnel.length > 0) {
                        processMissedChecks(branch, checkType.id, timeSlot, todayStr, responsiblePersonnel);
                    }
                }
            }
            
            // Açılış Check (sabah 11:00'e kadar)
            if (checkType.id === 'acilis') {
                const deadline = '11:00';
                ...
                const isActive = currentMinutes <= deadlineMinutes;
                
                // Sadece ilgili durumları göster
                if (!isAdmin && isMissed) continue;
                if (!isAdmin && !isActive && !isCompleted) continue;
                ...
                const remainingText = isActive && !isCompleted && remainingMinutes > 0 ? 
                    (remainingMinutes > 60 ? `${Math.floor(remainingMinutes / 60)} saat ${remainingMinutes % 60} dk` : `${remainingMinutes} dk`) : '';
                
                const rules = CHECK_RULES.acilis;
                html += renderSimpleCheckCard(checkType.name, deadline, branch, isCompleted, isMissed, isActive, remainingText, responsiblePersonnel, rules, 'sunrise');
            }
            
            // Kapanış Check (gece yarısına kadar)
            if (checkType.id === 'kapanis') {
                ...
                const now = new Date();
                const currentHour = now.getHours();
                // Kapanış: saat 18'den sonra aktif
                const isActive = currentHour >= 18;
                const isMissed = false; // Gece yarısına kadar
                ...
                if (isActive || isCompleted) {
                    ...
                    const remainingText = isActive && !isCompleted ? `${remainingHours} saat` : '';
                    
                    const rules = CHECK_RULES.kapanis;
                    html += renderSimpleCheckCard(checkType.name, 'Kapanış', branch, isCompleted, isMissed, isActive, remainingText, responsiblePersonnel, rules, 'sunset');
                }
            }
            
            // Temizlik ve Bakım (aylik) - Sabah/Akşam vardiyasına göre
            if (checkType.id === 'aylik') {
                const now = new Date();
                const currentHour = now.getHours();
                const dayOfMonth = now.getDate();
                
                // Sabah vardiyası: 10:00-16:00 arası
                // Akşam vardiyası: 20:00-02:00 arası
                const isSabah = currentHour >= 9 && currentHour < 16;
                const isAksam = currentHour >= 20 || currentHour < 2;
                
                if (isSabah || isAksam) {
                    const shiftType = isSabah ? 'sabah' : 'aksam';
                    const submissionId = `${branch.replace(/[^a-zA-Z0-9]/g, '_')}_aylik_${todayStr}_${shiftType}`;
                    let isCompleted = false;
                    ...
                    const responsiblePersonnel = getShiftPersonnel(branch, isSabah ? 'acilis' : 'kapanis');
                    const deadline = isSabah ? '16:00' : '02:00';
                    
                    hasActiveChecks = true;
                    const rules = CHECK_RULES.aylik;
                    html += renderSimpleCheckCard(`Temizlik (${isSabah ? 'Sabah' : 'Akşam'})`, deadline, branch, isCompleted, false, true, '', responsiblePersonnel, rules, 'calendar');
                }
            }
            
            // Müdür Kontrol (managerOnly)
            if (checkType.managerOnly) {
                const rules = CHECK_RULES.mudur || CHECK_RULES[checkType.id];
                const deadline = rules?.deadline || '21:00';
                
                // Submission kontrolü
                const submissionId = `${branch.replace(/[^a-zA-Z0-9]/g, '_')}_${checkType.id}_${todayStr}`;
                let isCompleted = false;
                let submissionData = null;
                ...
                if (isActive && !isCompleted) {
                    ...
                    if (remainingMinutes > 60) {
                        remainingText = `${Math.floor(remainingMinutes / 60)} saat ${remainingMinutes % 60} dk`;
                    } else {
                ...
                const isMoonberry = branch.includes('Şişli') || branch.includes('Merkez');
                let bgColor = 'var(--bg3)';
                let borderColor = 'var(--cardBorder)';
                let icon = '👔';
                ...
                if (isCompleted) {
                    ...
                    icon = '👔';
                    statusText = 'Aktif';
                    cardClass = 'check-card-active';
                }
                
                hasActiveChecks = true;
                html += `
                    <div class="${cardClass}" style="background:${bgColor};border:1px solid ${borderColor};border-radius:10px;padding:10px 14px;display:flex;align-items:center;justify-content:space-between;gap:8px;cursor:pointer;position:relative;overflow:hidden" onclick="goTo('checklist')">
                        ${isActive ? '<div class="check-card-shimmer" style="position:absolute;inset:0;pointer-events:none"></div>' : ''}
                        <div style="display:flex;align-items:center;gap:10px;position:relative">
                            <div style="font-size:1.2rem">${icon}</div>
                            <div>
                                <div style="font-weight:600;font-size:.9rem;color:var(--tx)">${checkType.name}</div>
                                <span style="font-size:.7rem;color:var(--tx3)">${deadline}'a kadar</span>
                            </div>
                        </div>
                        <div style="text-align:right;position:relative">
                            <div style="font-size:.8rem;font-weight:600;color:${borderColor}">${statusText}</div>
                            ${remainingText ? `<div style="font-size:.7rem;color:var(--tx3)">⏱️ ${remainingText}</div>` : ''}
                        </div>
                    </div>
                `;
            }
        ...
        if (branches.length > 1) {
            const isMoonberryBranch = branch.includes('Şişli') || branch.includes('Merkez');
            const brandBorderColor = isMoonberryBranch ? '#008c95' : '#ff8674';
            html += `
                <div style="display:flex;align-items:center;gap:8px;margin:16px 0 10px 0;padding-bottom:8px;border-bottom:2px solid ${brandBorderColor}">
                    <div style="width:8px;height:8px;background:${brandBorderColor};border-radius:50%"></div>
                    <span style="font-size:.9rem;font-weight:600;color:var(--tx)">${branch}</span>
                </div>
            `;
        }
        ...
        // Aktif platform check saatini bul
        let activePlatformTime = null;
        let nextPlatformTime = null;
        ...
        for (const timeStr of platformTimes) {
            ...
            // ±30 dakika penceresi
            if (Math.abs(diff) <= 30) {
                activePlatformTime = timeStr;
                platformRemainingMinutes = 30 - Math.abs(diff);
            } else if (diff > 0 && !nextPlatformTime) {
                nextPlatformTime = timeStr;
            }
        }
        ...
        const isMoonberryBranch = branch.includes('Şişli') || branch.includes('Merkez');
        const platformBrandColor = isMoonberryBranch ? '#008c95' : '#ff8674';
        const platformBrandBg = isMoonberryBranch ? 'rgba(0,140,149,.06)' : 'rgba(255,134,116,.06)';
        
        const platformPersonnel = [
            ...getShiftPersonnel(branch, 'acilis'),
            ...getShiftPersonnel(branch, 'ara'),
            ...getShiftPersonnel(branch, 'kapanis')
        ].filter((v, i, a) => a.indexOf(v) === i);
        
        const platformPersonnelText = platformPersonnel.length > 0 ? platformPersonnel.slice(0, 2).map(p => lookupPersonelDisplay(p, branch)).join(', ') : '';
        
        // Platform check durumu
        let platformStatus = '', platformStatusColor = '', platformBg = '';
        if (platformCompleted) {
            platformStatus = 'Tamam';
            platformStatusColor = '#27ae60';
            platformBg = 'rgba(39,174,96,.06)';
        } else if (activePlatformTime) {
            platformStatus = `${platformRemainingMinutes}dk`;
            platformStatusColor = platformBrandColor;
            platformBg = platformBrandBg;
        } else if (nextPlatformTime) {
            platformStatus = nextPlatformTime;
            platformStatusColor = 'var(--tx3)';
            platformBg = 'var(--bg3)';
        }
        
        hasActiveChecks = true;
        html += `
            <div class="${activePlatformTime && !platformCompleted ? 'check-pulse-active' : ''}" style="background:${platformBg};border-left:3px solid ${platformCompleted ? '#27ae60' : platformBrandColor};border-radius:8px;padding:10px 14px;display:flex;align-items:center;gap:10px;cursor:pointer;transition:all .15s;opacity:${platformCompleted ? '0.6' : '1'}" onclick="goTo('checklist'); setTimeout(() => { document.querySelector('[data-type=platform]')?.click(); }, 100);" onmouseover="this.style.transform='translateX(4px)'" onmouseout="this.style.transform='none'">
                <svg width="16" height="16" fill="none" stroke="${platformCompleted ? '#27ae60' : platformBrandColor}" stroke-width="2" viewBox="0 0 24 24"><rect x="5" y="2" width="14" height="20" rx="2"/><line x1="12" y1="18" x2="12.01" y2="18"/></svg>
                <span style="font-weight:500;font-size:.85rem;color:var(--tx)">Platform</span>
                ${activePlatformTime ? `<span style="font-size:.7rem;color:var(--tx3)">${activePlatformTime}</span>` : ''}
                <span style="font-size:.7rem;color:var(--tx3);margin-left:auto">${platformPersonnelText}</span>
                ${!platformCompleted && activePlatformTime ? `<span style="font-size:.65rem;color:#27ae60;font-weight:600">-1</span>` : ''}
                <span style="font-size:.75rem;font-weight:600;color:${platformStatusColor};padding:4px 10px;background:${platformCompleted ? 'rgba(39,174,96,.12)' : platformBrandBg};border-radius:6px">${platformStatus}</span>
            </div>
        `;
        
        // ============== TEMİZLİK CHECK ==============
        // Her zaman göster - gün içinde sabah, akşam/gece akşam
        // currentHour zaten yukarıda tanımlı (satır 11153)
        // 02:00-15:59 arası sabah, 16:00-01:59 arası akşam
        const isSabahTemizlik = currentHour >= 2 && currentHour < 16;
        const shiftType = isSabahTemizlik ? 'sabah' : 'aksam';
        const temizlikSubmissionId = `${branch.replace(/[^a-zA-Z0-9]/g, '_')}_aylik_${todayStr}_${shiftType}`;
        ...
        const temizlikPersonnel = getShiftPersonnel(branch, isSabahTemizlik ? 'acilis' : 'kapanis');
        const temizlikPersonnelText = temizlikPersonnel.length > 0 ? temizlikPersonnel.slice(0, 2).map(p => lookupPersonelDisplay(p, branch)).join(', ') : '';
        const temizlikDeadline = isSabahTemizlik ? '15:00' : '02:00';
        
        html += `
            <div style="background:${temizlikCompleted ? 'rgba(39,174,96,.06)' : 'var(--bg3)'};border-left:3px solid ${temizlikCompleted ? '#27ae60' : isMoonberryBranch ? '#008c95' : '#ff8674'};border-radius:8px;padding:10px 14px;display:flex;align-items:center;gap:10px;cursor:pointer;transition:all .15s;opacity:${temizlikCompleted ? '0.6' : '1'}" onclick="goTo('checklist'); setTimeout(() => { selectChecklistType('aylik'); }, 200);" onmouseover="this.style.transform='translateX(4px)'" onmouseout="this.style.transform='none'">
                <svg width="16" height="16" fill="none" stroke="${temizlikCompleted ? '#27ae60' : isMoonberryBranch ? '#008c95' : '#ff8674'}" stroke-width="2" viewBox="0 0 24 24"><path d="M3 6l3 1 2-3 2 3 3-1-1 3 3 2-3 2 1 3-3-1-2 3-2-3-3 1 1-3-3-2 3-2-1-3z"/></svg>
                <span style="font-weight:500;font-size:.85rem;color:var(--tx)">Temizlik Çizelgesi</span>
                <span style="font-size:.7rem;color:var(--tx3)">${isSabahTemizlik ? 'Sabah' : 'Akşam'}</span>
                <span style="font-size:.7rem;color:var(--tx3);margin-left:auto">${temizlikPersonnelText}</span>
                <span style="font-size:.75rem;font-weight:600;color:${temizlikCompleted ? '#27ae60' : 'var(--tx2)'};padding:4px 10px;background:${temizlikCompleted ? 'rgba(39,174,96,.12)' : 'var(--bg4)'};border-radius:6px">${temizlikCompleted ? 'Tamam' : temizlikDeadline + "'e kadar"}</span>
            </div>
        `;
    }
...
// Tek container'daki kartları sırala
function sortCheckCards(container) {
    const cards = Array.from(container.children).filter(el => 
        el.style.borderLeft || (el.style.border && !el.textContent.includes('Şişli') && !el.textContent.includes('Tuzla'))
    );
...
// Kart önceliğini belirle
function getCardPriority(card) {
    const text = card.textContent.toLowerCase();
    const style = card.getAttribute('style') || '';
    
    // Tamamlandı/Tamam - en altta
    if (text.includes('tamam') || text.includes('tamamlandı') || style.includes('#27ae60')) {
        return 90;
    }
    // Yapılmadı - altta
    if (text.includes('yapılmadı') || text.includes('süre doldu') || style.includes('#e74c3c') || style.includes('#9ca3af')) {
        return 80;
    }
    // Geç - orta
    if (text.includes('geç') || style.includes('#f39c12')) {
        return 20;
    }
    // Aktif - en üstte
...
// Basit check kartı render (Açılış, Kapanış, Temizlik için)
function renderSimpleCheckCard(name, deadline, branch, isCompleted, isMissed, isActive, remainingText, responsiblePersonnel, rules, iconType) {
    ...
    if (responsiblePersonnel && responsiblePersonnel.length > 0 && !isCompleted) {
        // lookupPersonelDisplay: codeName varsa onu, yoksa formatDisplayName kullanır
        const names = responsiblePersonnel.slice(0, 2).map(p => lookupPersonelDisplay(p)).join(', ');
        personnelHtml = `<span style="font-size:.7rem;color:var(--tx3);margin-left:auto">${names}</span>`;
    }
    
    // Puan gösterimi (aktif ise)
    let pointsHtml = '';
...
// Check kartı render helper - kompakt versiyon
function renderCheckCard(checkType, timeSlot, branch, isCompleted, submissionData, window, remaining, status, responsiblePersonnel) {
    // Sabit renkler - duruma göre
    const TEAL = '#008c95';      // Moonberry - Aktif
    const CORAL = '#ff8674';     // Sunberry - Yapılmadı
    const ORANGE = '#f39c12';    // Turuncu - Geç
    const GRAY = '#9ca3af';      // Gri - Tamamlandı
    const GREEN = '#27ae60';     // Yeşil - Başarılı
    ...
    if (isCompleted) {
        ...
        isActive = true;
        pulseClass = 'check-pulse-active';
        riskText = `Tamamla → +${onTimePoints}`;
    }
    
    // Sorumlu personel butonları - ADİL DAĞITIM
    let personnelHtml = '';
    if (responsiblePersonnel && responsiblePersonnel.length > 0 && !isCompleted) {
        if (responsiblePersonnel.length === 1) {
            personnelHtml = `<span style="font-size:.7rem;color:var(--tx2);margin-left:4px">• ${lookupPersonelDisplay(responsiblePersonnel[0])}</span>`;
        } else {
            // Birden fazla personel - butonlarla seç
    ...
    // Kalan süre ve risk bilgisi - daha iyi görünüm
    let timeHtml = '';
    ...
    return `
        <div class="${pulseClass}" style="background:${bgColor};border-left:3px solid ${accentColor};border-radius:8px;padding:10px 14px;display:flex;align-items:center;justify-content:space-between;cursor:pointer;margin-bottom:8px;transition:all .2s;opacity:${opacity}" onclick="goTo('checklist')">
            <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
...
async function loadAllChecksOverview() {
    ...
    container.innerHTML = '<div style="text-align:center;padding:40px;color:var(--tx2)"><div class="loading-spinner" style="margin:0 auto 16px"></div>Yükleniyor...</div>';
    ...
    try {
        ...
        // Şubeleri belirle
        const branches = branchFilter === 'all' ? (STATE.branches || ['Tuzla Port', 'Şişli (Merkez)']) : [branchFilter];
        
        let allSubmissions = [];
        let expectedChecks = [];
        
        // Her şube için beklenen checkleri hesapla
        for (const branch of branches) {
            const checkTypes = CHECKLIST_TYPES[branch] || [];
        ...
        html += `
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:16px;margin-bottom:24px">
                <div style="background:rgba(39,174,96,.1);border:1px solid rgba(39,174,96,.2);border-radius:12px;padding:20px;text-align:center">
                    ...
                    <div style="font-size:.8rem;color:#27ae60;font-weight:500">Tamamlandı</div>
                </div>
                ...
                <div style="background:rgba(231,76,60,.1);border:1px solid rgba(231,76,60,.2);border-radius:12px;padding:20px;text-align:center">
                    ...
                    <div style="font-size:.8rem;color:#e74c3c;font-weight:500">Yapılmadı</div>
                </div>
        ...
        html += `
            <div style="overflow-x:auto">
                <table style="width:100%;border-collapse:collapse;font-size:.85rem">
                    <thead>
                        <tr style="background:var(--bg3)">
                            <th style="padding:12px;text-align:left;border-bottom:1px solid var(--cardBorder)">Şube</th>
                            ...
                            <th style="padding:12px;text-align:center;border-bottom:1px solid var(--cardBorder)">İşlem</th>
                        </tr>
                    </thead>
                    <tbody>
        `;
        ...
        expectedChecks.forEach(expected => {
            ...
            if (submission) {
                ...
                const window = getCheckTimeWindow(expected.type, expected.timeSlot, expected.branch);
                if (window && window.isMissed) {
                    statusBadge = '❌ Yapılmadı';
                    statusColor = '#e74c3c';
                } else if (window && window.isLate) {
                    statusBadge = '⚠️ Geç Pencere';
                    statusColor = '#f39c12';
            ...
            const pointsDisplay = points !== 0 ? `<span style="color:${points > 0 ? '#27ae60' : '#e74c3c'};font-weight:600">${points > 0 ? '+' : ''}${points}</span>` : '-';
        ...
        if (expectedChecks.length === 0) {
            html = '<div style="text-align:center;padding:60px;color:var(--tx2)">Bu dönem için check bulunamadı.</div>';
        }
        
        container.innerHTML = html;
        
    } catch (error) {
        console.error('Checkler yüklenirken hata:', error);
        container.innerHTML = '<div style="text-align:center;padding:40px;color:#e74c3c">Yükleme hatası!</div>';
    }
}

// Manuel check ekleme modal'ı
function showManualCheckModal(branch, type, timeSlot, date) {
        ...
        <div style="padding:8px 0">
            ...
            <div class="form-group">
                ...
                <select id="manualCheckStatus" class="form-select">
                    <option value="onTime">✅ Vaktinde</option>
                    <option value="late">⏰ Geç</option>
                    <option value="missed">❌ Yapılmadı</option>
                </select>
            </div>
    ...
    `, `
        <button class="btn btn-ghost" onclick="closeModal()">İptal</button>
        <button class="btn btn-primary" onclick="saveManualCheck('${branch}', '${type}', '${timeSlot}', '${date}')">💾 Kaydet</button>
    `);
...
async function saveManualCheck(branch, type, timeSlot, date) {
    ...
    if (!personelId) {
        toast('Lütfen personel seçin', 'error');
        return;
    }
    ...
    try {
        ...
        await db.collection('checklistSubmissions').doc(submissionId).set({
            ...
            pointStatus: status,
            manualEntry: true,
            manualNote: note || 'Yönetici tarafından manuel eklendi',
            items: []
        });
        
        // Puantaj notu oluştur
        if (points !== 0) {
...
async function viewCheckDetails(submissionId) {
    try {
                ...
                <div style="margin-top:16px;max-height:300px;overflow-y:auto">
                    ...
                    ${data.items.map(item => `
                        <div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--bg3);font-size:.85rem">
                            <span style="color:${item.checked ? '#27ae60' : '#e74c3c'}">${item.checked ? '✓' : '✗'}</span>
                            <span>${item.id}</span>
                        </div>
                    `).join('')}
                </div>
        ...
        showModal('📋 Check Detayı', `
            <div style="padding:8px 0">
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px">
                    <div>
                        <div style="font-size:.75rem;color:var(--tx2);margin-bottom:4px">Şube</div>
                    ...
                    <div>
                        <div style="font-size:.75rem;color:var(--tx2);margin-bottom:4px">Gönderim Zamanı</div>
                ...
                ${data.manualEntry ? `
                    <div style="background:rgba(243,156,18,.1);border:1px solid rgba(243,156,18,.3);border-radius:8px;padding:12px;margin-bottom:16px">
                        <div style="font-size:.8rem;color:#f39c12;font-weight:600">⚠️ Manuel Ekleme</div>
            ...
            <div style="background:var(--bg3);border-radius:12px;padding:20px;border-left:4px solid ${brandColor}">
                ...
                <div style="margin-bottom:12px">
                    <label style="display:flex;align-items:center;gap:8px;cursor:pointer">
                        ...
                        <span style="font-size:.85rem;color:var(--tx2)">Tüm günler aynı kapanış saati</span>
                    </label>
                </div>
                
                <div id="sameClosing_${branch.replace(/\s/g, '_')}" style="${hours.closing.monday ? 'display:none' : ''}">
                    <div class="form-group">
                        <label class="form-label">Kapanış Saati (Tüm Günler)</label>
                ...
                <div id="dailyClosing_${branch.replace(/\s/g, '_')}" style="${!hours.closing.monday ? 'display:none' : ''}">
                    <label class="form-label" style="margin-bottom:8px">Günlük Kapanış Saatleri</label>
...
async function saveBranchHours() {
    ...
    try {
        ...
        toast('Mağaza saatleri kaydedildi', 'success');
    } catch (e) {
...
function renderCheckRulesSettings() {
    const container = document.getElementById('checkRulesContainer');
    if (!container) return;
    
    const defaultRules = {
        gunluk: { name: 'Günlük Check', window: { before: 60, after: 60 }, lateWindow: 60, points: { onTime: 1, late: -2, missed: -2 } },
        platform: { name: 'Platform Check', window: { before: 60, after: 60 }, lateWindow: 0, points: { onTime: 0, late: 0, missed: -1 } },
        mudur: { name: 'Müdür Kontrol', deadline: '21:00', points: { onTime: 1, late: -1, missed: -2 } },
        temizlik: { name: 'Temizlik', windows: { sabah: { start: '10:00', end: '16:00' }, kapanis: { start: '00:00', end: '02:00' } }, points: { onTime: 0, late: -1, missed: -1 } },
        haftalik: { name: 'Haftalık Görevler', points: { done: 0, missed: -1 } }
    };
    
    const rules = { ...defaultRules, ...CHECK_RULES };
    ...
    Object.entries(rules).forEach(([key, rule]) => {
        html += `
            <div style="background:var(--bg3);border-radius:12px;padding:16px">
                <div style="font-weight:600;color:var(--tx);margin-bottom:12px;display:flex;align-items:center;gap:8px">
                    <span style="background:var(--accent);color:#fff;padding:2px 8px;border-radius:4px;font-size:.7rem">${key.toUpperCase()}</span>
                    ${rule.name}
                </div>
                ...
                <div class="form-grid" style="gap:12px">
                    ${rule.window ? `
                        <div class="form-group">
                            <label class="form-label" style="font-size:.75rem">Pencere Önce (dk)</label>
                            <input type="number" class="form-input" id="rule_${key}_before" value="${rule.window.before}" min="0" max="180">
                        </div>
                        <div class="form-group">
                            <label class="form-label" style="font-size:.75rem">Pencere Sonra (dk)</label>
                            <input type="number" class="form-input" id="rule_${key}_after" value="${rule.window.after}" min="0" max="180">
                        </div>
                        <div class="form-group">
                            <label class="form-label" style="font-size:.75rem">Geç Penceresi (dk)</label>
                            <input type="number" class="form-input" id="rule_${key}_lateWindow" value="${rule.lateWindow || 0}" min="0" max="180">
                            <div style="font-size:.65rem;color:var(--tx3);margin-top:2px">0 = geç doldurulamaz</div>
                        </div>
                    ` : ''}
                    ${rule.deadline ? `
                        <div class="form-group">
                            <label class="form-label" style="font-size:.75rem">Son Saat</label>
                            <input type="time" class="form-input" id="rule_${key}_deadline" value="${rule.deadline}">
                        </div>
                    ` : ''}
                </div>
                
                <div style="margin-top:12px;padding-top:12px;border-top:1px solid var(--cardBorder)">
                    <div style="font-size:.75rem;font-weight:600;color:var(--tx2);margin-bottom:8px">Puanlama</div>
                    <div style="display:flex;gap:12px;flex-wrap:wrap">
                        <div style="display:flex;align-items:center;gap:6px">
                            <span style="font-size:.75rem;color:var(--tx2)">Vaktinde:</span>
                            <input type="number" class="form-input" style="width:60px;padding:4px 8px;font-size:.85rem" id="rule_${key}_onTime" value="${rule.points.onTime || 0}">
                        </div>
                        ${rule.points.late !== undefined ? `
                            <div style="display:flex;align-items:center;gap:6px">
                                <span style="font-size:.75rem;color:var(--tx2)">Geç:</span>
                                <input type="number" class="form-input" style="width:60px;padding:4px 8px;font-size:.85rem" id="rule_${key}_late" value="${rule.points.late || 0}">
                            </div>
                        ` : ''}
                        <div style="display:flex;align-items:center;gap:6px">
                            <span style="font-size:.75rem;color:var(--tx2)">Yapılmadı:</span>
                            <input type="number" class="form-input" style="width:60px;padding:4px 8px;font-size:.85rem" id="rule_${key}_missed" value="${rule.points.missed || -1}">
                        </div>
                    </div>
                </div>
            </div>
...
async function saveCheckRules() {
    const keys = ['gunluk', 'platform', 'mudur', 'temizlik', 'haftalik'];
    const checkRules = {};
    
    keys.forEach(key => {
        const rule = { name: CHECK_RULES[key]?.name || key, points: {} };
        
        // Window
        const before = document.getElementById(`rule_${key}_before`)?.value;
        const after = document.getElementById(`rule_${key}_after`)?.value;
        if (before !== undefined && after !== undefined) {
            rule.window = { before: parseInt(before), after: parseInt(after) };
            rule.lateWindow = parseInt(document.getElementById(`rule_${key}_lateWindow`)?.value || 0);
        }
        
        // Deadline
        const deadline = document.getElementById(`rule_${key}_deadline`)?.value;
        if (deadline) rule.deadline = deadline;
        
        // Points
        rule.points.onTime = parseInt(document.getElementById(`rule_${key}_onTime`)?.value || 0);
        const late = document.getElementById(`rule_${key}_late`)?.value;
        if (late !== undefined) rule.points.late = parseInt(late);
        rule.points.missed = parseInt(document.getElementById(`rule_${key}_missed`)?.value || -1);
    ...
    try {
        ...
        toast('Check kuralları kaydedildi', 'success');
    } catch (e) {
        console.error('Kayıt hatası:', e);
        toast('Kayıt hatası!', 'error');
    }
...
async function loadSystemConfig() {
    try {
        ...
        if (sysDoc.exists) {
            ...
            CHECK_RULES = SYSTEM_CONFIG.checkRules || {};
            SHIFT_DURATION = SYSTEM_CONFIG.shiftDuration || 9;
        }
        
        // ====== CHECK SYSTEM SETTINGS YÜKLE ======
        try {
            ...
            if (checkDoc.exists) {
                ...
                // Check sistem ayarlarını yükle ve CHECK_RULES'a uygula
                if (checkData.checkSystemSettings) {
                    const settings = checkData.checkSystemSettings;
                    window.CHECK_SYSTEM_SETTINGS = settings;
                    
                    // Puanları CHECK_RULES'a uygula
                    if (settings.points) {
                        // Platform puanları
                        if (CHECK_RULES.platform) {
                    ...
                    console.log('[Config] Check sistem ayarları yüklendi:', settings);
                }
            ...
            console.warn('[Config] Check ayarları yüklenemedi:', checkErr.message);
        }
        ...
        if (typesDoc.exists) {
            ...
            // updatedAt gibi meta alanları filtrele
            CHECKLIST_TYPES = {};
        ...
        // Temizlik Çizelgesi tipi yoksa ekle (her şube için)
        const branches = STATE.branches || ['Tuzla Port', 'Şişli (Merkez)'];
        branches.forEach(branch => {
            ...
            // Temizlik tipi var mı kontrol et
            const hasTemizlik = CHECKLIST_TYPES[branch].some(t => t.id === 'aylik');
            if (!hasTemizlik) {
                CHECKLIST_TYPES[branch].push({
                    id: 'aylik',
                    name: 'Temizlik Çizelgesi',
                    icon: 'calendar',
                    description: 'Günlük temizlik ve bakım maddeleri',
                    managerOnly: false
                });
                console.log(`✅ ${branch} için Temizlik Çizelgesi eklendi`);
            }
        });
        
        // Personel eşleştirmesi
        const persDoc = await db.collection('config').doc('personnel').get();
        if (persDoc.exists) {
            personnelMapping = persDoc.data().list || [];
        }
        
        console.log('Sistem config yüklendi:', {
        ...
        // Fallback - default checklist türleri
        const branches = STATE.branches || ['Tuzla Port', 'Şişli (Merkez)'];
        branches.forEach(branch => {
            if (!CHECKLIST_TYPES[branch]) {
                CHECKLIST_TYPES[branch] = [
                    { id: 'aylik', name: 'Temizlik Çizelgesi', icon: 'calendar', description: 'Günlük temizlik ve bakım' }
                ];
            }
        });
    }
...
// Personel eşleştirmesini Firebase'den yükle (eski uyumluluk için)
...
// Kullanıcının şubesini email'den al
...
// Kullanıcının adını email'den al
function getUserNameFromEmail(email) {
    const match = personnelMapping.find(p => p.email?.toLowerCase() === email?.toLowerCase());
    return match ? match.name : null;
}

// Checklist sayfası başlat
async function initChecklistPage() {
    ...
    // Mağaza müdürü/barista sadece kendi şubesini görsün ve değiştiremesin
    if (!isAdmin && userBranch && userBranch !== 'all') {
        branches = branches.filter(b => b === userBranch);
        subeSelect.disabled = true;
        subeSelect.style.opacity = '0.7';
        subeSelect.title = 'Şubeniz otomatik seçilmiştir';
    } else {
    ...
    // Kullanıcının şubesini seç veya default Tuzla Port
    ...
    // Yönetici butonlarını göster/gizle
    ...
    // Bugünkü checkler panelini yükle
    loadChecklistTodayPanel();
    
    // Checklist türlerini yükle
    loadChecklistTypes();
}

// Checklist sayfası - Bugünkü checkler paneli
async function loadChecklistTodayPanel() {
    ...
    // Shift verilerini yükle - SADECE BU HAFTANIN SHİFT'İ
    const shiftData = {};
    ...
    try {
        for (const branch of branches) {
            try {
                ...
                // JavaScript'te sırala
                const allShifts = [];
                shiftsSnapshot.forEach(doc => allShifts.push(doc.data()));
                allShifts.sort((a, b) => (b.weekStart || '').localeCompare(a.weekStart || ''));
                
                // SADECE bu haftanın shift'ini kullan - yoksa boş bırak
                const targetShift = allShifts.find(s => s.weekStart === weekKey);
                ...
                if (targetShift) {
                    ...
                    for (const [uniqueIdOrName, days] of Object.entries(personelShifts)) {
                        ...
                        if (cleanShift === 'A' || cleanShift === 'AÇILIŞ' || cleanShift.includes('AÇILI')) acilisPersonel.push(fullName);
                        else if (cleanShift === 'K' || cleanShift === 'KAPANIŞ' || cleanShift.includes('KAPANI')) kapanisPersonel.push(fullName);
                        else if (cleanShift && cleanShift !== 'İ' && cleanShift !== 'OFF' && cleanShift !== '-' && cleanShift !== 'X') araPersonel.push(fullName);
                    }
                    
                    shiftData[branch] = { acilis: acilisPersonel, kapanis: kapanisPersonel, ara: araPersonel };
                    console.log(`✅ ${branch} bu haftanın shift'i bulundu:`, weekKey);
                } else {
                    console.log(`⚠️ ${branch} için bu haftanın shift'i (${weekKey}) bulunamadı`);
                    shiftData[branch] = { acilis: [], kapanis: [], ara: [] };
                }
            } catch (e) {
                console.error(`${branch} shift yükleme hatası:`, e);
            }
        }
    } catch (e) {
        console.error('Shift yükleme hatası:', e);
        ...
        for (const checkType of checkTypes) {
            ...
            if (checkType.timeSlots && checkType.timeSlots.length > 0) {
                for (const timeSlot of checkType.timeSlots) {
                    ...
                    // Minimalist tasarım - marka renkleri
                    const brandBg = isMoonberry ? 'rgba(0,140,149,.06)' : 'rgba(255,134,116,.06)';
                    let bgColor = 'var(--bg3)', accentColor = 'var(--tx3)', icon = '○', statusText = '';
                    let isActive = false;
                    
                    if (isCompleted) { bgColor = 'rgba(39,174,96,.05)'; accentColor = '#27ae60'; icon = '●'; statusText = 'Tamam'; }
                    else if (window.isMissed) { bgColor = brandBg; accentColor = brandColor; icon = '○'; statusText = 'Yapılmadı'; }
                    else if (window.isLate) { bgColor = 'rgba(243,156,18,.05)'; accentColor = '#f39c12'; icon = '◐'; statusText = 'Geç'; isActive = true; }
                    else if (window.isInWindow) { bgColor = brandBg; accentColor = brandColor; icon = '●'; statusText = 'Aktif'; isActive = true; }
                    
                    const personnelText = responsiblePersonnel.length > 0 ? `• ${lookupPersonelDisplay(responsiblePersonnel[0])}` : '';
            ...
            if (checkType.id === 'acilis') {
                ...
                if (currentMinutes <= deadlineMinutes || isAdmin) {
                    ...
                    const statusText = isCompleted ? 'Tamam' : isMissed ? 'Yapılmadı' : 'Aktif';
                    ...
                    html += `
                        <div style="background:${bgColor};border-left:3px solid ${accentColor};border-radius:6px;padding:6px 10px;display:flex;align-items:center;justify-content:space-between;cursor:pointer;margin-bottom:4px;opacity:${isMissed ? '0.6' : '1'}" onclick="selectCheckFromPanel('${branch}', 'acilis', '')">
                            <div style="display:flex;align-items:center;gap:6px">
                                <span style="font-size:.8rem">🌅</span>
                                <span style="width:18px;height:18px;background:${brandColor};border-radius:4px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:.6rem;font-weight:600">${branchLabel}</span>
                                <span style="font-weight:500;font-size:.8rem;color:var(--tx)">Açılış</span>
                                <span style="font-size:.7rem;color:var(--tx3)">${personnelText}</span>
                            </div>
                            <span style="font-size:.7rem;font-weight:500;color:${accentColor}">${statusText}</span>
                        </div>
                    ...
                    html += `
                        <div style="background:${bgColor};border-left:3px solid ${accentColor};border-radius:6px;padding:6px 10px;display:flex;align-items:center;justify-content:space-between;cursor:pointer;margin-bottom:4px" onclick="selectCheckFromPanel('${branch}', 'kapanis', '')">
                            <div style="display:flex;align-items:center;gap:6px">
                                <span style="font-size:.8rem">🌇</span>
                                <span style="width:18px;height:18px;background:${brandColor};border-radius:4px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:.6rem;font-weight:600">${branchLabel}</span>
                                <span style="font-weight:500;font-size:.8rem;color:var(--tx)">Kapanış</span>
                                <span style="font-size:.7rem;color:var(--tx3)">${personnelText}</span>
                            </div>
                            <span style="font-size:.7rem;font-weight:500;color:${accentColor}">${statusText}</span>
                        </div>
...
async function loadChecklistTypes() {
    ...
    // Debug log
    console.log('Checklist türleri:', branch, types.length, 'adet');
    
    // Hidden select güncelle
    let selectHtml = '<option value="">-- Checklist Seçin --</option>';
    types.forEach(t => {
    ...
    // Butonları oluştur - Modern SVG ikonlar
    let buttonsHtml = '';
    types.forEach(t => {
        if (t.managerOnly && !isManager) return;
    ...
    if (!buttonsHtml) {
        buttonsHtml = '<div style="color:var(--tx3);font-size:.9rem;padding:10px">Bu şube için checklist bulunamadı. Lütfen seed.html ile verileri yükleyin.</div>';
    }
    ...
    document.getElementById('checklistFormContainer').innerHTML = `
        <div style="text-align:center;padding:60px;color:var(--tx2)">
            ...
            <div style="font-size:1.1rem;margin-bottom:8px">Checklist Seçin</div>
            <div style="font-size:.85rem;opacity:.6">Yukarıdan checklist türü seçerek başlayın</div>
        </div>
    `;
}

// Checklist türü seç (buton)
...
async function loadChecklist() {
    // Eşzamanlı yüklemeleri engelle (flickering fix)
    ...
    // Platform check için özel işlem - timeSlots olmadan direkt render
    if (type === 'platform') {
        timeSlotGroup.style.display = 'none';
        await renderPlatformChecklistWithStatus(container, branch, date);
        checklistLoadingLock = false;
        return;
    }
    
    if (timeSlots.length > 0) {
        // Otomatik saat seçimi - mevcut saate göre uygun pencereyi bul
        const now = new Date();
        const currentMinutes = now.getHours() * 60 + now.getMinutes();
        ...
        timeSlots.forEach(slot => {
            const window = getCheckTimeWindow(type, slot, branch);
            if (window) {
                const slotInfo = {
                    time: slot,
                    window,
                    status: getCheckStatus(type, slot, branch, false)
                };
                
                // Aktif veya geç penceresindeki slotları ekle
                if (window.isInWindow || window.isLate) {
                    availableSlots.push(slotInfo);
                    if (!autoSelectedSlot) autoSelectedSlot = slot;
                }
                // Henüz açılmamış ama bugün için olan slotlar
                else if (window.isBeforeWindow && !window.isMissed) {
                    availableSlots.push(slotInfo);
                }
            }
        });
        
        // Yönetici tüm saatleri görebilir
        const isAdmin = ['yonetici', 'bolge_muduru'].includes(currentUser?.role);
        
        if (isAdmin) {
            timeSlotGroup.style.display = 'block';
            timeSlotSelect.innerHTML = '<option value="">-- Tüm Saatler --</option>' + 
                timeSlots.map(t => {
                    const window = getCheckTimeWindow(type, t, branch);
                    const status = window ? getCheckStatus(type, t, branch, false) : null;
                    let badge = '';
                    if (status) {
                        if (status.status === 'active') badge = ' 🟢';
                        else if (status.status === 'lateWindow') badge = ' 🟡';
                        else if (status.status === 'missed') badge = ' 🔴';
                        else if (status.status === 'waiting') badge = ' ⏳';
                    }
                    return `<option value="${t}" ${t === autoSelectedSlot ? 'selected' : ''}>${t}${badge}</option>`;
                }).join('');
        } else {
            // Normal personel - sadece aktif penceredeki slotları göster
            if (availableSlots.length === 0) {
                timeSlotGroup.style.display = 'none';
                container.innerHTML = `
                    <div style="text-align:center;padding:60px;color:var(--tx2)">
                        <div style="font-size:4rem;margin-bottom:16px">🔒</div>
                        <div style="font-size:1.1rem;margin-bottom:8px">Aktif Check Yok</div>
                        <div style="font-size:.9rem">Şu an doldurmanız gereken bir check bulunmuyor</div>
                        <div style="margin-top:16px;padding:12px;background:var(--bg3);border-radius:10px;font-size:.85rem">
                            <strong>Check Saatleri:</strong> ${timeSlots.join(', ')}
                        </div>
                    </div>
                `;
                return;
            ...
            // Otomatik olarak uygun slot'u seç
            if (autoSelectedSlot) {
                timeSlotGroup.style.display = 'none';
                timeSlotSelect.value = autoSelectedSlot;
            } else {
                timeSlotGroup.style.display = 'block';
                timeSlotSelect.innerHTML = availableSlots.map(s => 
                    `<option value="${s.time}">${s.time}</option>`
                ).join('');
            }
        }
        
        // Saat seçilmemişse ve admin değilse bekle
        if (!timeSlotSelect.value && !autoSelectedSlot) {
            container.innerHTML = `
                <div style="text-align:center;padding:60px;color:var(--tx2)">
                    <div style="font-size:4rem;margin-bottom:16px">⏰</div>
                    <div style="font-size:1.1rem;margin-bottom:8px">Saat Dilimi Seçin</div>
                    <div style="font-size:.9rem">Bu checklist günün belirli saatlerinde doldurulur</div>
                </div>
            `;
            return;
        }
    ...
    // Zaman penceresi bilgisi göster
    let windowInfo = '';
            ...
            windowInfo = `
                <div style="margin-bottom:16px;padding:16px;border-radius:12px;background:var(--bg3);border:1px solid var(--cardBorder)">
                    <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px">
                        <div>
                            <div style="font-weight:600;color:var(--tx);margin-bottom:4px">⏱️ ${timeSlot} Check</div>
                            <div style="font-size:.8rem;color:var(--tx2)">
                                📗 ${window.start}-${window.end} arası: <strong style="color:#27ae60">+${rules.points.onTime} puan</strong>
                                ${rules.lateWindow > 0 ? `<br>📙 ${window.end}-${window.lateEnd} arası: <strong style="color:#f39c12">${rules.points.late} puan</strong>` : ''}
                                <br>📕 Süre sonrası: <strong style="color:#e74c3c">${rules.points.missed} puan</strong>
                            </div>
                        </div>
                        <div style="text-align:right">
                            <div style="padding:8px 16px;border-radius:20px;color:#fff;font-weight:600;${statusClass}">
                                ${remaining.isExpired ? '⏰ Süre Doldu!' : `⏱️ ${remaining.text}`}
                            </div>
                        </div>
                    </div>
                </div>
    ...
    try {
        ...
        try {
            ...
            console.warn('Firestore erişim hatası:', dbError);
        }
        ...
        if (items.length === 0) {
            ...
            if (items.length > 0) {
                try {
                    ...
                    console.log(`İlk kurulum: ${checklistId} Firebase'e kaydedildi`);
                } catch (e) {
                    console.warn('İlk kayıt hatası:', e);
                }
            }
        }
        
        // Eğer hala items boşsa hata göster
        if (!items || items.length === 0) {
            container.innerHTML = windowInfo + `<div style="text-align:center;padding:60px;color:var(--tx2)">
                <svg width="48" height="48" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" style="opacity:.4;margin-bottom:16px">
                    <circle cx="12" cy="12" r="10"/><path d="M12 8v4M12 16h.01"/>
                </svg>
                <div style="font-size:1.1rem;margin-bottom:8px;color:var(--tx)">Checklist Verisi Bulunamadı</div>
                <div style="font-size:.85rem;margin-bottom:16px;max-width:300px;margin-left:auto;margin-right:auto">
                    <strong>${branch}</strong> şubesi için <strong>${type}</strong> checklist'i henüz yüklenmemiş.
                </div>
                <div style="font-size:.8rem;color:var(--tx3)">
                    Yönetici: <code>seed.html</code> dosyasını açıp verileri yükleyin.
                </div>
            </div>`;
            return;
        }
        
        // Daha önce doldurulmuş mu kontrol et
        ...
        try {
            ...
            console.warn('Submission kontrol hatası:', subError);
        }
        ...
        try {
            ...
            const weekKey = formatDateLocal(weekStart);
            const checkHour = timeSlot ? parseInt(timeSlot.split(':')[0]) : 12;
            ...
            if (targetShift && targetShift.personelShifts) {
                for (const [name, days] of Object.entries(targetShift.personelShifts)) {
                    const todayShift = days[dayIndex] || '';
                    const cleanShift = todayShift.replace(/\*/g, '').trim().toUpperCase();
                    
                    if (checkHour < 15 && (cleanShift === 'A' || cleanShift === 'AÇILIŞ' || cleanShift.includes('AÇILI'))) {
                        shiftPersonnel.push(name);
                    } else if (checkHour >= 20 && (cleanShift === 'K' || cleanShift === 'KAPANIŞ' || cleanShift.includes('KAPANI'))) {
                        shiftPersonnel.push(name);
                    } else if (checkHour >= 15 && checkHour < 20) {
                        if (cleanShift && cleanShift !== 'İ' && cleanShift !== 'OFF' && cleanShift !== '-' && cleanShift !== 'X') {
                            shiftPersonnel.push(name);
                        }
                    }
                }
            ...
            console.warn('Shift personeli alınamadı:', e.message);
        }
        
        renderChecklistForm(items, branch, type, date, timeSlot, submittedData, shiftPersonnel);
        checklistLoadingLock = false;
        ...
        container.innerHTML = `<div style="color:var(--red);padding:20px;text-align:center">
            ...
            Checklist yüklenirken hata oluştu: ${error.message || 'Bilinmeyen hata'}
        </div>`;
        checklistLoadingLock = false;
    }
}
...
// Temizlik ve Bakım için tümünü göster flag'i
let showAllAylikItems = false;

function renderChecklistForm(items, branch, type, date, timeSlot, submittedData, shiftPersonnel = []) {
    const container = document.getElementById('checklistFormContainer');
    const isAdmin = currentUser?.role === 'yonetici' || currentUser?.role === 'bolge_muduru';
    const isManager = currentUser?.role === 'yonetici' || currentUser?.role === 'magaza_muduru' || currentUser?.role === 'bolge_muduru';
    ...
    // Temizlik ve Bakım için tarih filtreleme
    ...
    if (isAylik && !showAllAylikItems) {
        filteredItems = items.filter(item => {
            if (!item.dates || item.dates.length === 0) return true; // Her gün
            return item.dates.includes(dayOfMonth);
        });
    }
    
    // Zamanlama sıfırla (yeni checklist)
    if (!isSubmitted) {
        checklistStartTime = null;
        checklistFirstItemTime = null;
        // Beklenen süreyi sadece görünen maddeler için hesapla
        checklistExpectedDuration = filteredItems.reduce((sum, item) => sum + (item.duration || 60), 0);
    }
    
    // Kullanıcı adını belirle - öncelik: shift personeli > email > name
    let autoPersonName = '';
    ...
    if (shiftPersonnel && shiftPersonnel.length > 0) {
        ...
        // Fallback: email veya kullanıcı adı
        ...
        <div class="form-section">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;flex-wrap:wrap;gap:12px">
                <div>
                    ...
                    <div style="font-size:.85rem;color:var(--tx2)">${branch} • ${new Date(date).toLocaleDateString('tr-TR', {weekday: 'long', day: 'numeric', month: 'long'})}${timeSlot ? ' • ' + timeSlot : ''}</div>
                </div>
                <div style="display:flex;gap:8px;align-items:center">
                    ${isAylik ? `
                        <button class="btn btn-sm btn-ghost" onclick="toggleShowAllAylikItems()" style="font-size:.8rem">
                            ${showAllAylikItems ? '📅 Bugünü Göster' : '📋 Hepsini Göster'}
                        </button>
                        <span style="font-size:.75rem;color:var(--tx3);background:var(--bg3);padding:4px 10px;border-radius:12px">
                            ${filteredItems.length}/${totalItemCount} görev
                        </span>
                    ` : ''}
                    ${isSubmitted ? `
            ...
            ${isSubmitted ? `
                <div style="font-size:.75rem;color:var(--tx3);margin-bottom:16px">${submittedData.submittedByName} • ${submittedData.submittedAt?.toDate ? new Date(submittedData.submittedAt.toDate()).toLocaleTimeString('tr-TR', {hour: '2-digit', minute: '2-digit'}) : ''}</div>
            ` : ''}
            
            ${isManager && !isSubmitted ? `
                <!-- Düzenleme Toolbar - Sticky -->
            ...
            ${!checklistEditMode ? `
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:20px">
                    <div style="padding:16px;background:var(--bg3);border-radius:12px">
                        ...
                        ${shiftPersonnel && shiftPersonnel.length > 0 && !isSubmitted ? `
                            ${shiftPersonnel.length > 1 ? `
                                <select id="checklistPersonName" class="form-select" style="background:var(--bg2);border-color:var(--cardBorder)" autocomplete="off">
                                    ${shiftPersonnel.map(p => `<option value="${p}">${p}</option>`).join('')}
                                </select>
                            ` : `
                                <div style="font-size:1.1rem;font-weight:600;color:var(--tx);padding:8px 0">${shiftPersonnel[0]}</div>
                                <input type="hidden" id="checklistPersonName" value="${shiftPersonnel[0]}">
                            `}
                            <div style="font-size:.75rem;color:#008c95;margin-top:4px">✓ Shift'ten otomatik atandı</div>
                        ` : isSubmitted ? `
                        ...
                        ` : `
                            <div style="font-size:.9rem;color:var(--tx2);padding:8px 0">Shift bilgisi yok</div>
                    ...
                    <div style="padding:16px;background:var(--bg3);border-radius:12px">
                        ...
                        <div style="font-size:.75rem;color:var(--tx3)">${filteredItems.length} madde</div>
                    </div>
                </div>
                
                <!-- Başlangıç Zamanı Banner -->
                <div id="checklistTimerBanner" style="display:none;margin-bottom:16px;padding:14px 16px;background:linear-gradient(135deg,rgba(0,140,149,.15),rgba(0,140,149,.05));border:1px solid rgba(0,140,149,.3);border-radius:12px">
                    <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
                        <div style="display:flex;align-items:center;gap:8px">
                            ...
                            <span style="font-weight:600;color:var(--tx)">Checklist Başladı</span>
                        </div>
                ...
                <div style="margin-bottom:16px;padding:12px;background:rgba(255,134,116,.1);border-radius:10px;font-size:.85rem;color:var(--tx2)">
                    ⚠️ <strong>Önemli:</strong> İlk maddeyi işaretlediğinizde süre başlar. Tüm maddeleri dikkatlice kontrol edin.
                </div>
            ` : ''}
    ...
    filteredItems.forEach((item, index) => {
        ...
        // Tarih kutucukları HTML
        let datesHtml = '';
        if (isAylik) {
            if (hasDates) {
                // Tarihler beyaz arka planlı kutucuklarda
                ...
                // Her gün
                datesHtml = '<span style="font-size:.8rem;padding:4px 12px;border-radius:5px;background:rgba(255,255,255,.9);color:#1e293b;font-weight:600;box-shadow:0 1px 2px rgba(0,0,0,.1)">Her gün</span>';
            }
        }
        
        if (checklistEditMode) {
            // Düzenleme modu - SÜRÜKLE BIRAK + NUMARA
            html += `
                <div class="checklist-item" data-item-id="${item.id}" data-order="${index}" draggable="true"
                    ...
                    style="display:flex;align-items:flex-start;gap:12px;padding:16px;border-radius:14px;margin-bottom:12px;background:${hasColor ? itemColor : 'var(--bg2)'};box-shadow:0 2px 8px rgba(0,0,0,.1);cursor:grab;transition:transform .2s, opacity .2s">
                    
                    <!-- Sürükle Tutacak -->
                    <div class="drag-handle" style="display:flex;flex-direction:column;align-items:center;gap:4px;padding:4px;cursor:grab;opacity:.6">
                        <span style="font-size:1.1rem;font-weight:700;color:${hasColor ? 'rgba(255,255,255,.9)' : 'var(--tx2)'}">${itemNum}</span>
                        <svg width="16" height="16" fill="${hasColor ? 'rgba(255,255,255,.6)' : 'var(--tx3)'}" viewBox="0 0 24 24">
                    ...
                    <input type="checkbox" class="checklist-select-item" value="${item.id}" style="width:22px;height:22px;cursor:pointer;accent-color:#fff;margin-top:2px">
                    
                    <div style="flex:1">
                        <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;flex-wrap:wrap">
                            ${hasColor ? `<span style="font-size:.85rem;padding:5px 14px;border-radius:6px;background:rgba(255,255,255,.95);color:${itemColor};font-weight:700">${responsibleLabel}</span>` : ''}
                            ${datesHtml}
                        </div>
                        <div style="font-size:.95rem;color:${hasColor ? '#fff' : 'var(--tx)'};line-height:1.6;font-weight:500">${item.text}</div>
                    </div>
                    
                    <!-- Düzenle ve Sil Butonları -->
                    <div style="display:flex;gap:6px;flex-shrink:0">
                        <button onclick="showEditChecklistItemModal('${item.id}')" style="background:rgba(255,255,255,.2);border:none;color:${hasColor ? '#fff' : 'var(--tx2)'};cursor:pointer;padding:8px;border-radius:8px" title="Düzenle">
                        ...
                        <button onclick="deleteChecklistItem('${item.id}')" style="background:rgba(231,76,60,.2);border:none;color:#e74c3c;cursor:pointer;padding:8px;border-radius:8px" title="Sil">
            ...
            // Normal mod - TAM DOLGU RENGİ
            const canEdit = ['yonetici', 'bolge_muduru', 'magaza_muduru'].includes(currentUser?.role);
            html += `
                <div class="checklist-item" data-item-id="${item.id}" style="display:flex;align-items:flex-start;gap:14px;padding:18px 20px;border-radius:14px;margin-bottom:14px;background:${hasColor ? itemColor : 'var(--bg2)'};box-shadow:0 3px 12px rgba(0,0,0,.1)">
                    <!-- Madde Numarası -->
                    <div style="min-width:28px;height:28px;background:${hasColor ? 'rgba(255,255,255,.2)' : 'var(--bg3)'};border-radius:8px;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:.85rem;color:${hasColor ? 'rgba(255,255,255,.9)' : 'var(--tx2)'};flex-shrink:0">${itemNum}</div>
                    
                    <div style="display:flex;flex-direction:row;gap:8px;flex-shrink:0">
                        <label style="cursor:${isSubmitted ? 'default' : 'pointer'}">
                            <input type="radio" name="check_${item.id}" value="done" ${isChecked ? 'checked' : ''} ${isSubmitted ? 'disabled' : ''} style="display:none">
                            <span class="check-btn done" onclick="${isSubmitted ? '' : `selectCheckOption(this, '${item.id}', 'done')`}"
                                style="width:44px;height:44px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:1.4rem;
                                background:${isChecked ? '#22c55e' : 'rgba(255,255,255,.95)'};
                                border:3px solid ${isChecked ? '#22c55e' : 'rgba(34,197,94,.4)'};
                                color:${isChecked ? '#fff' : '#22c55e'};
                                transition:all .15s;font-weight:bold;cursor:pointer;box-shadow:0 2px 6px rgba(0,0,0,.1)">✓</span>
                        </label>
                        <label style="cursor:${isSubmitted ? 'default' : 'pointer'}">
                            <input type="radio" name="check_${item.id}" value="notdone" ${isNotDone ? 'checked' : ''} ${isSubmitted ? 'disabled' : ''} style="display:none">
                            <span class="check-btn notdone" onclick="${isSubmitted ? '' : `selectCheckOption(this, '${item.id}', 'notdone')`}"
                                style="width:44px;height:44px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:1.4rem;
                                background:${isNotDone ? '#ef4444' : 'rgba(255,255,255,.95)'};
                                border:3px solid ${isNotDone ? '#ef4444' : 'rgba(239,68,68,.4)'};
                    ...
                    <div style="flex:1">
                        ...
                        <div style="font-size:1rem;color:${hasColor ? '#fff' : 'var(--tx)'};line-height:1.6;font-weight:500">${item.text}</div>
                    </div>
                    ${canEdit && !isSubmitted ? `
                    <button onclick="showEditChecklistItemModal('${item.id}')" style="background:rgba(0,140,149,.15);border:none;color:#008c95;cursor:pointer;padding:10px;border-radius:10px;flex-shrink:0;transition:all .2s" title="Düzenle" onmouseover="this.style.background='rgba(0,140,149,.25)'" onmouseout="this.style.background='rgba(0,140,149,.15)'">
                        <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                    </button>
                    ` : ''}
                </div>
    ...
    html += `
            ...
            ${!checklistEditMode && isSubmitted ? `
                <div style="margin-top:24px;text-align:center;color:var(--tx2)">
                    Bu checklist zaten tamamlanmış
                </div>
            ` : ''}
            ${checklistEditMode ? `
...
// Temizlik ve Bakım - Hepsini/Bugünü Göster toggle
...
// Sıralama değişince numaraları güncelle ve Firebase'e kaydet
async function updateChecklistOrder() {
    ...
    try {
        ...
        if (doc.exists) {
            ...
            // Yeni sıraya göre items dizisini yeniden oluştur
        ...
        console.error('Sıralama kayıt hatası:', error);
...
async function deleteSelectedChecklistItems(branch, type) {
    ...
    if (selected.length === 0) {
        toast('Silmek için madde seçin', 'error');
        return;
    }
...
function selectCheckOption(btn, itemId, value) {
    ...
    // İlk madde işaretlendiğinde modal göster
    if (!checklistFirstItemTime) {
        ...
        showModal('✅ Checklist Başladı', `
            <div style="text-align:center;padding:20px 0">
                <div style="font-size:3rem;margin-bottom:16px">⏱️</div>
                <div style="font-size:1.2rem;font-weight:600;color:var(--tx);margin-bottom:8px">${typeName}</div>
                <div style="font-size:2rem;font-weight:700;color:#008c95;margin-bottom:8px">${timeStr}</div>
                <div style="font-size:1rem;color:var(--tx2)">${dateStr}</div>
                <div style="margin-top:16px;padding:12px;background:var(--bg3);border-radius:8px;font-size:.9rem;color:var(--tx2)">
                    Süre sayacı başladı. Tüm maddeleri kontrol edin.
                </div>
            </div>
        `, `
            <button class="btn btn-primary" onclick="confirmChecklistStart()">Tamam</button>
        `);
        
        // Geçici olarak zamanı sakla
        window.pendingChecklistStartTime = now;
    }
    
    // Tüm maddeler işaretlendi mi kontrol et
    checkAllItemsMarked();
}

// Checklist başlangıcını onayla
...
// Tüm maddelerin işaretlenip işaretlenmediğini kontrol et
...
async function renderPlatformChecklistWithStatus(container, branch, date) {
    ...
    try {
        ...
        console.warn('Platform submission kontrol hatası:', e);
    }
    
    // Ana render fonksiyonunu çağır
...
async function renderPlatformChecklist(container, branch, date, timeSlot, submittedData) {
    ...
    try {
        ...
        console.warn('Platform check shift alınamadı:', e.message);
    }
    ...
    // Platformlar
    const platforms = ['TRENDYOL', 'YEMEK SEPETİ', 'GETİR YEMEK', 'MİGROS'];
    
    let html = `
        <div class="form-section">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;flex-wrap:wrap;gap:12px">
                <div>
                    <div style="font-size:1.3rem;font-weight:700;color:var(--tx)">📱 Platform Check</div>
                ...
                <div style="display:flex;gap:12px;align-items:center">
                    ${isSubmitted ? `<div style="display:inline-flex;align-items:center;gap:6px;padding:8px 14px;background:rgba(39,174,96,.15);color:#27ae60;border-radius:20px;font-size:.85rem;font-weight:600">✓ Tamamlandı</div>` : ''}
                </div>
            </div>
            
            <div style="margin-bottom:20px;padding:16px;background:var(--bg3);border-radius:12px">
                <label class="form-label" style="margin-bottom:8px">👤 Sorumlu Personel</label>
                ${shiftPersonnel.length > 1 && !isSubmitted ? `
                    <select id="checklistPersonName" class="form-select" style="background:var(--bg2);border-color:var(--cardBorder)" autocomplete="off">
                        ${shiftPersonnel.map(p => `<option value="${p}">${p}</option>`).join('')}
                    </select>
                    <div style="font-size:.75rem;color:#008c95;margin-top:4px">✓ Shift'ten ${shiftPersonnel.length} personel</div>
                ` : `
                    <div style="font-size:1.1rem;font-weight:600;color:var(--tx);padding:8px 0">${submittedData?.personName || autoPersonName}</div>
                    <input type="hidden" id="checklistPersonName" value="${submittedData?.personName || autoPersonName}" autocomplete="off">
                    ${!isSubmitted && shiftPersonnel.length > 0 ? '<div style="font-size:.75rem;color:#008c95;margin-top:4px">✓ Shift\'ten otomatik atandı</div>' : ''}
                `}
            </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;overflow-x:auto">
                ...
                <div style="border-radius:12px;overflow:hidden;border:1px solid var(--cardBorder)">
                    <table style="width:100%;border-collapse:collapse" data-brand="MOONBERRY" data-branch="Şişli">
                        <thead>
                            <tr>
                ...
                <td style="padding:4px;text-align:center;border:1px solid var(--cardBorder);background:var(--bg2)">
                    <div style="display:flex;gap:2px;justify-content:center">
                        <span class="platform-btn" onclick="${isSubmitted ? '' : `selectPlatformCell(this, '${key}', 'open')`}"
                            ...
                            background:${isOpen ? '#008c95' : 'var(--bg3)'};border:1px solid ${isOpen ? '#008c95' : 'var(--cardBorder)'};color:${isOpen ? '#fff' : 'var(--tx3)'};cursor:pointer">✓</span>
                        <span class="platform-btn" onclick="${isSubmitted ? '' : `selectPlatformCell(this, '${key}', 'closed')`}"
                            style="display:inline-flex;width:24px;height:24px;border-radius:6px;align-items:center;justify-content:center;font-size:.85rem;
                            background:${isClosed ? '#e74c3c' : 'var(--bg3)'};border:1px solid ${isClosed ? '#e74c3c' : 'var(--cardBorder)'};color:${isClosed ? '#fff' : 'var(--tx3)'};cursor:pointer">✗</span>
                    </div>
            ...
            html += `
                <td style="padding:4px;text-align:center;border:1px solid var(--cardBorder);background:var(--bg2)">
                    <div style="display:flex;gap:2px;justify-content:center">
                        <span class="platform-btn" onclick="${isSubmitted ? '' : `selectPlatformCell(this, '${key}', 'open')`}"
                            style="display:inline-flex;width:24px;height:24px;border-radius:6px;align-items:center;justify-content:center;font-size:.85rem;
                            background:${isOpen ? '#ff8674' : 'var(--bg3)'};border:1px solid ${isOpen ? '#ff8674' : 'var(--cardBorder)'};color:${isOpen ? '#fff' : 'var(--tx3)'};cursor:pointer">✓</span>
                        <span class="platform-btn" onclick="${isSubmitted ? '' : `selectPlatformCell(this, '${key}', 'closed')`}"
                            style="display:inline-flex;width:24px;height:24px;border-radius:6px;align-items:center;justify-content:center;font-size:.85rem;
                            background:${isClosed ? '#e74c3c' : 'var(--bg3)'};border:1px solid ${isClosed ? '#e74c3c' : 'var(--cardBorder)'};color:${isClosed ? '#fff' : 'var(--tx3)'};cursor:pointer">✗</span>
                    </div>
...
async function submitChecklist(branch, type, date, timeSlot) {
    ...
    if (!personName) {
        toast('Lütfen personel adını girin', 'error');
        return;
    }
    
    // YÖNETİCİ KONTROLÜ - yöneticiler her şeyi doldurabilir
    ...
    if (!isManager) {
        ...
        // Kullanıcı kendi adını mı girdi?
        ...
        if (!isSelf) {
            toast('Sadece kendi adınıza checklist doldurabilirsiniz', 'error');
            return;
        }
        ...
        // Kullanıcı sorumlu listesinde mi?
        ...
        if (!isResponsible) {
            ...
            toast('Bu saatte shift\'iniz bulunmuyor. Checklist dolduramıyorsunuz.', 'error');
            return;
        }
    }
    
    // Tüm maddelerin cevaplandığını kontrol et
    const items = document.querySelectorAll('#checklistItems .checklist-item');
    const responses = [];
    ...
    if (!allAnswered) {
        toast('Lütfen tüm maddeleri işaretleyin', 'error');
        return;
    }
...
// Sorumlu olmayan personel uyarısı
    ...
    modal.innerHTML = `
        <div style="position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:10000;display:flex;align-items:center;justify-content:center;padding:20px">
            <div style="background:var(--bg2);border-radius:20px;max-width:420px;width:100%;padding:32px;text-align:center;border:1px solid var(--cardBorder)">
                ...
                <h3 style="font-size:1.3rem;font-weight:700;color:var(--tx);margin-bottom:12px">Sorumlu Personel Değilsiniz</h3>
                <p style="color:var(--tx2);margin-bottom:16px;line-height:1.6">
                    Bu checklist için sorumlu personeller:<br>
                    <strong style="color:var(--tx)">${responsibleNames.join(', ')}</strong>
                </p>
                <p style="color:var(--tx2);font-size:.85rem;margin-bottom:24px">
                    Sadece sorumlu personeller checklist doldurabilir. Yardım için müdürünüzle iletişime geçin.
                </p>
    ...
    modal.innerHTML = `
        <div style="position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:10000;display:flex;align-items:center;justify-content:center;padding:20px">
            <div style="background:var(--bg2);border-radius:20px;max-width:420px;width:100%;padding:32px;text-align:center;border:1px solid var(--cardBorder)">
                ...
                <h3 style="font-size:1.3rem;font-weight:700;color:var(--tx);margin-bottom:12px">Süre Kontrolü</h3>
                <p style="color:var(--tx2);margin-bottom:24px;line-height:1.6">
                    <strong>${typeName}</strong> için tahmini süre <strong>${expectedMinutes} dakika</strong> iken, 
                    siz <strong>${actualMinutes} dakikada</strong> tamamladınız.<br><br>
                    Tüm kontrolleri gerçekleştirdiğinizden emin misiniz?
                </p>
...
async function saveChecklistAndShare(branch, type, date, timeSlot, personName, responses, actualMinutes, expectedMinutes, doneCount, totalCount) {
    try {
        ...
        if (rules && timeSlot) {
            ...
            // Müdür hariç puan uygula
            const isMudur = ['magaza_muduru', 'bolge_muduru', 'yonetici'].includes(currentUser?.role);
            if (!isMudur && points !== 0) {
                // Puantaj notunu oluştur - normalize edilmiş personelId kullan
                await createAutoPuantajNote(
        ...
        console.error('Checklist kayıt hatası:', error);
        toast('Kayıt hatası!', 'error');
    }
}

// Checklist tamamlandı modal'ı - WhatsApp paylaşımı ile
function showChecklistCompleteModal(branch, type, date, timeSlot, personName, actualMinutes, expectedMinutes, doneCount, totalCount, points = 0, pointStatus = 'completed') {
    ...
    if (points !== 0 || pointStatus !== 'completed') {
        ...
        const pointIcon = points > 0 ? '🎉' : points < 0 ? '⚠️' : '✓';
        const pointLabel = pointStatus === 'onTime' ? 'Vaktinde' : pointStatus === 'late' ? 'Geç' : '';
        pointsHtml = `
    ...
    modal.innerHTML = `
        <div style="position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:10000;display:flex;align-items:center;justify-content:center;padding:20px">
            <div style="background:var(--bg2);border-radius:20px;max-width:480px;width:100%;padding:32px;text-align:center;border:1px solid var(--cardBorder)">
                ...
                <h3 style="font-size:1.5rem;font-weight:700;color:var(--tx);margin-bottom:8px">Checklist Tamamlandı!</h3>
                <p style="color:var(--tx2);margin-bottom:24px">${typeName} • ${branch}</p>
                ...
                <div style="background:var(--bg3);border-radius:12px;padding:20px;margin-bottom:24px;text-align:left">
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
                        ...
                        <div>
                            <div style="font-size:.75rem;color:var(--tx3);margin-bottom:4px">Süre</div>
...
function shareChecklistToWhatsApp(branch, typeName, personName, dateStr, timeStr, duration, rate, done, total) {
    ...
    const message = `✅ *CHECKLIST TAMAMLANDI*
...
✓ Tamamlanan: ${done}/${total} (%${rate})

_Moonberry İK Sistemi_`;

    const encodedMessage = encodeURIComponent(message);
    
    // WhatsApp grup linkini aç
    if (whatsappLink) {
        // Önce mesajı kopyala
        navigator.clipboard?.writeText(message).then(() => {
            toast('Mesaj kopyalandı! WhatsApp grubu açılıyor...', 'success');
        }).catch(() => {});
...
async function submitPlatformChecklist(branch, date, timeSlot) {
    ...
    if (!allAnswered) {
        toast('Lütfen tüm platformları işaretleyin', 'error');
        return;
    }
    ...
    try {
        ...
        // Müdür hariç herkes için kaydedilir
        const isMudur = ['magaza_muduru', 'bolge_muduru', 'yonetici'].includes(currentUser?.role);
        ...
        // Başarı mesajı ve modal
        showPlatformCompleteModal(date, personName, moonberryOpen, moonberryClosed, sunberryOpen, sunberryClosed);
        
    } catch (error) {
        console.error('Platform check kayıt hatası:', error);
        toast('Kayıt hatası!', 'error');
    }
}

// Platform check tamamlandı modal'ı
    ...
    modal.innerHTML = `
        <div style="position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:10000;display:flex;align-items:center;justify-content:center;padding:20px">
            <div style="background:var(--bg2);border-radius:20px;max-width:480px;width:100%;padding:32px;text-align:center;border:1px solid var(--cardBorder)">
                ...
                <h3 style="font-size:1.4rem;font-weight:700;color:var(--tx);margin-bottom:8px">Platform Check Tamamlandı!</h3>
                <p style="color:var(--tx2);margin-bottom:24px">${dateStr} • ${timeStr}</p>
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:24px">
                    <div style="background:#008c95;border-radius:12px;padding:16px;color:#fff">
                        ...
                        <div style="font-size:.9rem">✓ Açık: ${moonberryOpen}</div>
                        <div style="font-size:.9rem">✗ Kapalı: ${moonberryClosed}</div>
                    </div>
                    <div style="background:#ff8674;border-radius:12px;padding:16px;color:#fff">
                        ...
                        <div style="font-size:.9rem">✓ Açık: ${sunberryOpen}</div>
                        <div style="font-size:.9rem">✗ Kapalı: ${sunberryClosed}</div>
                    </div>
                </div>
...
// Yeni checklist türü ekleme modal
function showAddChecklistTypeModal() {
    const branch = document.getElementById('checklistSube')?.value;
    
    if (!branch) {
        toast('Önce şube seçin', 'error');
        return;
    }
    
    showModal('Yeni Checklist Türü Ekle', `
        <div class="form-group">
            <label class="form-label">Şube</label>
        ...
        <div class="form-group">
            ...
            <input type="text" id="newChecklistTypeId" class="form-input" placeholder="örn: ogle_kontrol">
            <div style="font-size:.75rem;color:var(--tx3);margin-top:4px">Küçük harf, boşluksuz, Türkçe karakter olmadan</div>
        </div>
        <div class="form-group">
            <label class="form-label">Tür Adı *</label>
            <input type="text" id="newChecklistTypeName" class="form-input" placeholder="örn: Öğle Kontrol">
        </div>
        <div class="form-group">
            <label class="form-label">İkon</label>
            <select id="newChecklistTypeIcon" class="form-select">
                <option value="clipboard-check">📋 Checklist</option>
                <option value="sun">☀️ Güneş (Açılış)</option>
                <option value="moon">🌙 Ay (Kapanış)</option>
                <option value="clock">🕐 Saat</option>
                <option value="calendar">📅 Takvim</option>
                <option value="refresh-cw">🔄 Güncel</option>
                <option value="coffee">☕ Kahve</option>
                <option value="truck">🚚 Araç</option>
            </select>
        </div>
        <div class="form-group">
            <label class="form-label">Saat Dilimleri (opsiyonel)</label>
            <input type="text" id="newChecklistTypeTimeSlots" class="form-input" placeholder="örn: 10:00,14:00,18:00">
            <div style="font-size:.75rem;color:var(--tx3);margin-top:4px">Virgülle ayırın, boş bırakılırsa saat dilimi olmaz</div>
        </div>
    `, `
        <button class="btn btn-ghost" onclick="closeModal()">İptal</button>
...
async function addChecklistType(branch) {
    ...
    if (!typeId || !typeName) {
        toast('Tür ID ve adı gerekli', 'error');
        return;
    }
    
    if (!/^[a-z0-9_]+$/.test(typeId)) {
        toast('Tür ID sadece küçük harf, rakam ve alt çizgi içerebilir', 'error');
        return;
    }
    ...
    try {
        ...
        closeModal();
        toast('Checklist türü eklendi', 'success');
        loadChecklistTypes();
...
function showAddChecklistItemModal() {
    ...
    if (!branch || !type) {
        toast('Önce checklist seçin', 'error');
        return;
    }
    
    const isAylik = type === 'aylik';
    
    // Tarih seçenekleri (1-31)
    let dateOptions = '';
    for (let i = 1; i <= 31; i++) {
    ...
    showModal('Yeni Madde Ekle', `
        ...
        <div class="form-group">
            ...
            <select id="newChecklistItemResponsible" class="form-input" onchange="updateResponsibleColor()">
                <option value="">Seçin...</option>
                <option value="AÇILIŞÇI" data-color="#0ea5e9">🔵 Açılışçı</option>
                <option value="HER SHIFT" data-color="#eab308">🟡 Her Shift</option>
                <option value="ARACI" data-color="#dc2626">🔴 Aracı</option>
                <option value="KAPANIŞÇI" data-color="#f97316">🟠 Kapanışçı</option>
            </select>
        </div>
        <div class="form-group">
            <label class="form-label">Tarihler</label>
            <div style="margin-bottom:8px">
                <button type="button" class="btn btn-sm btn-ghost" onclick="selectAllDates()">Tümünü Seç</button>
                <button type="button" class="btn btn-sm btn-ghost" onclick="selectOddDates()">Tek Günler (1,3,5...)</button>
                <button type="button" class="btn btn-sm btn-ghost" onclick="selectEvenDates()">Çift Günler (2,4,6...)</button>
            ...
            <div style="margin-top:8px">
                <label style="display:flex;align-items:center;gap:8px;cursor:pointer">
                    ...
                    <span style="font-weight:600">Her gün (tarih seçmeden)</span>
                </label>
            </div>
        </div>
        ` : `
        <div class="form-group">
            ...
            <input type="text" id="newChecklistItemPenalty" class="form-input" placeholder="örn: -100 puan">
        </div>
        `}
    `, `
        <button class="btn btn-ghost" onclick="closeModal()">İptal</button>
...
async function addChecklistItem(branch, type) {
    ...
    try {
        ...
        console.error('Madde ekleme hatası:', error);
        toast('Ekleme hatası!', 'error');
    }
}
...
async function deleteChecklistItem(itemId) {
    ...
    try {
        ...
        console.error('Madde silme hatası:', error);
        toast('Silme hatası!', 'error');
    }
}

// Checklist madde düzenle modal
async function showEditChecklistItemModal(itemId) {
    const branch = document.getElementById('checklistSube')?.value;
    const type = document.getElementById('checklistType')?.value;
    
    if (!branch || !type) {
        toast('Checklist bulunamadı', 'error');
        return;
    }
    ...
    try {
        ...
        if (!item) {
            toast('Madde bulunamadı', 'error');
            return;
        }
        
        const isAylik = type === 'aylik';
        
        // Tarih checkboxları (aylik için)
        let dateOptions = '';
        if (isAylik) {
            for (let i = 1; i <= 31; i++) {
        ...
        showModal('Madde Düzenle', `
            ...
            <div class="form-group">
                ...
                <select id="editChecklistItemResponsible" class="form-input">
                    <option value="">Seçin...</option>
                    <option value="AÇILIŞÇI" data-color="#0ea5e9" ${item.responsible === 'AÇILIŞÇI' ? 'selected' : ''}>🔵 Açılışçı</option>
                    <option value="HER SHIFT" data-color="#eab308" ${item.responsible === 'HER SHIFT' ? 'selected' : ''}>🟡 Her Shift</option>
                    <option value="ARACI" data-color="#dc2626" ${item.responsible === 'ARACI' ? 'selected' : ''}>🔴 Aracı</option>
                    <option value="KAPANIŞÇI" data-color="#f97316" ${item.responsible === 'KAPANIŞÇI' ? 'selected' : ''}>🟠 Kapanışçı</option>
                </select>
            </div>
            <div class="form-group">
                <label class="form-label">Tarihler</label>
                <div style="margin-bottom:8px">
                    <button type="button" class="btn btn-sm btn-ghost" onclick="editSelectAllDates()">Tümünü Seç</button>
                    <button type="button" class="btn btn-sm btn-ghost" onclick="editSelectOddDates()">Tek Günler</button>
                    <button type="button" class="btn btn-sm btn-ghost" onclick="editSelectEvenDates()">Çift Günler</button>
            ...
            <div class="form-group">
                <label class="form-label">Ceza (opsiyonel)</label>
                <input type="text" id="editChecklistItemPenalty" class="form-input" value="${item.penalty || ''}" placeholder="örn: -100 puan">
            </div>
            `}
        `, `
            <button class="btn btn-ghost" onclick="closeModal()">İptal</button>
        ...
        console.error('Madde yükleme hatası:', error);
        toast('Yükleme hatası!', 'error');
    }
}

// Edit modal tarih yardımcıları
...
async function editChecklistItem(itemId, branch, type) {
    ...
    try {
        ...
        closeModal();
        toast('Madde güncellendi', 'success');
        loadChecklist();
        
    } catch (error) {
        console.error('Madde güncelleme hatası:', error);
        toast('Güncelleme hatası!', 'error');
    }
}

// Checklist geçmişi göster
async function showChecklistHistory() {
    ...
    try {
        ...
        // JavaScript'te filtrele ve sırala
        let submissions = [];
        snapshot.forEach(doc => {
            const data = doc.data();
            // Type filtresi
        ...
        if (submissions.length === 0) {
            html += '<p style="color:var(--tx2);text-align:center;padding:20px">Henüz checklist kaydı yok</p>';
        } else {
                ...
                html += `
                    <div style="display:flex;align-items:center;gap:12px;padding:12px;background:var(--bg2);border-radius:10px;margin-bottom:8px">
                        ...
                        <div style="flex:1">
                            ...
                            <div style="font-size:.8rem;color:var(--tx2)">${data.personName} • ${date.toLocaleDateString('tr-TR')} ${date.toLocaleTimeString('tr-TR', {hour: '2-digit', minute: '2-digit'})}</div>
                        </div>
                        <div style="text-align:right">
        ...
        showModal('📊 Checklist Geçmişi', html, '<button class="btn btn-ghost" onclick="closeModal()">Kapat</button>');
...
function showChecklistSettings() {
    ...
    let html = `
        <p style="color:var(--tx2);margin-bottom:16px">${branch} için checklist ayarları</p>
    ...
    showModal('⚙️ Checklist Ayarları', html, '<button class="btn btn-ghost" onclick="closeModal()">Kapat</button>');
}

// Default checklist maddeleri - TÜM EXCEL MADDELERİ
function getDefaultChecklistItems(branch, type) {
    // Temizlik Çizelgesi maddeleri
    if (type === 'aylik') {
        return [
            { id: 'tem_01', text: 'Tüm masalar ve sandalyeler temizlendi', category: 'Salon' },
            { id: 'tem_02', text: 'Zemin süpürüldü ve silindi', category: 'Salon' },
            { id: 'tem_03', text: 'Vitrin camları temizlendi', category: 'Salon' },
            { id: 'tem_04', text: 'Menü tahtaları/standları temizlendi', category: 'Salon' },
            { id: 'tem_05', text: 'Kapı girişi ve paspaslar temizlendi', category: 'Salon' },
            { id: 'tem_06', text: 'Bar tezgahı dezenfekte edildi', category: 'Bar' },
            { id: 'tem_07', text: 'Espresso makinesi temizlendi', category: 'Bar' },
            { id: 'tem_08', text: 'Öğütücüler temizlendi', category: 'Bar' },
            { id: 'tem_09', text: 'Blenderlar temizlendi', category: 'Bar' },
            { id: 'tem_10', text: 'Buzdolabı rafları düzenlendi', category: 'Bar' },
            { id: 'tem_11', text: 'Bardak ve fincanlar dizildi', category: 'Bar' },
            { id: 'tem_12', text: 'Çöpler boşaltıldı', category: 'Genel' },
            { id: 'tem_13', text: 'WC kontrol edildi ve temizlendi', category: 'WC' },
            { id: 'tem_14', text: 'Sabunluk ve kağıt havlu dolduruldu', category: 'WC' },
            { id: 'tem_15', text: 'Depo düzenlendi', category: 'Depo' },
            { id: 'tem_16', text: 'Ürün son kullanma tarihleri kontrol edildi', category: 'Depo' }
        ];
    }
    
    // Günlük Check maddeleri (açılış/kapanış)
    if (type === 'gunluk') {
        return [
            { id: 'gun_01', text: 'Kasa açılışı/kapanışı yapıldı', category: 'Kasa' },
            { id: 'gun_02', text: 'Espresso makinesi kontrol edildi', category: 'Ekipman' },
            { id: 'gun_03', text: 'Buzdolabı sıcaklıkları kontrol edildi', category: 'Gıda Güvenliği' },
            { id: 'gun_04', text: 'Malzeme eksikleri not edildi', category: 'Stok' },
            { id: 'gun_05', text: 'Tezgah temizliği yapıldı', category: 'Temizlik' }
        ];
    }
    
    // Diğer tipler için boş döndür - Firebase'den yüklenecek
    return [];
}


console.log('✓ legacy.js yüklendi');
